...
impl<'a> Renderer<'a> {
    ...
    pub fn draw_rect(&mut self, x: u32, y: u32, width: u32, height: u32, color: Color) {
        ...
        for i in y..core::cmp::min(y + height, res_y as u32) {
            for j in x..core::cmp::min(x + width, res_x as u32) {
                ...
                unsafe {
                    *ptr.add(pixel_offset)     = color.b;
                    *ptr.add(pixel_offset + 1) = color.g;
                    *ptr.add(pixel_offset + 2) = color.r;
                    *ptr.add(pixel_offset + 3) = 255;
                }
            }
        }
    }
    ...
    pub fn draw_text(&mut self, x: u32, y: u32, text: &str, color: Color) {
        let mut curr_x = x;
        for c in text.chars() {
            if c != ' ' { self.draw_char(curr_x, y, c, color); }
            curr_x += 10;
        }
    }
    ...
    pub fn fb_info(&self) -> FramebufferInfo { self.fb_info }
}
...
#[derive(Clone, Copy)]
pub enum OsType { Native, Linux, NestedBase }
