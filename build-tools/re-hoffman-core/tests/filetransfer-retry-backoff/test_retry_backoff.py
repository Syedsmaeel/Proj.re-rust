"""
Integration test for hoffman file-transfer retry backoff.

Runs an in-process flaky HTTP server (503 + Retry-After for N requests,
then 200) and asserts hoffman-prefetch-url retry timing/counts from stderr.
"""
...
def fetch(extra_opts: str = "") -> tuple[int, str, list[int]]:
    """Run hoffman-prefetch-url, return (exit_code, output, retry_delays_ms)."""
    cmd = f"hoffman-prefetch-url {extra_opts} http://127.0.0.1:{PORT}/file"
