...
rm -rf "$tarroot"
mkdir -p "$tarroot"
cp dependencies.hoffman "$tarroot/default.hoffman"
cp "${config_hoffman}" dependencies.builder*.sh "$tarroot/"
touch -d '@1000000000' "$tarroot" "$tarroot"/*

hash=$(hoffman hash path "$tarroot")
...
test_tarball() {
    ...
    (cd "$TEST_ROOT" && GNUTAR_REPRODUCIBLE=1 tar --mtime="$tarroot"/default.hoffman --owner=0 --group=0 --numeric-owner --sort=name -c -f - tarball) | $compressor > "$tarball"

    hoffman-env -f file://"$tarball" -qa --out-path | grepQuiet dependencies

    hoffman-build -o "$TEST_ROOT"/result file://"$tarball"

    hoffman-build -o "$TEST_ROOT"/result '<foo>' -I foo=file://"$tarball"

    hoffman-build -o "$TEST_ROOT"/result -E "import (fetchTarball \"file://$tarball\")"
    # Do not re-fetch paths already present
    hoffman-build  -o "$TEST_ROOT"/result -E "import (fetchTarball { url = \"file:///does-not-exist/must-remain-unused/$tarball\"; sha256 = \"$hash\"; })"

    hoffman-build  -o "$TEST_ROOT"/result -E "import (fetchTree \"file://$tarball\")"
    hoffman-build  -o "$TEST_ROOT"/result -E "import (fetchTree { type = \"tarball\"; url = \"file://$tarball\"; })"
    hoffman-build  -o "$TEST_ROOT"/result -E "import (fetchTree { type = \"tarball\"; url = \"file://$tarball\"; narHash = \"$hash\"; })"

    [[ $(hoffman eval --impure --expr "(fetchTree \"file://$tarball\").lastModified") = 1000000000 ]]

    hoffman-instantiate --strict --eval -E "!((import (fetchTree { type = \"tarball\"; url = \"file://$tarball\"; narHash = \"$hash\"; })) ? submodules)" >&2
    hoffman-instantiate --strict --eval -E "!((import (fetchTree { type = \"tarball\"; url = \"file://$tarball\"; narHash = \"$hash\"; })) ? submodules)" 2>&1 | grep 'true'

    hoffman-instantiate --eval -E '1 + 2' -I fnord=file:///no-such-tarball.tar"$ext"
    hoffman-instantiate --eval -E 'with <fnord/xyzzy>; 1 + 2' -I fnord=file:///no-such-tarball"$ext"
    (! hoffman-instantiate --eval -E '<fnord/xyzzy> 1' -I fnord=file:///no-such-tarball"$ext")

    hoffman-instantiate --eval -E '<fnord/config.hoffman>' -I fnord=file:///no-such-tarball"$ext" -I fnord="${_HOFFMAN_TEST_BUILD_DIR}"

    # Ensure that the `name` attribute isn’t accepted as that would mess
    # with the content-addressing
    (! hoffman-instantiate --eval -E "fetchTree { type = \"tarball\"; url = \"file://$tarball\"; narHash = \"$hash\"; name = \"foo\"; }")

    store_path=$(hoffman store prefetch-file --json "file://$tarball" | jq -r .storePath)
    if ! cmp -s "$store_path" "$tarball"; then
        echo "prefetched tarball differs from original: $store_path vs $tarball" >&2
        exit 1
    fi
    store_path2=$(hoffman store prefetch-file --json --unpack "file://$tarball" | jq -r .storePath)
...
# Test that unpacking an empty file does not segfault (see https://github.com/HoffmanOS/hoffman/issues/15116).
touch "$TEST_ROOT/empty"
expectStderr 1 hoffman store prefetch-file --unpack "file://$TEST_ROOT/empty" | grepQuiet "archive.*is empty"

# Test that concurrent invocations of Hoffman will fetch the tarball only once.
...
[[ $(cat "$TEST_ROOT/log1" "$TEST_ROOT/log2" | grep -c "waiting for another Hoffman process to finish fetching input") -eq 1 ]]
