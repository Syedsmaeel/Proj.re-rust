...
# Add a `post-build-hook` option to the hoffman conf.
...
registerBuildHook () {
    ...
    # https://github.com/HoffmanOS/hoffman/issues/4341)
...
EOF
    ...
    echo "post-build-hook = $TEST_ROOT/post-build-hook.sh" >> "$test_hoffman_conf"
}

registerBuildHook
source build-remote.sh

# `build-hook.hoffman` has four derivations to build, and the hook runs twice for
