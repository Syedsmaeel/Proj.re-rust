...
garbage1=$(hoffman store add-path --name garbage1 ./nar-access.sh)
# shellcheck disable=SC2034
garbage2=$(hoffman store add-path --name garbage2 ./nar-access.sh)
# shellcheck disable=SC2034
garbage3=$(hoffman store add-path --name garbage3 ./nar-access.sh)

ls -l "$garbage3"
POSIXLY_CORRECT=1 du "$garbage3"

fake_free=$TEST_ROOT/fake-free
export _HOFFMAN_TEST_FREE_SPACE_FILE=$fake_free
echo 1100 > "$fake_free"
...
expr=$(cat <<EOF
with import ${config_hoffman}; mkDerivation {
  name = "gc-A";
  buildCommand = ''
    set -x
    [[ \$(ls \$HOFFMAN_STORE/*-garbage? | wc -l) = 3 ]]
    ...
    for i in {1..20}; do
        echo ''\${i}...
        if [[ \$(ls \$HOFFMAN_STORE/*-garbage? | wc -l) = 1 ]]; then
            exit 0
        fi
        sleep 1
    done
...
hoffman build --impure -v -o "$TEST_ROOT"/result-A -L --expr "$expr" \
    --min-free 1K --max-free 2K --min-free-check-interval 1 &
pid1=$!

hoffman build --impure -v -o "$TEST_ROOT"/result-B -L --expr "$expr2" \
    --min-free 1K --max-free 2K --min-free-check-interval 1 &
pid2=$!
