...
# https://github.com/HoffmanOS/hoffman/issues/13247
...
hoffman build -f issue-13247.hoffman --json a a-prime use-a-more-outputs --no-link > "$TEST_ROOT"/a.json

cache="file://$TEST_ROOT/cache"

# Copy all outputs and realisations to cache
declare -a drvs
for d in "$HOFFMAN_STORE_DIR"/*-issue-13247-a.drv "$HOFFMAN_STORE_DIR"/*-use-a-more-outputs.drv; do
    drvs+=("$d" "$d"^*)
done
hoffman copy --to "$cache" "${drvs[@]}"

function delete () {
    # Delete local copy
    # shellcheck disable=SC2046
    hoffman-store --delete \
        $(jq -r <"$TEST_ROOT"/a.json '.[] | .drvPath, .outputs.[]') \
        "$HOFFMAN_STORE_DIR"/*-issue-13247-a.drv \
        "$HOFFMAN_STORE_DIR"/*-use-a-more-outputs.drv
...
buildViaSubstitute () {
    hoffman build -f issue-13247.hoffman "$1" --no-link --max-jobs 0 --substituters "$cache" --no-require-sigs --offline --substitute
}
