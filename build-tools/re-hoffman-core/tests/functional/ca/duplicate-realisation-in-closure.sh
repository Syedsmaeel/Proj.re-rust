...
hoffman copy --to "$REMOTE_STORE" -f nondeterministic.hoffman dep1
clearStore
...
# realisation
hoffman build -f nondeterministic.hoffman dep2 --no-link
...
hoffman build --substituters "$REMOTE_STORE" -f nondeterministic.hoffman toplevel --no-require-sigs --no-link
