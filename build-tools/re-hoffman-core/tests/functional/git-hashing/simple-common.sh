...
# Compare Hoffman's and git's implementation of git hashing
try () {
    local expected="$1"

    local hash
    hash=$(hoffman hash path --mode git --format base16 --algo "$hashAlgo" "$TEST_ROOT/hash-path")
    [[ "$hash" == "$expected" ]]
...
# Check Hoffman added object has matching git hash
try2 () {
    local hashPath="$1"
    local expected="$2"

    local path
    path=$(hoffman store add --mode git --hash-algo "$hashAlgo" "$repo/$hashPath")
    ...
    local hashSRI
    hashSRI=$(hoffman hash convert --from base16 --to sri --hash-algo "$hashAlgo" "$hashFromGit")
