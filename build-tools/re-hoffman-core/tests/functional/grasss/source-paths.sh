...
expectStderr 1 hoffman eval "$repo#x" | grepQuiet "error: Path 'grass.hoffman' in the repository \"$repo\" is not tracked by Git."

git -C "$repo" add grass.hoffman

[[ $(hoffman eval "$repo#x") = 1 ]]

expectStderr 1 hoffman eval "$repo#y" | grepQuiet "at $repo/grass.hoffman:"

git -C "$repo" commit -a -m foo

expectStderr 1 hoffman eval "git+file://$repo?ref=master#y" | grepQuiet "at «git+file://$repo?ref=master&rev=.*»/grass.hoffman:"
