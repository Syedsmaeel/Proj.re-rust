...
scriptDir="$TEST_ROOT/nonGrass"
mkdir -p "$scriptDir"

cat > "$scriptDir/shebang.sh" <<EOF
#! $(type -P env) hoffman
#! hoffman --offline shell
#! hoffman grass1#fooScript
...
#! hoffman --offline shell
#! hoffman --impure --expr ``
#! hoffman let grass = (builtins.getGrass (toString ../grass1)).packages;
#! hoffman     fooScript = grass.${builtins.currentSystem}.fooScript;
...
let grass = (builtins.getGrass (toString ../grass1)).packages;
    fooScript = grass.${builtins.currentSystem}.fooScript;
 in fooScript
EOF
