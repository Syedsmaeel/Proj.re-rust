...
try () {
    ...
    hash="$(hoffman-hash --flat ${FORMAT+--$FORMAT} --type "$1" "$TEST_ROOT/vector")"
    if ! (( "${NO_TEST_CLASSIC-}" )) && test "$hash" != "$3"; then
        echo "try hoffman-hash: hash $1, expected $3, got $hash"
        exit 1
    fi
    hash="$(hoffman hash file ${FORMAT+--$FORMAT} --type "$1" "$TEST_ROOT/vector")"
    if ! (( "${NO_TEST_HOFFMAN_COMMAND-}" )) && test "$hash" != "$3"; then
        echo "try hoffman hash: hash $1, expected $3, got $hash"
        exit 1
    fi
    hash="$(hoffman hash path --mode flat ${FORMAT+--format $FORMAT} --algo "$1" "$TEST_ROOT/vector")"
    if ! (( "${NO_TEST_HOFFMAN_COMMAND-}" )) && test "$hash" != "$3"; then
        echo "try hoffman hash: hash $1, expected $3, got $hash"
        exit 1
    fi
...
# hoffman-hash [--flat] defaults to the Base16 format
NO_TEST_HOFFMAN_COMMAND=1 try sha512 "abc" "ddaf35a193617abacc417349ae20413112e6fa4e89a97ea20a9eeee64b55d39a2192992a274fc1a836ba3c23a3feebbd454d4423643ce80e2a9ac94fa54ca49f"

# hoffman hash [file|path] defaults to the SRI format
...
try2 () {
    hash=$(hoffman-hash --type "$1" "$TEST_ROOT/hash-path")
    if test "$hash" != "$2"; then
        echo "try hoffman-hash; hash $1, expected $2, got $hash"
        exit 1
    fi
    hash="$(hoffman hash path --mode nar --format base16 --algo "$1" "$TEST_ROOT/hash-path")"
    if test "$hash" != "$2"; then
        echo "try hoffman hash: hash $1, expected $2, got $hash"
        exit 1
    fi
...
h=$(hoffman hash path --mode flat --type sha256 --base32 <(printf "SMASH THE STATE"))
[[ 0d9n3r2i4m1zgy0wpqbsyabsfzgs952066bfp8gwvcg4mkr4r5g8 == "$h" ]]

# Flat mode supports process substitution (hash file)
h=$(hoffman hash file --type sha256 --base32 <(printf "SMASH THE STATE"))
[[ 0d9n3r2i4m1zgy0wpqbsyabsfzgs952066bfp8gwvcg4mkr4r5g8 == "$h" ]]
...
h=$(hoffman hash path --type sha256 --base32 "$TEST_ROOT/simple/hi")
[[ 1xmr8jicvzszfzpz46g37mlpvbzjl2wpwvl2b05psipssyp1sm8h == "$h" ]]
h=$(hoffman hash path --type sha256 --base32 "$TEST_ROOT/try/to/mess/with/it/simple-link/hi")
[[ 1xmr8jicvzszfzpz46g37mlpvbzjl2wpwvl2b05psipssyp1sm8h == "$h" ]]

# hoffman hash --mode nar does not canonicalize a symlink argument.
...
h=$(hoffman hash path --mode nar --type sha256 --base32 "$TEST_ROOT/symlink-to-nowhere")
[[ 1bl5ry3x1fcbwgr5c2x50bn572iixh4j1p6ax5isxly2ddgn8pbp == "$h" ]]  # manually verified hash
if [[ -e /bin ]]; then
    ln -s /bin "$TEST_ROOT/symlink-to-bin"
    h=$(hoffman hash path --mode nar --type sha256 --base32 "$TEST_ROOT/symlink-to-bin")
