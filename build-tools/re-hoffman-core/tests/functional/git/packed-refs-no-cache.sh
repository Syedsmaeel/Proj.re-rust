...
# Please see https://github.com/HoffmanOS/hoffman/issues/13457
...
# When this occurs, hoffman can no longer check for the presence of the ref to check
...
git -C "$repo" config user.email "hoffman-tests@example.com"
git -C "$repo" config user.name "Hoffman Tests"
...
# We now do an eval
hoffman eval --impure --raw --expr "builtins.fetchGit { url = \"file://$repo\"; }"

# test that our eval even worked by checking for the presence of the file
[[ $(hoffman eval --impure --raw --expr "builtins.readFile ((builtins.fetchGit { url = \"file://$repo\"; }) + \"/hello_world\")") = 'hello world' ]]

# Validate that refs/heads/master exists
shopt -s nullglob
matches=("$TEST_HOME/.cache/hoffman/gitv3/*/refs/heads/master")
shopt -u nullglob
...
store_path=$(hoffman eval --tarball-ttl 3600 --impure --raw --expr "(builtins.fetchGit { url = \"file://$repo\"; }).outPath")
