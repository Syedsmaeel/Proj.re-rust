...
cat <<EOF >"$TEST_HOME/flake.hoffman"
{
...
sed -i "$TEST_HOME/flake.hoffman" -e 's/devShells/devShells2/' # remove devShells
[[ $(hoffman develop . -L --command sh -c "echo \$x") == "bar" ]]
sed -i "$TEST_HOME/flake.hoffman" -e 's/devShell/devShell2/' # remove devShell
