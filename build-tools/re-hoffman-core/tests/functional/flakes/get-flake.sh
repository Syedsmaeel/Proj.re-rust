...
cat > "$flake1Dir/subflake/flake.hoffman" <<EOF
{
  outputs = { self }:
    let
...
EOF
git -C "$flake1Dir" add subflake/flake.hoffman

[[ $(hoffman eval "$flake1Dir/subflake#x") = 123 ]]

[[ $(hoffman eval "$flake1Dir/subflake#y") = 123 ]]
