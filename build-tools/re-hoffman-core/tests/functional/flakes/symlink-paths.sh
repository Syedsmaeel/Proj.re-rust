...
test_symlink_points_to_flake() {
    ...
    [[ $(hoffman eval "$TEST_ROOT/flake1_sym#x") = 2 ]]
...
test_symlink_points_to_flake_in_subdir() {
    ...
    [[ $(hoffman eval "$TEST_ROOT/subdir_sym/flake1#x") = 2 ]]
...
test_symlink_points_to_dir_in_repo() {
    ...
    cat > "$repoDir/subdir/flake.hoffman" <<EOF
{
    outputs = { self }: { x = 2; };
}
EOF
    git -C "$repoDir" add subdir/flake.hoffman
...
EOF
    git -C "$repoDir" add subdir/flake.hoffman file
    git -C "$repoDir" commit -m Initial
    ...
    [[ $(hoffman eval "$repo2Dir/flake1_sym#x") == \"Hello\\n\" ]]
