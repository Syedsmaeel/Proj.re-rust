...
cat > "$flakeFollowsA"/flake.hoffman <<EOF
{
    description = "Flake A";
    inputs = {
        B = {
...
cat > "$flakeFollowsB"/flake.hoffman <<EOF
{
    description = "Flake B";
    inputs = {
        foobar.url = "path:$flakeFollowsA/flakeE";
...
cat > "$flakeFollowsC"/flake.hoffman <<EOF
{
    description = "Flake C";
    inputs = {
        foobar.url = "path:$flakeFollowsA/flakeE";
...
cat > "$flakeFollowsD"/flake.hoffman <<EOF
{
    description = "Flake D";
    inputs = {};
    outputs = { ... }: {};
...
cat > "$flakeFollowsE"/flake.hoffman <<EOF
{
    description = "Flake E";
    inputs = {};
    outputs = { ... }: {};
...
git -C "$flakeFollowsA" add flake.hoffman flakeB/flake.hoffman \
  flakeB/flakeC/flake.hoffman flakeD/flake.hoffman flakeE/flake.hoffman

hoffman flake metadata "$flakeFollowsA"

hoffman flake update --flake "$flakeFollowsA"

hoffman flake lock "$flakeFollowsA"
...
# Ensure removing follows from flake.hoffman removes them from the lockfile
...
git -C "$flakeFollowsA" add flake.hoffman

expect 1 hoffman flake lock "$flakeFollowsA" 2>&1 | grep '/flakeB.*is forbidden in pure evaluation mode'
expect 1 hoffman flake lock --impure "$flakeFollowsA" 2>&1 | grep "'flakeB' is too short to be a valid store path"
...
echo 123 > "$flakeFollowsA"/foo.hoffman

git -C "$flakeFollowsA" add flake.hoffman foo.hoffman

hoffman flake lock "$flakeFollowsA"

[[ $(hoffman eval --json "$flakeFollowsA"#e) = 123 ]]
...
hoffman flake lock "$flakeFollowsA" 2>&1 | grep "warning: input 'B' has an override for a non-existent input 'invalid'"
hoffman flake lock "$flakeFollowsA" 2>&1 | grep "warning: input 'B' has an override for a non-existent input 'invalid2'"
...
cat > "$flakeFollowsOverloadD/flake.hoffman" <<EOF
{
    description = "Flake D";
    inputs = {};
    outputs = { ... }: {};
...
cat > "$flakeFollowsOverloadC/flake.hoffman" <<EOF
{
    description = "Flake C";
    inputs.D.url = "path:./flakeD";
    outputs = { ... }: {};
...
cat > "$flakeFollowsOverloadB/flake.hoffman" <<EOF
{
    description = "Flake B";
    inputs = {
        C = {
...
cat > "$flakeFollowsOverloadA/flake.hoffman" <<EOF
{
    description = "Flake A";
    inputs = {
        B = {
...
git -C "$flakeFollowsOverloadA" add flake.hoffman flakeB/flake.hoffman \
  flakeB/flakeC/flake.hoffman flakeB/flakeC/flakeD/flake.hoffman

hoffman flake metadata "$flakeFollowsOverloadA"
hoffman flake update --flake "$flakeFollowsOverloadA"
hoffman flake lock "$flakeFollowsOverloadA"
...
cat > "$flakeFollowCycle"/flake.hoffman <<EOF
{
    description = "Flake A";
    inputs = {
        foo.follows = "bar";
...
checkRes=$(hoffman flake lock "$flakeFollowCycle" 2>&1 && fail "hoffman flake lock should have failed." || true)
...
# A -> B -> C
#
# And override B/C to flake D, first in A's flake.hoffman and then with --override-input.
#
# A -> B -> D
...
cat > "$flakeFollowsCustomUrlD/flake.hoffman" <<EOF
{
    description = "Flake D";
    inputs = {};
    outputs = { ... }: {};
...
cat > "$flakeFollowsCustomUrlC/flake.hoffman" <<EOF
{
    description = "Flake C";
    inputs = {};
    outputs = { ... }: {};
...
cat > "$flakeFollowsCustomUrlB/flake.hoffman" <<EOF
{
    description = "Flake B";
    inputs = {
        C = {
...
cat > "$flakeFollowsCustomUrlA/flake.hoffman" <<EOF
{
    description = "Flake A";
    inputs = {
        B = {
...
git -C "$flakeFollowsCustomUrlA" add flake.hoffman flakeB/flake.hoffman \
  flakeB/flakeC/flake.hoffman flakeB/flakeD/flake.hoffman

# lock "original" entry should contain overridden url
json=$(hoffman flake metadata "$flakeFollowsCustomUrlA" --json)
...
json=$(hoffman flake metadata "$flakeFollowsCustomUrlA" --override-input B/C "$flakeFollowsCustomUrlD" --json)
...
cat <<EOF > "$flakeFollowsD"/flake.hoffman
{ outputs = _: {}; }
EOF
cat <<EOF > "$flakeFollowsC"/flake.hoffman
{
  inputs.D.url = "path:nosuchflake";
  outputs = _: {};
}
EOF
cat <<EOF > "$flakeFollowsB"/flake.hoffman
{
  inputs.C.url = "path:$flakeFollowsC";
  outputs = _: {};
}
EOF
cat <<EOF > "$flakeFollowsA"/flake.hoffman
{
...
hoffman flake lock "$flakeFollowsA" --recreate-lock-file
...
expectStderr 1 hoffman flake lock "$flakeFollowsA" --recreate-lock-file | grepQuiet "flake input has both a flake reference and a follows attribute"
