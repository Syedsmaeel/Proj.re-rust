...
hash=$(hoffman hash path "$flake2Dir")
...
hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a1"
[[ -e $TEST_ROOT/result/simple.hoffman ]]

hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a2"
[[ $(cat "$TEST_ROOT/result") = bar ]]

hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a3"

hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a4"

hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a6"
[[ -e $TEST_ROOT/result/simple.hoffman ]]

hoffman build --impure --json --out-link "$TEST_ROOT/result" "$flake1Dir#a8"
diff common.sh "$TEST_ROOT/result"

expectStderr 1 hoffman build --impure --json --out-link "$TEST_ROOT/result" "$flake1Dir#a9" \
  | grepQuiet "has 0 entries in its context. It should only have exactly one entry"

hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir"#a10
[[ $(readlink -e "$TEST_ROOT/result") = *simple.drv ]]

expectStderr 1 hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a11" \
  | grepQuiet "has a context which refers to a complete source and binary closure"

hoffman build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a12"
[[ -e $TEST_ROOT/result/hello ]]

expectStderr 1 hoffman build --impure --json --out-link "$TEST_ROOT/result" "$flake1Dir#a13" \
...
hoffman build --json --no-link "$flake1Dir"#a14.foo | jq --exit-status '
  (.[0] |
