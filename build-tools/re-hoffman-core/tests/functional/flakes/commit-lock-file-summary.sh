...
cat > "$lockfileSummaryFlake/flake.hoffman" <<EOF
{
  inputs = {
...
git -C "$lockfileSummaryFlake" add flake.hoffman
git -C "$lockfileSummaryFlake" commit -m 'Add lockfileSummaryFlake'

testSummary="test summary 1"
hoffman flake lock "$lockfileSummaryFlake" --commit-lock-file --commit-lock-file-summary "$testSummary"
...
hoffman flake lock "$lockfileSummaryFlake" --commit-lock-file --option commit-lockfile-summary "$testSummary"
[[ -e "$lockfileSummaryFlake/flake.lock" ]]
[[ -z $(git -C "$lockfileSummaryFlake" diff main || echo failed) ]]
[[ "$(git -C "$lockfileSummaryFlake" log --format=%s -n 1)" = "$testSummary" ]]
