...
rm -rf "${repo}-tmp" "$TEST_HOME/.cache/hoffman"
...
path=$(hoffman eval --raw --impure --expr "(builtins.fetchGit { url = $repo; ref = \"master\"; }).outPath")
...
valid_ref() {
    ...
    path1=$(hoffman eval --raw --impure --expr "(builtins.fetchGit { url = $repo; ref = ''$1''; }).outPath")
    [[ $path1 = "$path" ]]
...
invalid_ref() {
    ...
    expect 1 hoffman --debug eval --raw --impure --expr "(builtins.fetchGit { url = $repo; ref = ''$1''; }).outPath" 2>&1 | grep 'invalid Git branch/tag name' >/dev/null
}
