...
# Note: ssh://localhost bypasses ssh, directly invoking hoffman-store as a
# child process. This allows us to test LegacySSHStore::buildDerivation().
# ssh-ng://... likewise allows us to test RemoteStore::buildDerivation().
hoffman build -L -v -f "$file" -o "$TEST_ROOT/result" --max-jobs 0 \
  --arg busybox "$busybox" \
...
testPrintOutPath=$(hoffman build -L -v -f "$file" --no-link --print-out-paths --max-jobs 0 \
  --arg busybox "$busybox" \
  --store "$TEST_ROOT/machine0" \
  --builders "$(join_by '; ' "${builders[@]}")"
)
...
output=$(hoffman path-info --store "$TEST_ROOT/machine1" --all)
...
output=$(hoffman path-info --store "$TEST_ROOT/machine2" --all)
...
output=$(hoffman path-info --store "$TEST_ROOT/machine3" --all)
echo "$output" | grepQuietInverse builder-build-remote-input-1.sh
echo "$output" | grepQuietInverse builder-build-remote-input-2.sh
...
hoffman log --store "$TEST_ROOT/machine0" --file "$file" --arg busybox "$busybox" "passthru.$i" | grep hi-$i
done

# Behavior of keep-failed
out="$(hoffman-build 2>&1 failing.hoffman \
  --no-out-link \
  --builders "$(join_by '; ' "${builders[@]}")"  \
  --keep-failed \
  --store "$TEST_ROOT/machine0" \
