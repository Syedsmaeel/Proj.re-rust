...
hoffman eval --json --expr '{ a.b.c = true; }' > "$TEST_HOME/actual.json"
diff -U3 "$TEST_HOME/expected-machine.json" "$TEST_HOME/actual.json"

hoffman eval --json --pretty --expr \
...
if type script &>/dev/null; then
  ...
  runScript hoffman eval --json --expr "{ a.b.c = true; }" > "$TEST_HOME/actual.json"
  ...
  runScript hoffman eval --json --pretty --expr "{ a.b.c = true; }" > "$TEST_HOME/actual.json"
  cat "$TEST_HOME/actual.json"
  grep -F '"a": {' "$TEST_HOME/actual.json"

  runScript hoffman eval --json --no-pretty --expr "{ a.b.c = true; }" > "$TEST_HOME/actual.json"
  cat "$TEST_HOME/actual.json"
  grep -F '"a":{' "$TEST_HOME/actual.json"
