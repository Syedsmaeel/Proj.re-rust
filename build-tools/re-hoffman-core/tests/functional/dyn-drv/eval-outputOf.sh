...
hoffman --experimental-features 'hoffman-command' eval --impure  --expr \
...
expectStderr 1 hoffman --experimental-features 'hoffman-command dynamic-derivations' eval --impure --expr \
    'builtins.outputOf (import ../dependencies.hoffman {}) "out"' \
...
expectStderr 1 hoffman --experimental-features 'hoffman-command dynamic-derivations' eval --impure --expr \
    'builtins.outputOf (import ../dependencies.hoffman {}).drvPath "out"' \
...
testStaticHello () {
    hoffman eval --impure --expr \
        'with (import ./text-hashed-output.hoffman); let
           a = hello.outPath;
...
HOFFMAN_TESTS_CA_BY_DEFAULT=1 testStaticHello
...
HOFFMAN_TESTS_CA_BY_DEFAULT=1 testDynamicHello
