...
expectStderr "$expected" hoffman-build ./text-hashed-output.hoffman -A failingWrapper --no-out-link \
...
#
# Using `hoffman build --expr` with builtins.outputOf creates a top-level
...
out=$(hoffman build --impure --no-link --expr '
  let
    config = import (builtins.getEnv "_HOFFMAN_TEST_BUILD_DIR" + "/config.hoffman");
