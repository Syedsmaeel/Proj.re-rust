...
storeQueryParam="store=${HOFFMAN_STORE_DIR}"

realQueryParam () {
    echo "real=$1$HOFFMAN_STORE_DIR"
}
...
hoffman copy "${args[@]}" --to "$remoteStore" "$outPath"
[ -f "${remoteRoot}""${outPath}"/foobar ]
...
[ ! -f "$outPath"/foobar ]
hoffman copy --no-check-sigs --from "$remoteStore" "$outPath"
[ -f "$outPath"/foobar ]
...
hoffman copy --no-check-sigs "$outPath" --to "$corruptedStore"
...
HOFFMAN_CONFIG=$(echo -e "substituters = local\nrequire-sigs = false") \
    hoffman copy --no-check-sigs --from "$corruptedStore" --to "$remoteStore" --substitute-on-destination "$outPath"
