...
setupStoreDirs () {
  ...
  mkdir -p "$storeVolume"/{store-a/hoffman/store,store-b,merged-store/hoffman/store,workdir}
}

# Mounting Overlay Store
mountOverlayfs () {
  mount -t overlay overlay \
    -o lowerdir="$storeA/hoffman/store" \
    -o upperdir="$storeBTop" \
    -o workdir="$storeVolume/workdir" \
    "$storeBRoot/hoffman/store" \
    || skipTest "overlayfs is not supported"

  # shellcheck disable=SC2329
  cleanupOverlay () {
    # shellcheck disable=2317
    umount -n "$storeBRoot/hoffman/store"
    # shellcheck disable=2317
...
remountOverlayfs () {
  mount -o remount "$storeBRoot/hoffman/store"
}
...
toRealPath () {
  ...
  echo "$storeDir""$(echo "$storePath" | sed "s^${HOFFMAN_STORE_DIR:-/hoffman/store}^^")"
}

initLowerStore () {
  # Init lower store with some stuff
  hoffman-store --store "$storeA" --add ../dummy

  # Build something in lower store
  drvPath=$(hoffman-instantiate --store "$storeA" ../hermetic.hoffman --arg withFinalRefs true --arg busybox "$busybox" --arg seed 1)
  # shellcheck disable=SC2034
  pathInLowerStore=$(hoffman-store --store "$storeA" --realise "$drvPath")
}
...
addTextToStore() {
  ...
  hoffman-store --store "$storeDir" --add "$filePath"
}
