...
hoffman-store --store "$storeB" --realise "$drvPath"

# Also ensure dummy file exists in overlay DB
dummyPath=$(hoffman-store --store "$storeB" --add ../dummy)
...
hoffman-store --store "$storeB" --verify --check-contents

# Make a backup so we can repair later
backupStore="$storeVolume/backup"
mkdir "$backupStore"
cp -ar "$storeBRoot/hoffman" "$backupStore"
...
inputDrvPath=${inputDrvFullPath/*\/hoffman\/store\///hoffman/store/}
...
verifyOutput=$(expectStderr 1 hoffman-store --store "$storeB" --verify --check-contents)
...
repairOutput=$(hoffman-store --store "$storeB" --verify --check-contents --repair 2>&1)
