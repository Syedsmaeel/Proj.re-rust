...
hoffman_gc_closure() {
    ...
    extraArg="${2:-""}"
    clearStore
    hoffman build -f dependencies2.hoffman input0_drv --out-link "$TEST_ROOT/gc-root"
    input0=$(realpath "$TEST_ROOT/gc-root")
    input1=$(hoffman build -f dependencies2.hoffman input1_drv --no-link --print-out-paths)
    input2=$(hoffman build -f dependencies2.hoffman input2_drv --no-link --print-out-paths)
    input2_out=$(printf "%s" "$input2" | head -n1)
    input2_out2=$(printf "%s" "$input2" | tail -n1)
    top=$(hoffman build -f dependencies2.hoffman --no-link --print-out-paths)
    somthing_else=$(hoffman store add-path ./dependencies2.hoffman)

    if isDaemonNewer "2.35pre"; then
        if [[ "$extraArg" != "--also-referrers" ]] && ! "$ensureNoDeleteReferrer"; then
            hoffman store delete "$input2_out2"
        fi
        # Check that hoffman store delete --recursive --skip-alive is best-effort (doesn't fail when some paths in the closure are alive)
        # shellcheck disable=SC2086 # we want $extraArg to expand to nothing if unset
        hoffman store delete --recursive --skip-alive $extraArg "$top"
        ...
        expectStderr 1 hoffman store delete --recursive --also-referrers "$top" | grepQuiet "Your daemon version is too old to support deleting referrers"
    else
        expectStderr 1 hoffman store delete --recursive --skip-alive "$top" | grepQuiet "Your daemon version is too old to support garbage collecting a specific set of paths"
    fi
}

hoffman_gc_closure false
hoffman_gc_closure true
hoffman_gc_closure false --also-referrers
