...
happy () {
    ...
    hoffman --store local?read-only=true path-info "$dummyPath"

    # `local://` also works.
    hoffman --store local://?read-only=true path-info "$dummyPath"

    # We can "write" an already-present store-path a read-only store, because no IO is actually required
    hoffman-store --store local?read-only=true --add dummy
}
...
expectStderr 1 hoffman-store --store local?read-only=true --add dummy | grepQuiet "database does not exist, and cannot be created in read-only mode"

# Make sure the store actually has a current-database, with at least one store object
dummyPath=$(hoffman-store --add dummy)

# Try again and make sure we fail when adding a item not already in the store
expectStderr 1 hoffman-store --store local?read-only=true --add eval.hoffman | grepQuiet "attempt to write a readonly database"
...
expectStderr 1 hoffman-store --add eval.hoffman | grepQuiet "This command may have been run as non-root in a single-user Hoffman installation"
expectStderr 1 hoffman-store --store local?read-only=true --add eval.hoffman | grepQuiet "Permission denied"
