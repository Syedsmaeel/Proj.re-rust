...
# `_HOFFMAN_TEST_ACCEPT` is non-empty also update the expected output so
...
function diffAndAcceptInner() {
    ...
    # Update expected if `_HOFFMAN_TEST_ACCEPT` is non-empty.
    if test -n "${_HOFFMAN_TEST_ACCEPT-}"; then
        cp "$got" "$expected"
...
function characterisationTestExit() {
    ...
    if test -n "${_HOFFMAN_TEST_ACCEPT-}"; then
        if (( "$badDiff" )); then
            ...
            set +x
            echo >&2 'NOTE: Environment variable _HOFFMAN_TEST_ACCEPT is defined,'
        ...
        if (( "$badDiff" )); then
            ...
            echo >&2 ''
            echo >&2 "    _HOFFMAN_TEST_ACCEPT=1 meson test --suite ${TEST_SUITE_NAME} ${TEST_NAME}"
            echo >&2 ''
