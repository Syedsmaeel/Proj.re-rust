...
hoffman eval --lint-url-literals fatal --expr '"http://example.com?foo=bar"' 2>&1 | grepQuietInverse "error:"

# Test: The setting can be enabled via HOFFMAN_CONFIG
expect 1 env HOFFMAN_CONFIG='lint-url-literals = fatal' hoffman eval --expr 'http://example.com' 2>&1 | grepQuiet "error: URL literal"

# Test: Using old experimental feature name produces helpful warning
hoffman eval --extra-experimental-features no-url-literals --expr '"test"' 2>&1 \
