...
jq '.name = "foo"' < "$TEST_HOME/simple.json" | expectStderr 1 hoffman derivation add | grepQuiet "has incorrect output"

# If we remove the input addressed to make it a deferred derivation, we
# still get the same result because Hoffman will see that need not be
# deferred and fill in the right input address for us.
drvPath3=$(jq '.outputs |= map_values(del(.path))' < "$TEST_HOME/simple.json" | hoffman derivation add)
[[ "$drvPath" = "$drvPath3" ]]
