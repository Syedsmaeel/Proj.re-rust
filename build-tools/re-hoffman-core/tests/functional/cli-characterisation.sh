...
# Characterization tests for hoffman-env, hoffman-build, and hoffman-instantiate.
# Captures error messages, output formats, and edge-case behavior.
#
# Expected output files in: cli-characterisation/
# Regenerate with: _HOFFMAN_TEST_ACCEPT=1 meson test -C build cli-characterisation
...
normalize() {
    sed -i \
        -e "s|${HOFFMAN_STORE_DIR:-/hoffman/store}/[a-z0-9]\{32\}|/test-root/store/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx|g" \
        -e "s|$TEST_ROOT|/test-root|g" \
        -e "s|$(pwd)|/pwd|g" \
        -e "s|$system|SYSTEM|g" \
        -e "s|/hoffman/store/[a-z0-9]\{32\}|/hoffman/store/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx|g" \
...
drvPath=$(hoffman-instantiate ./cli-characterisation/simple.hoffman)

# Constructor: "building more than one derivation output is not supported"
testName=hoffman-build-multi-output
echo "testing $testName"
if
    expect 1 hoffman-build "$drvPath!out,bin" \
        1> "cli-characterisation/$testName.out" \
        2> "cli-characterisation/$testName.err"
then
    normalize "cli-characterisation/$testName.out" "cli-characterisation/$testName.err"
...
testName=hoffman-build-bad-output
echo "testing $testName"
if
    expect 1 hoffman-build "$drvPath!nonexistent" \
        1> "cli-characterisation/$testName.out" \
        2> "cli-characterisation/$testName.err"
then
    normalize "cli-characterisation/$testName.out" "cli-characterisation/$testName.err"
