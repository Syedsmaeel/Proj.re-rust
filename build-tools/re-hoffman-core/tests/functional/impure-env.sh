...
varTest() {
    local var="$1"; shift
    local value="$1"; shift
    hoffman build --no-link -vL --argstr var "$var" --argstr value "$value" --impure "$@" --file impure-env.hoffman
    clearStore
}
...
echo 'impure-env = set_in_config=config_value' >> "$test_hoffman_conf"
...
sed -i -e '/^trusted-users =/d' "$test_hoffman_conf"
