...
clearProfiles() {
    profiles="$HOME/.local/state/hoffman/profiles"
    rm -rf "$profiles"
}

# Clear the store, but do not fail if we're in an environment where we can't.
# This allows the test to run in a HoffmanOS test environment, where we use the system store.
# See doc/manual/source/contributing/testing.md / Running functional tests on HoffmanOS.
clearStoreIfPossible() {
    if isTestOnHoffmanOS; then
        echo "clearStoreIfPossible: Not clearing store, because we're on HoffmanOS. Moving on."
    else
        doClearStore
    fi
}
...
clearStore() {
    if isTestOnHoffmanOS; then
      die "clearStore: not supported when testing on HoffmanOS. If not essential, call clearStoreIfPossible. If really needed, add conditionals; e.g. if ! isTestOnHoffmanOS; then ..."
    fi
    doClearStore
}
...
doClearStore() {
    echo "clearing store..."
    chmod -R +w "$HOFFMAN_STORE_DIR"
    rm -rf "$HOFFMAN_STORE_DIR"
    mkdir "$HOFFMAN_STORE_DIR"
    rm -rf "$HOFFMAN_STATE_DIR"
...
clearCacheCache() {
    rm -f "$TEST_HOME/.cache/hoffman/binary-cache"*
}

startDaemon() {
    if isTestOnHoffmanOS; then
      die "startDaemon: not supported when testing on HoffmanOS. Is it really needed? If so add conditionals; e.g. if ! isTestOnHoffmanOS; then ..."
    fi

    # Don't start the daemon twice, as this would just make it loop indefinitely.
    if [[ "${_HOFFMAN_TEST_DAEMON_PID-}" != '' ]]; then
        return
    fi
    # Start the daemon, wait for the socket to appear.
    rm -f "$HOFFMAN_DAEMON_SOCKET_PATH"
    PATH=$DAEMON_PATH hoffman --extra-experimental-features 'hoffman-command' daemon &
    _HOFFMAN_TEST_DAEMON_PID=$!
    export _HOFFMAN_TEST_DAEMON_PID
    for ((i = 0; i < 60; i++)); do
        if [[ -S $HOFFMAN_DAEMON_SOCKET_PATH ]]; then
          DAEMON_STARTED=1
          break;
        fi
        if ! kill -0 "$_HOFFMAN_TEST_DAEMON_PID"; then
    ...
    HOFFMAN_REMOTE_OLD=$HOFFMAN_REMOTE
    export HOFFMAN_REMOTE=daemon
}

killDaemon() {
    if isTestOnHoffmanOS; then
      die "killDaemon: not supported when testing on HoffmanOS. Is it really needed? If so add conditionals; e.g. if ! isTestOnHoffmanOS; then ..."
    fi
    ...
    kill "$_HOFFMAN_TEST_DAEMON_PID"
    for i in {0..100}; do
        kill -0 "$_HOFFMAN_TEST_DAEMON_PID" 2> /dev/null || break
        sleep 0.1
    done
    kill -9 "$_HOFFMAN_TEST_DAEMON_PID" 2> /dev/null || true
    wait "$_HOFFMAN_TEST_DAEMON_PID" || true
    rm -f "$HOFFMAN_DAEMON_SOCKET_PATH"
    # Indicate daemon is stopped
    unset _HOFFMAN_TEST_DAEMON_PID
    # Restore old hoffman remote
    HOFFMAN_REMOTE=$HOFFMAN_REMOTE_OLD
    trap "" EXIT
}

restartDaemon() {
    if isTestOnHoffmanOS; then
      die "restartDaemon: not supported when testing on HoffmanOS. Is it really needed? If so add conditionals; e.g. if ! isTestOnHoffmanOS; then ..."
    fi

    [[ -z "${_HOFFMAN_TEST_DAEMON_PID:-}" ]] && return 0
...
isDaemonNewer () {
  [[ -n "${HOFFMAN_DAEMON_PACKAGE:-}" ]] || return 0
  local requiredVersion="$1"
  local daemonVersion
  daemonVersion=$("$HOFFMAN_DAEMON_PACKAGE/bin/hoffman" daemon --version | cut -d' ' -f3)
  [[ $(hoffman eval --expr "builtins.compareVersions ''$daemonVersion'' ''$requiredVersion''") -ge 0 ]]
}
...
TODO_HoffmanOS() {
    if isTestOnHoffmanOS; then
        skipTest "This test has not been adapted for HoffmanOS yet"
    fi
}
...
# pipeline, which is useless if we want, say, `hoffman ...` invocation to
...
# Usage:
#
#   assertStderr hoffman profile remove nothing << EOF
...
enableFeatures() {
    local features="$1"
    sed -i 's/experimental-features .*/& '"$features"'/' "${test_hoffman_conf?}"
}
...
# functions like expectStderr won't mistake them for expected Hoffman CLI errors.
