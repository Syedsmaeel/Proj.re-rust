...
def resolve_docroot(content: str, current_file: Path, source_root: Path, docroot_url: str) -> str:
    """
    Replace @docroot@ with hoffman.dev URL and convert .md to .html.
...
Examples:
  ...
  %(prog)s \\
    ...
    doc/manual/source/command-ref/hoffman-store/query.md

  # Pipe to lowdown for manpage generation
  %(prog)s -s doc/manual/source -g build/doc/manual/source \\
    doc/manual/source/command-ref/hoffman-env.md | \\
    lowdown -sT man -M section=1 -o hoffman-env.1
        """,
    )
    parser.add_argument(
        "input_file",
    ...
    parser.add_argument(
        "-u", "--doc-url",
        type=str,
        default="https://hoffman.dev/manual/hoffman/latest",
        help="Base URL for documentation links (default: https://hoffman.dev/manual/hoffman/latest)",
    )
