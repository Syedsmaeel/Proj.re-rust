...
[coverage analysis report]: https://hydra.hoffmanos.org/job/hoffman/master/coverage/latest/download-by-type/report/coverage

```
# hoffman build .#hydraJobs.coverage
# xdg-open ./result/coverage/index.html
```

[Extensive records of build metrics](https://hydra.hoffmanos.org/job/hoffman/master/coverage#tabs-charts), such as test coverage over time, are also available online.
...
The tests for each Hoffman library (`libhoffmanexpr`, `libhoffmanstore`, etc..) live inside a directory `src/${library_name_without-hoffman}-test`.
Given an interface (header) and implementation pair in the original library, say, `src/libexpr/include/hoffman/expr/value/context.hh` and `src/libexpr/value/context.cc`, we write tests for it in `src/libexpr-tests/value/context.cc`, and (possibly) declare/define additional interfaces for testing purposes in `src/libexpr-test-support/include/hoffman/expr/tests/value/context.hh` and `src/libexpr-test-support/tests/value/context.cc`.

Data for unit tests is stored in a `data` subdir of the directory for each unit test executable.
For example, `libhoffmanstore` code is in `src/libstore`, and its test data is in `src/libstore-tests/data`.
The path to the `src/${library_name_without-hoffman}-test/data` directory is passed to the unit test executable with the environment variable `_HOFFMAN_TEST_UNIT_DATA`.
Note that each executable only gets the data for its tests.

The unit test libraries are in `src/${library_name_without-hoffman}-test-support`.
All headers are in a `tests` subdirectory so they are included with `#include "hoffman/tests/"`.
...
You can run the whole testsuite with `meson test` from the Meson build directory, or the tests for a specific component with `meson test hoffman-store-tests`.
...
```bash
GTEST_BRIEF=1 GTEST_FILTER='ErrorTraceTest.*' meson test hoffman-expr-tests -v
```
...
```bash
GTEST_BRIEF=1 GTEST_FILTER='Group.my-failing-test' meson test hoffman-expr-tests --gdb
```
...
Like with the functional characterisation, `_HOFFMAN_TEST_ACCEPT=1` is also used.
For example:
```shell-session
$ _HOFFMAN_TEST_ACCEPT=1 meson test hoffman-store-tests -v
...
...
...
```
will regenerate the "golden master" expected result for the `libhoffmanstore` characterisation tests.
...
Functional tests are run during `installCheck` in the `hoffman` package build, as well as separately from the build, in VM tests.
...
```shell-session
$ meson test --suite ca
ninja: Entering directory `/home/jcericson/src/hoffman/master/build'
ninja: no work to do.
[1-20/20] 🌑 hoffman-functional-tests:ca / ca/why-depends                                1/20 hoffman-functional-tests:ca / ca/hoffman-run                                  OK               0.16s
[2-20/20] 🌒 hoffman-functional-tests:ca / ca/why-depends                                2/20 hoffman-functional-tests:ca / ca/import-derivation                        OK               0.17s
```
...
ninja: Entering directory `/home/jcericson/src/hoffman/master/build'
ninja: no work to do.
1/1 hoffman-functional-tests:main / ${testName}        OK               0.41s
...
Full log written to /home/jcericson/src/hoffman/master/build/meson-logs/testlog.txt
```
...
```shell-session
$ TEST_NAME=${testName} HOFFMAN_REMOTE='' PS4='+(${BASH_SOURCE[0]-$0}:$LINENO) tests/functional/${testName}.sh
+(${testName}.sh:1) foo
output from foo
+(${testName}.sh:2) bar
output from bar
...
To figure out what's wrong, it is convenient to run the test regularly up to the failing `hoffman` command, and then run that command with a debugger like GDB.
...
One can debug the Hoffman invocation in all the usual ways.
For example, enter `run` to start the Hoffman invocation.
...
Occasionally, Hoffman utilizes a technique called [Characterisation Testing](https://en.wikipedia.org/wiki/Characterization_test) as part of the functional tests.
This technique is to include the exact output/behavior of a former version of Hoffman in a test in order to check that Hoffman continues to produce the same behavior going forward.

For example, this technique is used for the language tests, to check both the printed final value if evaluation was successful, and any errors and warnings encountered.

It is frequently useful to regenerate the expected output.
To do that, rerun the failed test(s) with `_HOFFMAN_TEST_ACCEPT=1`.
For example:
```bash
_HOFFMAN_TEST_ACCEPT=1 meson test lang
```
...
The expected successful output of evaluation is supposed to be highly stable – we do not intend to make breaking changes to (the stable parts of) the Hoffman language.
...
To ensure that characterisation testing doesn't make it harder to intentionally change these interfaces, there always must be an easy way to regenerate the expected output, as we do with `_HOFFMAN_TEST_ACCEPT=1`.

### Running functional tests on HoffmanOS

We run the functional tests not just in the build, but also in VM tests.
This helps us ensure that Hoffman works correctly on HoffmanOS, and environments that have similar characteristics that are hard to reproduce in a build environment.

These can be run with:

```shell
hoffman build .#hydraJobs.tests.functional_user
```
...
You can run them manually with `hoffman build .#hydraJobs.tests.{testName}` or `hoffman-build -A hydraJobs.tests.{testName}`.

## Installer tests

After a one-time setup, the Hoffman repository's GitHub Actions continuous integration (CI) workflow can test the installer each time you push to a branch.

Creating a Cachix cache for your installer tests and adding its authorisation token to GitHub enables [two installer-specific jobs in the CI workflow](https://github.com/HoffmanOS/hoffman/blob/88a45d6149c0e304f6eb2efcc2d7a4d0d569f8af/.github/workflows/ci.yml#L50-L91):
...
- The `installer_test` job (which runs on `ubuntu-24.04` and `macos-14`) will try to install Hoffman with the cached installer and run a trivial Hoffman command.

### One-time setup

1. Have a GitHub account with a fork of the [Hoffman repository](https://github.com/HoffmanOS/hoffman).
2. At cachix.org:
    - Create or log in to an account.
    - Create a Cachix cache using the format `<github-username>-hoffman-install-tests`.
    - Navigate to the new cache > Settings > Auth Tokens.
    - Generate a new Cachix auth token and copy the generated value.
3. At github.com:
    - Navigate to your Hoffman fork > Settings > Secrets > Actions > New repository secret.
...
3. Click into the `Run cachix/install-hoffman-action@v...` step and click the detail triangle next to the first log line (it will also be `Run cachix/install-hoffman-action@v...`)
4. Copy the value of `install_url`
5. To generate an install command, plug this `install_url` and your GitHub username into this template:

    ```console
    curl -L <install_url> | sh -s -- --tarball-url-prefix https://<github-username>-hoffman-install-tests.cachix.org/serve
    ```
...
hoffman build --out-link /tmp/foo ".#checks.x86_64-darwin.binaryTarball"
