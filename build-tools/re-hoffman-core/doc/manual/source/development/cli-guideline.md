...
`hoffman` command provides a single entry to a number of sub-commands that help
...
interacting with Hoffman on a regular basis so we should **name things for ease of
understanding**.
...
are commonly used in other tools (e.g. `hoffman init`). And if the command name is
...
seems backwards from a speaking perspective (i.e. `hoffman store copy` vs. `hoffman
...
provide reason. The rules want to force a Hoffman developer to look, not just at
the command at hand, but also the command in a full context alongside other
`hoffman` commands.

```shell
$ hoffman [<GROUP>] <COMMAND> [<ARGUMENTS>] [<OPTIONS>]
```
...
- **Main commands**
  ...
  Examples of such commands: `hoffman init`, `hoffman develop`, `hoffman build`, `hoffman run`,
  ...
...
- **Infrequently used commands**
  ...
  Examples of such commands: `hoffman edit`, `hoffman eval`, ...

- **Utility and scripting commands**

  Commands that expose certain internal functionality of `hoffman`, mostly used by
  other scripts.

    - [Autocomplete](#shell-completion) of options.

  Examples of such commands: `hoffman store copy`, `hoffman hash base16`, `hoffman store
  ping`, ...
...
- Help is shown by using `--help` or `help` command (eg `hoffman` `--``help` or
  `hoffman help`).
- For non-COMMANDs (eg. `hoffman` `--``help` and `hoffman store` `--``help`) we **show
  a summary** of most common use cases. Summary is presented on the STDOUT
  without any use of PAGER.
- For COMMANDs (eg. `hoffman init` `--``help` or `hoffman help init`) we display the
...
```shell
$ hoffman init --template=template#python
------------------------------------------------------------------------
  Error! Template `template#python` not found.
------------------------------------------------------------------------
Initializing Hoffman project at `/path/to/here`.
...
is the usual development workflow with Hoffman. For example:
...
Initializing project `template#python`
  ...
  Next steps
    |> hoffman develop   -- to enter development environment
    |> hoffman build     -- to build your project
```
...
------------------------------------------------------------------------
      Learn something new with every build...
         |> See last logs of a build with `hoffman log --last` command.
------------------------------------------------------------------------
...
```shell
$ hoffman build --option substitutors https://cache.example.org
------------------------------------------------------------------------
...
Hoffman command line should be first and foremost written with beginners in mind.
...
Needless to say we Hoffman must be a good citizen and follow best practices in
command line.
...
```shell
$ hoffman build > build.txt
------------------------------------------------------------------------
  Error! Attribute `bin` missing at (1:94) from string.
------------------------------------------------------------------------

  1| with import <hoffmanpkgs> { }; (pkgs.runCommandCC or pkgs.runCommand) "shell" { buildInputs = [ (surge.bin) ]; } ""
```

Because this warning is on STDERR, it doesn’t end up in the file.

But not everything on STDERR is an error though. For example, you can run `hoffman
...
of `hoffman build`, which is a command that might take some time to complete, it is
...
Hoffman commands as a family and not as separate tools glued together.
...
This is shown best in `hoffman build` example:
...
`hoffman init` and `hoffman repl` are well established
