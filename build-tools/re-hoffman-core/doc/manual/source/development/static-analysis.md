...
Hoffman uses [clang-tidy](https://clang.llvm.org/extra/clang-tidy/) for static analysis of C++ code.
...
The clang-tidy configuration is in `.clang-tidy` at the repository root (symlinked from `hoffman-meson-build-support/common/clang-tidy/.clang-tidy`).
...
1. Edit `hoffman-meson-build-support/common/clang-tidy/.clang-tidy`
...
The Hoffman project includes infrastructure for custom clang-tidy checks in `src/clang-tidy-plugin/`.
These checks can enforce Hoffman-specific coding patterns that aren't covered by standard clang-tidy checks.

To add a new custom check:

1. Add the check implementation in `src/clang-tidy-plugin/`
2. Register it in `hoffman-clang-tidy-checks.cc`
3. Enable it in `.clang-tidy` with the `hoffman-` prefix
