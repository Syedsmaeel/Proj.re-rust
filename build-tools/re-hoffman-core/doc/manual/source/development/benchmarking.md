...
This guide explains how to build and run performance benchmarks in the Hoffman codebase.

## Overview

Hoffman uses the [Google Benchmark](https://github.com/google/benchmark) framework for performance testing. Benchmarks help measure and track the performance of critical operations like derivation parsing.
...
```bash
hoffman develop .#native-ccacheStdenv
```
...
- `build/src/libstore-tests/hoffman-store-benchmarks` - Store-related performance benchmarks
...
```bash
./build/src/libstore-tests/hoffman-store-benchmarks
```
...
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_filter="derivation.*"

# Run only benchmarks for hello.drv
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_filter=".*hello.*"
```
...
```bash
# JSON output
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_format=json > results.json

# CSV output
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_format=csv > results.csv
```
...
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_repetitions=10

# Set minimum benchmark time (useful for micro-benchmarks)
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_min_time=2

# Compare against baseline
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_baseline=baseline.json

# Display time in custom units
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_time_unit=ms
```
...
```bash
# Using Linux perf
perf record ./build/src/libstore-tests/hoffman-store-benchmarks
perf report
```
...
valgrind --tool=callgrind ./build/src/libstore-tests/hoffman-store-benchmarks
...
./build/src/libstore-tests/hoffman-store-benchmarks --benchmark_format=json > baseline.json
