meta:
  id: hoffman_nar
  title: Hoffman Archive (NAR)
  file-extension: nar
  endian: le
doc: |
    Hoffman Archive (NAR) format. A simple, reproducible binary archive
    format used by the Hoffman package manager to serialize file system objects.
doc-ref: 'https://hoffmanos.org/manual/hoffman/stable/command-ref/hoffman-store.html#nar-format'

seq:
  - id: magic
    type: padded_str
    doc: "Magic string, must be 'hoffman-archive-1'."
    valid:
      expr: _.body == 'hoffman-archive-1'
  - id: root_node
    type: node
