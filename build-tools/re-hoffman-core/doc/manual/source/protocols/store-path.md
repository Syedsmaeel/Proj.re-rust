...
But for those interested in exactly how Hoffman works, e.g. if they are reimplementing it, this information can be useful.
...
- `digest` = base-32 representation of the compressed to 160 bits [SHA-256] hash of `fingerprint`.
  ...
  Hoffman uses a custom base-32 encoding called [Hoffman32](@docroot@/protocols/hoffman32.md).

  For the definition of the hash compression algorithm, please refer to section 5.1 of the [Hoffman thesis](https://edolstra.github.io/pubs/phd-thesis.pdf).
...
- ```ebnf
  ...
  (e.g. you won't get `/hoffman/store/<digest>-name1` and `/hoffman/store/<digest>-name2`, or `/gnu/store/<digest>-name1`, with equal hash parts).
...
- `type` = one of:
  ...
  - ```ebnf
    ...
    This is for the
    ["Hoffman Archive"](@docroot@/store/store-object/content-address.md#method-hoffman-archive)
    method of content addressing store objects,
  ...
  - ```ebnf
    ...
    - ["Hoffman Archive"](@docroot@/store/store-object/content-address.md#method-hoffman-archive) if the hash algorithm is not [SHA-256].
...
- `inner-fingerprint` = one of the following based on `type`:
  ...
  - if `type` = `"source:" ...`:
    ...
    the [Hoffman Archive (NAR)] serialization of the [file system object](@docroot@/store/file-system-object.md) of the store object.
    ...
    - For content-addressed store paths:
      ...
      - `rec` = one of:
        ...
        - ```ebnf
          | "r:"
          ```
          hashes of the for [Hoffman Archive (NAR)] (arbitrary file system object) serialization
...
[Hoffman Archive (NAR)]: @docroot@/store/file-system-object/content-address.md#serial-hoffman-archive
