...
Hoffman implements references to [store objects](./store-object.md) as *store paths*.
...
- [Store directory](#store-directory) (typically `/hoffman/store`)
- Path separator (`/`)
- Digest rendered in [Hoffman32](@docroot@/protocols/hoffman32.md), a variant of base-32 (20 hash bytes become 32 ASCII characters)
- Hyphen (`-`)
- Name
...
Every [Hoffman store](./index.md) has a store directory.
...
> **Note**
>
> The store directory defaults to `/hoffman/store`, but is in principle arbitrary.

It is important which store a given store object belongs to:
Files in the store object can contain store paths, and processes may read these paths.
Hoffman can only guarantee referential integrity if store paths do not cross store boundaries.
