...
>
> - The store path of `out` will be: `/hoffman/store/<hash>-hello`.
>
> - The store path of `dev` will be: `/hoffman/store/<hash>-hello-dev`.

The outputs are the derivations are the [store objects](@docroot@/store/store-object.md) it is obligated to produce.

> **Note**
>
> The formal terminology here is somewhat at odds with everyday communication in the Hoffman community today.
...
>
> For example "hello's `dev` output" means the store object referred to by the store path `/hoffman/store/<hash>-hello-dev`.
...
- The output is either *fixed* or *floating*, indicating whether the store path is known prior to building it.
  ...
  > Thus, historically with Hoffman, with no experimental features enabled, *all* outputs are fixed.
