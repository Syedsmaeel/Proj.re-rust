...
- In addition, the following variables are set:
  ...
  - `HOFFMAN_BUILD_TOP` contains the path of the temporary directory for
    this build.
  ...
  - `HOFFMAN_STORE` is set to the path of the top-level Hoffman store
    directory (typically, `/hoffman/store`).

  - `HOFFMAN_ATTRS_JSON_FILE` & `HOFFMAN_ATTRS_SH_FILE` if `__structuredAttrs`
  ...
  - For each output declared in `outputs`, the corresponding
    ...
    Hoffman store for that output. Each output path is a concatenation
    of the cryptographic hash of all build inputs, the `name`
    attribute and the output name. (The output name is omitted if
    it’s `out`.)

- If an output path already exists, it is removed. Also, locks are
  acquired to prevent multiple [Hoffman instances][Hoffman instance] from performing the same
  build at the same time.

- A log of the combined standard output and error is written to
  `/hoffman/var/log/hoffman`.
...
- **Normalize the file permissions**
  ...
  Hoffman sets the last-modified timestamp on all files
  ...
  > **Note**
  >
  > Setuid and setgid programs are not currently supported by Hoffman.
  > This is because the Hoffman archives used in deployment have no concept of ownership information,
  > and because it makes the build result dependent on the user performing the build.

- **Calculate the references**

  Hoffman scans each output path for
  references to input paths by looking for the hash parts of the input
  paths. Since these are potential runtime dependencies, Hoffman registers
  them as dependencies of the output paths.

  Hoffman also scans for references to other outputs' paths in the same way, because outputs are allowed to refer to each other.
