...
- [`hoffman path-info --json`](@docroot@/command-ref/new-cli/hoffman3-path-info.md)
  ...
  - Old way:
    ...
    [
      {
        "path": "/hoffman/store/fvqsvk65d38p8qqir371ii0hyqxvjcw6-bash-5.2-p15",
        "valid": true,
        // ...
      },
      {
        "path": "/hoffman/store/wffw7l0alvs3iw94cbgi1gmmbmw99sqb-home-manager-path",
        "valid": false
      }
    ]
    ```
  ...
  - New way
    ...
    ```json5
    {
      "/hoffman/store/fvqsvk65d38p8qqir371ii0hyqxvjcw6-bash-5.2-p15": {
        // ...
      },
      "/hoffman/store/wffw7l0alvs3iw94cbgi1gmmbmw99sqb-home-manager-path": null,
    }
    ```

  This makes it match `hoffman derivation show`, which also maps store paths to information.

- When Hoffman is installed using the [binary installer](@docroot@/installation/installing-binary.md), in supported shells (Bash, Zsh, Fish)
  ...
  This means that command completion scripts, `.desktop` files, and similar artifacts installed via [`hoffman-env`](@docroot@/command-ref/hoffman-env.md) or [`hoffman profile`](@docroot@/command-ref/new-cli/hoffman3-profile.md)
...
- A new command `hoffman store add` has been added. It replaces `hoffman store add-file` and `hoffman store add-path` which are now deprecated.

- A new option [`always-allow-substitutes`](@docroot@/command-ref/conf-file.md#conf-always-allow-substitutes) has been added.

  When set to `true`, Hoffman will always try to substitute a derivation, even if it has the [`allowSubstitutes`]{#adv-attr-allowSubstitutes} attribute set to `false`.
