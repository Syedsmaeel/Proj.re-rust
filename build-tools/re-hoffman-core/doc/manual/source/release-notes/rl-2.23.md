...
- New builtin: `builtins.warn` [#306026](https://github.com/HoffmanOS/hoffman/issues/306026) [#10592](https://github.com/HoffmanOS/hoffman/pull/10592)
...
- Make `hoffman build --keep-going` consistent with `hoffman-build --keep-going`

  This means that if e.g. multiple fixed-output derivations fail to
  build, all hash mismatches are displayed.

- Modify `hoffman derivation {add,show}` JSON format [#9866](https://github.com/HoffmanOS/hoffman/issues/9866) [#10722](https://github.com/HoffmanOS/hoffman/pull/10722)
  ...
  This JSON format is only used by the experimental `hoffman derivation` family of commands, at this time.
  Future revisions are expected as the JSON format is still not entirely in compliance even after these changes.

- Warn on unknown settings anywhere in the command line [#10701](https://github.com/HoffmanOS/hoffman/pull/10701)

  All `hoffman` commands will now properly warn when an unknown option is specified anywhere in the command line.

  Before:

  ```console
  $ hoffman-instantiate --option foobar baz --expr '{}'
  warning: unknown setting 'foobar'
  $ hoffman-instantiate '{}' --option foobar baz --expr
  $ hoffman eval --expr '{}' --option foobar baz
  { }
  ```
...
- `hoffman env shell` is the new `hoffman shell`, and `hoffman shell` remains an accepted alias [#10504](https://github.com/HoffmanOS/hoffman/issues/10504) [#10807](https://github.com/HoffmanOS/hoffman/pull/10807)

  This is part of an effort to bring more structure to the CLI subcommands.

  `hoffman env` will be about the process environment.
  Future commands may include `hoffman env run` and `hoffman env print-env`.
...
- Store object info JSON format now uses `null` rather than omitting fields [#9995](https://github.com/HoffmanOS/hoffman/pull/9995)

  The [store object info JSON format](@docroot@/protocols/json/store-object-info.md), used for e.g. `hoffman path-info`, no longer omits fields to indicate absent information, but instead includes the fields with a `null` value.
...
- Large path warnings [#10661](https://github.com/HoffmanOS/hoffman/pull/10661)
