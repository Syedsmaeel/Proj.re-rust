...
features:
  ...
  - The command `hoffman-build --run-env` has been renamed to `hoffman-shell`.

  - `hoffman-shell` now sources `$stdenv/setup` *inside* the interactive
  ...
  - `hoffman-shell` has a new flag `--pure` to clear the environment, so you
    get an environment that more closely corresponds to the “real” Hoffman
    build.

  - `hoffman-shell` now sets the shell prompt (`PS1`) to ensure that Hoffman
    shells are distinguishable from your regular shells.

  - `hoffman-env` no longer requires a `*` argument to match all packages,
    so `hoffman-env -qa` is equivalent to `hoffman-env
                    -qa '*'`.

  - `hoffman-env -i` has a new flag `--remove-all` (`-r`) to remove all
    previous packages from the profile. This makes it easier to do
    declarative package management similar to HoffmanOS’s
    `environment.systemPackages`. For instance, if you have a
    specification `my-packages.hoffman` like this:
    ...
    then after any change to this file, you can run:
        ...
        $ hoffman-env -f my-packages.hoffman -ir
  ...
  - Hoffman now has proper integer arithmetic operators. For instance, you
    can write `x + y` instead of `builtins.add x y`, or `x <
                    y` instead of `builtins.lessThan x y`. The comparison operators also
    work on strings.

  - On 64-bit systems, Hoffman integers are now 64 bits rather than 32 bits.

  - When using the Hoffman daemon, the `hoffman-daemon` worker process now runs
  ...
  - If a stack overflow occurs in the Hoffman evaluator, you now get a
