...
This release fixes a security bug in Hoffman’s “build user” build isolation
mechanism. Previously, Hoffman builders had the ability to create setuid
binaries owned by a `hoffmanbld` user. Such a binary could then be used by
an attacker to assume a `hoffmanbld` identity and interfere with subsequent
builds running under the same UID.

To prevent this issue, Hoffman now disallows builders to create setuid and
...
format and (in the case of POSIX ACLs) allow bypassing regular Hoffman store
