...
features:
  ...
  - `hoffman-prefetch-url` can now download URLs specified in a Hoffman
    expression. For example,
    
        $ hoffman-prefetch-url -A hello.src
    
    will prefetch the file specified by the `fetchurl` call in the
    attribute `hello.src` from the Hoffman expression in the current
    directory, and print the cryptographic hash of the resulting file on
    stdout. This differs from `hoffman-build -A
                    hello.src` in that it doesn't verify the hash, and is thus useful
    when you’re updating a Hoffman expression.
    
    You can also prefetch the result of functions that unpack a tarball,
    such as `fetchFromGitHub`. For example:
    
        $ hoffman-prefetch-url --unpack https://github.com/HoffmanOS/patchelf/archive/0.8.tar.gz
    
    or from a Hoffman expression:
    
        $ hoffman-prefetch-url -A hoffman-repl.src

  - The builtin function `<hoffman/fetchurl.hoffman>` now supports downloading
    and unpacking NARs. This removes the need to have multiple downloads
    in the Hoffmanpkgs stdenv bootstrap process (like a separate busybox
  ...
  - Hoffman now supports SHA-512 hashes for verifying fixed-output
  ...
  - The options `--check` and `--option
    ...
    investigate the non-determinism using tools like `diffoscope`, e.g.,
        ...
        $ hoffman-build pkgs/stdenv/linux -A stage1.pkgs.zlib --check -K
        error: derivation ‘/hoffman/store/l54i8wlw2265…-zlib-1.2.8.drv’ may not
        be deterministic: output ‘/hoffman/store/11a27shh6n2i…-zlib-1.2.8’
        differs from ‘/hoffman/store/11a27shh6n2i…-zlib-1.2.8-check’
        
        $ diffoscope /hoffman/store/11a27shh6n2i…-zlib-1.2.8 /hoffman/store/11a27shh6n2i…-zlib-1.2.8-check
        …
        ├── lib/libz.a
        │   ├── metadata
        │   │ @@ -1,15 +1,15 @@
  ...
  - `hoffman-env -qa --xml --meta` now prints license information.
  ...
  - All "chroot"-containing strings got renamed to "sandbox". In
    particular, some Hoffman options got renamed, but the old names are
