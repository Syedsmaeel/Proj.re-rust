...
NOTE: this version of Hoffman uses Berkeley DB 4.3 instead of 4.2. The
database is upgraded automatically, but you should be careful not to use
old versions of Hoffman that still use Berkeley DB 4.2. In particular, if
you use a Hoffman installed through Hoffman, you should run
...
first.
  ...
  - Hoffman now uses Berkeley DB 4.3.
  ...
  - The `derivation` primitive is lazier. Attributes of dependent
    ...
    advantage is that it allows commands such as `hoffman-env -qa` and
    `hoffman-env -i` to be much faster since they no longer need to
  ...
  - `hoffman-build` now defaults to using `./default.hoffman` if no Hoffman
    expression is specified.

  - `hoffman-instantiate`, when applied to a Hoffman expression that evaluates
    to a function, will call the function automatically if all its
    arguments have defaults.

  - Hoffman now uses libtool to build dynamic libraries. This reduces the
    size of executables.
  ...
  - Some currently undocumented primops to support low-level build
    management using Hoffman (i.e., using Hoffman as a Make replacement). See
