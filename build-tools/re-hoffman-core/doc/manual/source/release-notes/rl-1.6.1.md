...
This is primarily a bug fix release. Changes of interest are:
  ...
  - Hoffman 1.6 accidentally changed the semantics of antiquoted paths in
    strings, such as `"${/foo}/bar"`. This release reverts to the Hoffman
    1.5.3 behaviour.

  - Previously, Hoffman optimised expressions such as `"${expr}"` to *expr*.
    ...
    "` evaluates to `"/hoffman/store/hash-foo "`). Hoffman now checks the type
    of antiquoted expressions and applies coercions.

  - Hoffman now shows the exact position of undefined variables. In
  ...
  - Bash completion in `hoffman-shell` now works correctly.
