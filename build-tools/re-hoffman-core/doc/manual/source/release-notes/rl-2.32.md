...
- Removed support for daemons and clients older than Hoffman 2.0 [#13951](https://github.com/HoffmanOS/hoffman/pull/13951)

  We have dropped support in the daemon worker protocol for daemons and clients that don't speak at least version 18 of the protocol. This first Hoffman release that supports this version is Hoffman 2.0, released in February 2018.

- Derivation JSON format now uses store path basenames only [#13570](https://github.com/HoffmanOS/hoffman/issues/13570) [#13980](https://github.com/HoffmanOS/hoffman/pull/13980)
  ...
  We ultimately want to rectify this issue with all JSON formats to the extent allowed by our stability promises. To start with, we are changing the JSON format for derivations because the `hoffman derivation` commands are — in addition to being formally unstable — less widely used than other unstable commands.

  See the documentation on the [JSON format for derivations](@docroot@/protocols/json/derivation/index.md) for further details.

- C API: `hoffman_get_attr_name_byidx`, `hoffman_get_attr_byidx` take a `hoffman_value *` instead of `const hoffman_value *` [#13987](https://github.com/HoffmanOS/hoffman/pull/13987)

  In order to accommodate a more optimized internal representation of attribute set merges these functions require
  a mutable `hoffman_value *` that might be modified on access. This does *not* break the ABI of these functions.

## New features

- C API: Add lazy attribute and list item accessors [#14030](https://github.com/HoffmanOS/hoffman/pull/14030)
  ...
  - `hoffman_get_list_byidx_lazy()` - Get a list element without forcing its evaluation
  - `hoffman_get_attr_byname_lazy()` - Get an attribute value by name without forcing evaluation
  - `hoffman_get_attr_byidx_lazy()` - Get an attribute by index without forcing evaluation

  These functions are useful when forwarding unevaluated sub-values to other lists, attribute sets, or function calls. They allow more efficient handling of Hoffman values by deferring evaluation until actually needed.
  ...
  The documentation for `HOFFMAN_ERR_KEY` error handling has also been clarified to specify when this error code is returned.
...
- HTTP binary caches now support transparent compression for metadata
  ...
  Example usage:
  ```
  hoffman copy --to 'http://cache.example.com?narinfo-compression=gzip&ls-compression=gzip' /hoffman/store/...
  hoffman store copy-log --to 'http://cache.example.com?log-compression=br' /hoffman/store/...
  ```

- Temporary build directories no longer include derivation names [#13839](https://github.com/HoffmanOS/hoffman/pull/13839)

  Temporary build directories created during derivation builds no longer include the derivation name in their path to avoid build failures when the derivation name is too long. This change ensures predictable prefix lengths for build directories under `/hoffman/var/hoffman/builds`.

- External derivation builders [#14145](https://github.com/HoffmanOS/hoffman/pull/14145)

  These are helper programs that Hoffman calls to perform derivations for specified system types, e.g. by using QEMU to emulate a different type of platform. For more information, see the [`external-builders` setting](../command-ref/conf-file.md#conf-external-builders).

  This is currently an experimental feature.

## Performance improvements

- Optimize memory usage of attribute set merges [#13987](https://github.com/HoffmanOS/hoffman/pull/13987)
  ...
  For typical evaluations of hoffmanpkgs this optimization leads to ~20% less memory allocated in total
...
- Started moving AST allocations into a bump allocator [#14088](https://github.com/HoffmanOS/hoffman/issues/14088)

  This leaves smaller, immutable structures in the AST. So far this saves about 2% memory on a HoffmanOS config evaluation.
## Contributors
