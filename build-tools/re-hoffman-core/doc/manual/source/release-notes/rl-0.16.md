...
This release has the following improvements:
  ...
  - The Hoffman expression evaluator is now much faster in most cases:
    typically, [3 to 8 times compared to the old
    implementation](http://www.mail-archive.com/hoffman-dev@cs.uu.nl/msg04113.html).
  ...
  - Support for configurable parallelism inside builders. Build scripts
    ...
    performed in parallel was not configurable. Hoffman now has an option
    `--cores
                            N` as well as a configuration setting `build-cores =
                            N` that causes the environment variable `HOFFMAN_BUILD_CORES` to be set
    ...
    discretion to perform a parallel build, e.g., by calling `make -j
                            N`. In Hoffmanpkgs, this can be enabled on a per-package basis by
    setting the derivation attribute `enableParallelBuilding` to `true`.

  - `hoffman-store -q` now supports XML output through the `--xml` flag.
