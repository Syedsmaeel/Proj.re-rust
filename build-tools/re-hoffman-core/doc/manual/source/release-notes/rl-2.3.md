...
changes:
  ...
  - Hoffman now uses BSD file locks instead of POSIX file locks. Because of
    this, you should not use Hoffman 2.3 and previous releases at the same
    time on a Hoffman store.

It also has the following changes:

  - `builtins.fetchGit`'s `ref` argument now allows specifying an
    absolute remote ref. Hoffman will automatically prefix `ref` with
  ...
  - The `hoffman` command has a new `--print-build-logs` (`-L`) flag to
  ...
  - Add a `trace-function-calls` setting to log the duration of Hoffman
