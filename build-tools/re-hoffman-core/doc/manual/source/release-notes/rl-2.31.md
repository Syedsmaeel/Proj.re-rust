...
- `build-cores = 0` now auto-detects CPU cores [#13402](https://github.com/HoffmanOS/hoffman/pull/13402)

  When `build-cores` is set to `0`, Hoffman now automatically detects the number of available CPU cores and passes this value via `HOFFMAN_BUILD_CORES`, instead of passing `0` directly. This matches the behavior when `build-cores` is unset. This prevents the builder from having to detect the number of cores.

- Fix Git LFS SSH issues [#13337](https://github.com/HoffmanOS/hoffman/issues/13337) [#13743](https://github.com/HoffmanOS/hoffman/pull/13743)

  Fixed some outstanding issues with Git LFS and SSH.

  * Added support for `HOFFMAN_SSHOPTS`.
...
- Add support for `user@address:port` syntax in store URIs [#7044](https://github.com/HoffmanOS/hoffman/issues/7044) [#3425](https://github.com/HoffmanOS/hoffman/pull/3425)

  It's now possible to specify the port used for SSH stores directly in the store URL in accordance with [RFC3986](https://datatracker.ietf.org/doc/html/rfc3986). Previously the only way to specify custom ports was via `ssh_config` or the `HOFFMAN_SSHOPTS` environment variable, because Hoffman incorrectly passed the port number together with the host name to the SSH executable.
...
- Enable parallel marking in bdwgc [#13708](https://github.com/HoffmanOS/hoffman/pull/13708)
...
- Add `warn-short-path-literals` setting [#13489](https://github.com/HoffmanOS/hoffman/pull/13489)
  ...
  This setting, when enabled, causes Hoffman to emit warnings when encountering relative path literals that don't start with `.` or `/`, for instance suggesting that `foo/bar` should be rewritten to `./foo/bar`.
