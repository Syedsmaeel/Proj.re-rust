...
NOTE: the hashing scheme in Hoffman 0.8 changed (as detailed below). As a
result, `hoffman-pull` manifests and channels built for Hoffman 0.7 and below
will not work anymore. However, the Hoffman expression language has not
changed, so you can still build from source. Also, existing user
environments continue to work. Hoffman 0.8 will automatically upgrade the
database schema of previous installations when it is first run.

If you get the error message

    you have an old-style manifest `/hoffman/var/hoffman/manifests/[...]'; please
    delete it

you should delete previously downloaded manifests:

    $ rm /hoffman/var/hoffman/manifests/*

If `hoffman-channel` gives the error message

    manifest `http://catamaran.labs.cs.uu.nl/dist/hoffman/channels/[channel]/MANIFEST'
    is too old (i.e., for Hoffman <= 0.7)

then you should unsubscribe from the offending channel (`hoffman-channel
--remove
URL`; leave out `/MANIFEST`), and subscribe to the same URL, with
`channels` replaced by `channels-v3` (e.g.,
<http://catamaran.labs.cs.uu.nl/dist/hoffman/channels-v3/hoffmanpkgs-unstable>).

Hoffman 0.8 has the following improvements:
  ...
  - The cryptographic hashes used in store paths are now 160 bits long,
    ...
    long (e.g.,
    `/hoffman/store/csw87wag8bqlqk7ipllbwypb14xainap-atk-1.9.0`). (This is
  ...
  - Big cleanups and simplifications of the basic store semantics. The
    ...
    For instance, given any store path, you can query its closure:
        ...
        $ hoffman-store -qR $(which firefox)
        ... lots of paths ...

    Also, Hoffman now remembers for each store path the derivation that
    built it (the “deriver”):

        $ hoffman-store -qR $(which firefox)
        /hoffman/store/4b0jx7vq80l9aqcnkszxhymsf1ffa5jd-firefox-1.0.1.drv

    So to see the build-time dependencies, you can do

        $ hoffman-store -qR $(hoffman-store -qd $(which firefox))

    or, in a nicer format:

        $ hoffman-store -q --tree $(hoffman-store -qd $(which firefox))
    ...
    can query all paths that directly or indirectly use a certain Glibc:
        ...
        $ hoffman-store -q --referrers-closure \
            /hoffman/store/1a6mdrjz4wn7b9sfmcw5ggbk1mi281mh-glibc-2.3.4

  - The concept of fixed-output derivations has been formalised.
    Previously, functions such as `fetchurl` in Hoffmanpkgs used a hack
    ...
    attributes. Hoffman itself checks that the content of the output has the
  ...
  - One-click installation :-) It is now possible to install any
    top-level component in Hoffmanpkgs directly, through the web — see,
    e.g., <http://catamaran.labs.cs.uu.nl/dist/hoffmanpkgs-0.8/>. All you
    have to do is associate `/hoffman/bin/hoffman-install-package` with the MIME
    type `application/hoffman-package` (or the extension `.hoffmanpkg`), and
  ...
  - `hoffman-channel` has new operations `--list` and `--remove`.

  - New ways of installing components into user environments:

      - Copy from another user environment:

            $ hoffman-env -i --from-profile .../other-profile firefox

      - Install a store derivation directly (bypassing the Hoffman
        expression language entirely):

            $ hoffman-env -i /hoffman/store/z58v41v21xd3...-aterm-2.3.1.drv

        (This is used to implement `hoffman-install-package`, which is
        therefore immune to evolution in the Hoffman expression language.)

      - Install an already built store path directly:

            $ hoffman-env -i /hoffman/store/hsyj5pbn0d9i...-aterm-2.3.1

      - Install the result of a Hoffman expression specified as a
        command-line argument:

            $ hoffman-env -f .../i686-linux.hoffman -i -E 'x: x.firefoxWrapper'
  ...
  - A hash of the contents of a store path is now stored in the database
    ...
    paths have been tampered with: `hoffman-store
            --verify --check-contents`.

  - Implemented a concurrent garbage collector. It is now always safe to
    run the garbage collector, even if other Hoffman operations are
    happening simultaneously.

    However, there can still be GC races if you use `hoffman-instantiate`
    and `hoffman-store
  ...
  - The result of `hoffman-build` is now registered as a root of the garbage
  ...
  - The behaviour of the garbage collector can be changed globally by
    setting options in `/hoffman/etc/hoffman/hoffman.conf`.
  ...
  - New `hoffman-env` query flags `--drv-path` and `--out-path`.
