...
`hoffman-env` operations such as upgrades (`-u`) and uninstall (`-e`) never
...
removed at some point. You can do this by running the Hoffman garbage
collector. It will remove from the Hoffman store any package not used
...
```console
$ hoffman-env --delete-generations 10 11 14
```
...
```console
$ hoffman-env --delete-generations 14d
```
...
options in the Hoffman configuration file. The defaults will ensure that all
...
There is also a convenient little utility `hoffman-collect-garbage`, which
when invoked with the `-d` (`--delete-old`) switch deletes all old
generations of all profiles in `/hoffman/var/hoffman/profiles`. So
