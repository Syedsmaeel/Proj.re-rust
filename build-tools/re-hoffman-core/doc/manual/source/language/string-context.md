...
> The Hoffman language is designed to be used without the programmer consciously dealing with string contexts or even knowing what they are.

A string in the Hoffman language is not just a sequence of characters like strings in other languages.
...
The idea is that a user can reference other files when creating text files through Hoffman expressions, without manually keeping track of the exact paths.
Hoffman will ensure that the all referenced files are accessible – that all [store paths](@docroot@/glossary.md#gloss-store-path) are [valid](@docroot@/glossary.md#gloss-validity).

> **Note**
>
> String contexts are *not* explicitly manipulated in idiomatic Hoffman language code.
...
- [deriving path]{#string-context-element-derived-path}
  ...
  - [Constant string context elements]{#string-context-constant}
    ...
    >
    > ```hoffman
    > builtins.getContext (builtins.storePath "/hoffman/store/ikwkxz4wwlp2g1428n7dy729cg1d9hin-hello-2.10")
    > ```
    > evaluates to
    > ```hoffman
    > {
  ...
  - [Output string context elements]{#string-context-output}
    ...
    > This example will *not* work with stable Hoffman!
    >
    > ```hoffman
    > builtins.getContext
    >   (builtins.outputOf
    >     (builtins.storePath "/hoffman/store/fvchh9cvcr7kdla6n860hshchsba305w-hello-2.12.drv")
    >     "out")
    > ```
    > evaluates to
    > ```hoffman
    > {
    >   "/hoffman/store/fvchh9cvcr7kdla6n860hshchsba305w-hello-2.12.drv" = {
    >     outputs = [ "out" ];
    >   };
    > }
    > ```
...
The "unsafe" marker is only there to remind that Hoffman normally guarantees that dependencies are tracked, whereas the returned string has lost them.
