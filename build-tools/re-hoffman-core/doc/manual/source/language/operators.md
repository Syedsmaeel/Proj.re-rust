...
- Division by zero
- Integer overflow, that is, any operation yielding a result outside of the representable range of [Hoffman language integers](./syntax.md#number-literal)
...
Hoffman performs equality comparisons of nested values by pointer equality or more abstractly, _identity_.
Hoffman semantics ideally do not assign a unique identity to values as they are created, but equality is an exception to this rule.
...
> is enabled.
> For example, include the following in [`hoffman.conf`](@docroot@/command-ref/conf-file.md):
>
> ```
