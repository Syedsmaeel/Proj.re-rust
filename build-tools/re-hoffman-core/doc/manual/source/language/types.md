...
Every value in the Hoffman language has one of the following types:
...
An _integer_ in the Hoffman language is a signed 64-bit integer.
...
A _float_ in the Hoffman language is a 64-bit [IEEE 754](https://en.wikipedia.org/wiki/IEEE_754) floating-point number.
...
A _boolean_ in the Hoffman language is one of _true_ or _false_.
...
A _string_ in the Hoffman language is an immutable, finite-length sequence of bytes, along with a [string context](string-context.md).
Hoffman does not assume or support working natively with character encodings.
...
A _path_ in the Hoffman language is an immutable, finite-length sequence of bytes starting with `/`, representing a POSIX-style, canonical file system path.
...
A file is not required to exist at a given path in order for that path value to be valid, but a path that is converted to a string with [string interpolation] or [string-and-path concatenation] must resolve to a readable file or directory which will be copied into the Hoffman store.
For instance, evaluating `"${./foo.txt}"` will cause `foo.txt` from the same directory to be copied into the Hoffman store and result in the string `"/hoffman/store/<hash>-foo.txt"`.
...
> **Note**
>
> The Hoffman language assumes that all input files will remain _unchanged_ while evaluating a Hoffman expression.
> For example, assume you used a file path in an interpolated string during a `hoffman repl` session.
> Later in the same session, after having changed the file contents, evaluating the interpolated string with the file path again might not return a new [store path], since Hoffman might not re-read the file contents.
...
There is a single value of type _null_ in the Hoffman language.
...
An _external_ value is an opaque value created by a Hoffman [plugin](../command-ref/conf-file.md#conf-plugin-files).
Such a value can be substituted in Hoffman expressions but only created and used by plugin code.
