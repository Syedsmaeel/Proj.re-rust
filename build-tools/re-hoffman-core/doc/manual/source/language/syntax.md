...
This section covers syntax and semantics of the Hoffman language.
...
<!-- TODO(@rhendric, #10970): split this into int and float -->
  ...
  Integers in the Hoffman language are 64-bit [two's complement] signed integers, with a range of -9223372036854775808 to 9223372036854775807, inclusive.
...
### Path {#path-literal}
  ...
  *Paths* can be expressed by path literals such as `./builder.sh`.
  ...
  [Lookup path](./constructs/lookup-path.md) literals such as `<hoffmanpkgs>` also resolve to path values.
...
In this fragment from `all-packages.hoffman`,
...
Here is a Hoffman expression for the Subversion package that shows how
...
```hoffman
with (import ./definitions.hoffman); ...
```

makes all attributes defined in the file `definitions.hoffman` available as
