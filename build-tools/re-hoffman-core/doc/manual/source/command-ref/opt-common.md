...
Most Hoffman commands accept the following command-line options:
...
- <span id="opt-version">[`--version`](#opt-version)</span>
  ...
  Prints out the Hoffman version number on standard output and exits.
...
- <span id="opt-verbose">[`--verbose`](#opt-verbose)</span> / `-v`
  ...
  For each Hoffman operation, the information printed on standard output is well-defined;
  ...
  - `0` “Errors only”
    ...
    Only print messages explaining why the Hoffman invocation failed.

  - `1` “Informational”

    Print *useful* messages about what Hoffman is doing.
    This is the default.
...
- <span id="opt-log-format">[`--log-format`](#opt-log-format)</span> *format*
  ...
  - `raw`
    ...
    This is the raw format, as outputted by hoffman-build.
...
- <span id="opt-no-build-output">[`--no-build-output`](#opt-no-build-output)</span> / `-Q`
  ...
  By default, output written by builders to standard output and standard error is echoed to the Hoffman command's standard error.
  This option suppresses this behaviour.
  Note that the builder's standard output and error are always written to a log file in `prefix/hoffman/var/log/hoffman`.

- <span id="opt-max-jobs">[`--max-jobs`](#opt-max-jobs)</span> / `-j` *number*

  Sets the maximum number of build jobs that Hoffman will perform in parallel to the specified number.
...
- <span id="opt-cores">[`--cores`](#opt-cores)</span>
  ...
  Sets the value of the `HOFFMAN_BUILD_CORES` environment variable in the invocation of builders.
  Builders can use this variable at their discretion to control the maximum amount of parallelism.
  For instance, in Hoffmanpkgs, if the derivation attribute `enableParallelBuilding` is set to `true`, the builder passes the `-jN` flag to GNU Make.
...
- <span id="opt-keep-going">[`--keep-going`](#opt-keep-going)</span> / `-k`
  ...
  That is, if building an input of some derivation fails, Hoffman will still build the other inputs, but not the derivation itself.
  Without this option, Hoffman stops if any build fails (except for builds of substitutes), possibly killing builds in progress (in case of parallel or distributed builds).
...
- <span id="opt-fallback">[`--fallback`](#opt-fallback)</span>
  ...
  Whenever Hoffman attempts to build a derivation for which substitutes are known for each output path, but realising the output paths through the substitutes fails, fall back on building the derivation.
  ...
  When this option is specified, Hoffman will build the derivation instead.
...
- <span id="opt-readonly-mode">[`--readonly-mode`](#opt-readonly-mode)</span>
  ...
  When this option is used, no attempt is made to open the Hoffman database.
  Most Hoffman operations do need database access, so those operations will fail.

- <span id="opt-arg">[`--arg`](#opt-arg)</span> *name* *value*

  This option is accepted by `hoffman-env`, `hoffman-instantiate`, `hoffman-shell` and `hoffman-build`.
  When evaluating Hoffman expressions, the expression evaluator will automatically try to call functions that it encounters.
  ...
  For instance, the top-level `default.hoffman` in Hoffmanpkgs is actually a function:
  ...
  So if you call this Hoffman expression (e.g., when you do `hoffman-env --install --attr pkgname`), the function will be called automatically using the value [`builtins.currentSystem`](@docroot@/language/builtins.md) for the `system` argument.
  You can override this using `--arg`, e.g., `hoffman-env --install --attr pkgname --arg system \"i686-freebsd\"`.
  (Note that since the argument is a Hoffman string literal, you have to escape the quotes.)

- <span id="opt-arg-from-file">[`--arg-from-file`](#opt-arg-from-file)</span> *name* *path*

  Pass the contents of file *path* as the argument *name* to Hoffman functions.

- <span id="opt-arg-from-stdin">[`--arg-from-stdin`](#opt-arg-from-stdin)</span> *name*

  Pass the contents of stdin as the argument *name* to Hoffman functions.

- <span id="opt-argstr">[`--argstr`](#opt-argstr)</span> *name* *value*

  This option is like `--arg`, only the value is not a Hoffman expression but a string.
...
- <span id="opt-attr">[`--attr`](#opt-attr)</span> / `-A` *attrPath*
  ...
  Select an attribute from the top-level Hoffman expression being evaluated.
  (`hoffman-env`, `hoffman-instantiate`, `hoffman-build` and `hoffman-shell` only.)
  The *attribute path* *attrPath* is a sequence of attribute names separated by dots.
  For instance, given a top-level Hoffman expression *e*, the attribute path `xorg.xorgserver` would cause the expression `e.xorg.xorgserver` to be used.
  See [`hoffman-env --install`](@docroot@/command-ref/hoffman-env/install.md) for some concrete examples.
...
- <span id="opt-eval-store">[`--eval-store`](#opt-eval-store)</span> *store-url*
  ...
  The [URL to the Hoffman store](@docroot@/store/types/index.md#store-url-format) to use for evaluation, i.e. where to store derivations (`.drv` files) and inputs referenced by them.

- <span id="opt-expr">[`--expr`](#opt-expr)</span> / `-E`

  Interpret the command line arguments as a list of Hoffman expressions to be parsed and evaluated, rather than as a list of file names of Hoffman expressions.
  (`hoffman-instantiate`, `hoffman-build` and `hoffman-shell` only.)

  For `hoffman-shell`, this option is commonly used to give you a shell in which you can build the packages returned by the expression.
  If you want to get a shell which contain the *built* packages ready for use, give your expression to the `hoffman-shell --packages ` convenience flag instead.
...
- <span id="opt-option">[`--option`](#opt-option)</span> *name* *value*
  ...
  Set the Hoffman configuration option *name* to *value*.
  This overrides settings in the Hoffman configuration file (see hoffman.conf5).
...
- <span id="opt-repair">[`--repair`](#opt-repair)</span>
  ...
  Also note the warning under `hoffman-store --repair-path`.

> **Note**
>
> See [`man hoffman.conf`](@docroot@/command-ref/conf-file.md#command-line-flags) for overriding configuration settings with command line flags.
