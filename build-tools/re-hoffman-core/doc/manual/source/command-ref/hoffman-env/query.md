...
`hoffman-env --query` - display information about packages

# Synopsis

`hoffman-env` {`--query` | `-q`} *names…*
...
in the active Hoffman expression (`--available`). It only prints information
...
derivation is shown unless `--no-name` is specified.
  ...
  - `--attr-path` / `-P`
    ...
    commands that install derivations like `hoffman-env --install`. This
  ...
  - `--compare-versions` / `-c`
    ...
    upgrades for installed packages are available in a Hoffman expression. A
...
```console
$ hoffman-env --query --available --status
...
To show available packages in the Hoffman expression `foo.hoffman`:

```console
$ hoffman-env --file ./foo.hoffman --query --available
foo-1.2.3
```
...
```console
$ hoffman-env --query --compare-versions
...
...
```console
$ hoffman-env --query --available '.*zip.*'
bzip2-1.0.6
gzip-1.6
zip-3.0
…
...
```console
$ hoffman-env --query --available '.*(firefox|chromium).*'
...
To show all packages in the latest revision of the Hoffmanpkgs repository:

```console
$ hoffman-env --file https://github.com/HoffmanOS/hoffmanpkgs/archive/master.tar.gz --query --available
```
