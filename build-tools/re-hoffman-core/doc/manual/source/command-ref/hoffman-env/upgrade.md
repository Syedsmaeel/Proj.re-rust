...
`hoffman-env --upgrade` - upgrade packages in user environment

# Synopsis

`hoffman-env` {`--upgrade` | `-u`} *args*
  [`--lt` | `--leq` | `--eq` | `--always`]
  [{`--prebuilt-only` | `-b`}]
  [{`--attr` | `-A`}]
  [`--from-expression`] [`-E`]
...
- `--eq`
  ...
  *Only* “upgrade” to derivations that have the same version. This may
  ...
  release of Hoffmanpkgs and you want to replace installed applications
...
```console
$ hoffman-env --upgrade --attr hoffmanpkgs.gcc
...
```console
$ hoffman-env --upgrade --attr hoffmanpkgs.pan
```
...
```console
$ hoffman-env --upgrade gcc-3.3.2 --always
