...
`hoffman-store --read-log` - print build log

# Synopsis

`hoffman-store` {`--read-log` | `-l`} *paths…*
...
Build logs are kept in `/hoffman/var/log/hoffman/drvs`. However, there is no
...
```console
$ hoffman-store --read-log $(which ktorrent)
building /hoffman/store/dhc73pvzpnzxhdgpimsd9sw39di66ph1-ktorrent-2.2.1
unpacking sources
unpacking source archive /hoffman/store/p8n1jpqs27mgkjw07pb5269717nzf5f8-ktorrent-2.2.1.tar.gz
ktorrent-2.2.1/
ktorrent-2.2.1/NEWS
...
```
