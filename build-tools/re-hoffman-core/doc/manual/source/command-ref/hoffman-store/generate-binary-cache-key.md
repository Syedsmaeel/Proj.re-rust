...
`hoffman-store --generate-binary-cache-key` - generate key pair to use for a binary cache

## Synopsis

`hoffman-store` `--generate-binary-cache-key` *key-name* *secret-key-file* *public-key-file*
