...
`hoffman-store --add-fixed` - add paths to store using given hashing algorithm

## Synopsis

`hoffman-store` `--add-fixed` [`--recursive`] *algorithm* *paths…*

## Description

The operation `--add-fixed` adds the specified paths to the Hoffman store.
...
```console
$ hoffman-store --add-fixed sha256 ./hello-2.10.tar.gz
