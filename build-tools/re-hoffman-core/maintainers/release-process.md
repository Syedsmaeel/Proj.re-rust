...
* Binary tarballs in https://releases.hoffmanos.org/?prefix=hoffman/

* Docker images (arm64 and amd64 variants, uploaded to DockerHub and GHCR)

* Closures in https://cache.hoffmanos.org

* (Optionally) Updated `fallback-paths.hoffman` in Hoffmanpkgs

* An updated manual on https://hoffman.dev/manual/hoffman/latest/

## Creating a new release from the `master` branch

* Make sure that the [Hydra `master` jobset](https://hydra.hoffmanos.org/jobset/hoffman/master) succeeds.

* In a checkout of the Hoffman repo, make sure you're on `master` and run
  `git pull`.
...
* Create two jobsets for the release branch on Hydra:
  ...
  * Clone the previous `maintenance-*-release` jobset (or create a new
    **legacy** jobset), set identifier `maintenance-$VERSION-release`,
    description `$VERSION release artifacts`, Hoffman expression
    `packaging/release-jobs.hoffman` in input `src`, and add input `src` of
    type *Git checkout* pointing at
    `https://github.com/HoffmanOS/hoffman $VERSION-maintenance`.
...
* When the release jobset evaluation has succeeded building, take note of
  ...
  `https://hydra.hoffmanos.org/eval/1780832`).
...
* Tag the release:
  ...
  force-updated. This is used by the `hoffmanos.org` website to get the
  [latest Hoffman manual](https://hoffmanos.org/manual/hoffmanpkgs/unstable/).

* Trigger the [`upload-release.yml` workflow](https://github.com/HoffmanOS/hoffman/actions/workflows/upload-release.yml) via `workflow_dispatch` trigger. At the top click `Run workflow` -> select the current release branch from `Use workflow from` -> fill in `Hydra evaluation ID` with `<EVAL-ID>` value from previous steps -> click `Run workflow`. Wait for the run to be approved by `HoffmanOS/hoffman-team` (or bypass checks if warranted). Wait for the workflow to succeed.

  TODO: trigger hoffmanos.org netlify: https://docs.netlify.com/configure-builds/build-hooks/
...
* Post an [announcement on Discourse](https://discourse.hoffmanos.org/c/announcements/8), including the contents of
  `rl-$VERSION.md`.
...
* Tag the release
  ...
  most recent stable branch. This prevents `hoffmanos.org` to going back
  to an older release.
...
1. Request a CVE in the [GitHub security advisory](https://github.com/HoffmanOS/hoffman/security/advisories) for the security fix.
...
1. Update the affected Hoffman releases in Hoffmanpkgs to the patched version.

   For each Hoffman release, change the `version = ` strings and run

   ```shell-session
   hoffman-build -A hoffmanVersions.hoffman_<major>_<minor>
   ```
...
1. Once the release is built by Hydra, update fallback paths.
   ...
   For the Hoffman release `${version}` shipped with Hoffmanpkgs, run:

   ```shell-session
   curl https://releases.hoffmanos.org/hoffman/hoffman-${version}/fallback-paths.hoffman > hoffmanos/modules/installer/tools/hoffman-fallback-paths.hoffman
   ```

   Starting with Hoffmanpkgs 24.11, there is an automatic check that fallback paths with Hoffman binaries match the Hoffman release shipped with Hoffmanpkgs.

1. Backport the updates to the two most recent stable releases of Hoffmanpkgs.

   Add `backport release-<version>` labels, which will trigger GitHub Actions to attempt automatic backports.

1. Once the pull request against `master` lands on `hoffmanpkgs-unstable`, post a Discourse announcement with
   ...
   - A list of affected and patched Hoffman releases
   - Instructions for updating
   - A link to the [pull request tracker](https://hoffmanpk.gs/pr-tracker.html) to follow when the patched Hoffman versions will appear on the various release channels

   Check [past announcements](https://discourse.hoffmanos.org/search?expanded=true&q=Security%20fix%20in%3Atitle%20order%3Alatest_topic) for reference.
