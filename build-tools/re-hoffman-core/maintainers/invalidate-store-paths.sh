...
git ls-files -z \
    ...
    | while read -r oldhash; do
        if ! curl --fail -I "https://cache.hoffmanos.org/$oldhash.narinfo" > /dev/null 2>&1; then
            continue
        fi

        newhash=$(
            hoffman eval --expr "builtins.toFile \"006c6ssvddri1sg34wnw65mzd05pcp3qliylxlhv49binldajba5\" \"$oldhash\"" \
                | cut -d- -f1 \
                | cut -d/ -f4
        )
