...
#! hoffman-shell -i perl -p awscli2 perl perlPackages.LWPUserAgent perlPackages.LWPProtocolHttps perlPackages.FileSlurp perlPackages.NetAmazonS3 perlPackages.GetoptLongDescriptive gnupg1
...
my ($opt, $usage) = describe_options(
    ...
    [ 'docker-owner=s',   'Docker image owner', { default => 'hoffmanos/hoffman' } ],
...
my $releasesBucketName = "hoffman-releases";
my $channelsBucketName = "hoffman-channels";

my $TMPDIR = $ENV{'TMPDIR'} // "/tmp";

my $isLatest = ($ENV{'IS_LATEST'} // "") eq "1";

# FIXME: cut&paste from hoffmanos-channel-scripts.
sub fetch {
    my ($url, $type) = @_;
...
my $buildInfo = decode_json(fetch("$evalUrl/job/build.hoffman-everything.x86_64-linux", 'application/json'));
#print Dumper($buildInfo);

my $releaseName = $buildInfo->{hoffmanname};
...
my $tmpDir = "$TMPDIR/hoffman-release/$releaseName";
File::Path::make_path($tmpDir);

my $narCache = "$TMPDIR/nar-cache";
File::Path::make_path($narCache);

my $binaryCache = "https://cache.hoffmanos.org/?local-nar-cache=$narCache";
...
sub copyManual {
    ...
    unless (-e $manualNar) {
        system("HOFFMAN_REMOTE=$binaryCache hoffman store dump-path '$manual' | xz > '$manualNar'.tmp") == 0
            or die "unable to fetch $manual\n";
        rename("$manualNar.tmp", $manualNar) or die;
    }

    unless (-e "$tmpDir/manual") {
        system("xz -d < '$manualNar' | hoffman-store --restore $tmpDir/manual.tmp") == 0
            or die "unable to unpack $manualNar\n";
        rename("$tmpDir/manual.tmp/share/doc/hoffman/manual", "$tmpDir/manual") or die;
...
sub downloadFile {
    ...
    if (!-e $tmpFile) {
        ...
        my $fileInfo = decode_json(`HOFFMAN_REMOTE=$binaryCache hoffman store ls --json '$srcFile'`);

        $srcFile = $fileInfo->{target} if $fileInfo->{type} eq 'symlink';

        #print STDERR $srcFile, " ", Dumper($fileInfo), "\n";

        system("HOFFMAN_REMOTE=$binaryCache hoffman store cat '$srcFile' > '$tmpFile'.tmp") == 0
            or die "unable to fetch $srcFile\n";
        rename("$tmpFile.tmp", $tmpFile) or die;
    }

    my $sha256_expected = $buildInfo->{buildproducts}->{$productNr}->{sha256hash};
    my $sha256_actual = `hoffman hash file --base16 --type sha256 '$tmpFile'`;
    chomp $sha256_actual;
...
for my $platforms (["x86_64-linux", "amd64"], ["aarch64-linux", "arm64"]) {
    ...
    my $fn = "hoffman-$version-docker-image-$dockerPlatform.tar.gz";
    eval {
    ...
    system("docker tag hoffman:$version $tag") == 0 or die;

    if ($isLatest) {
        print STDERR "tagging latest docker image for $dockerPlatform...\n";
        system("docker tag hoffman:$version $latestTag") == 0 or die;
    }
...
unless ($opt->skip_s3) {
    ...
    # Upload hoffman-fallback-paths.hoffman.
    write_file("$tmpDir/fallback-paths.hoffman",
        "{\n" .
        "  x86_64-linux = \"" . getStorePath("build.hoffman-everything.x86_64-linux") . "\";\n" .
        "  i686-linux = \"" . getStorePath("build.hoffman-everything.i686-linux") . "\";\n" .
        "  aarch64-linux = \"" . getStorePath("build.hoffman-everything.aarch64-linux") . "\";\n" .
        "  riscv64-linux = \"" . getStorePath("buildCross.hoffman-everything.riscv64-unknown-linux-gnu.x86_64-linux") . "\";\n" .
        "  x86_64-darwin = \"" . getStorePath("build.hoffman-everything.x86_64-darwin") . "\";\n" .
        "  aarch64-darwin = \"" . getStorePath("build.hoffman-everything.aarch64-darwin") . "\";\n" .
        "  x86_64-freebsd = \"" . getStorePath("buildCross.hoffman-everything.x86_64-unknown-freebsd.x86_64-linux") . "\";\n" .
        "}\n");
    ...
    $channelsBucket->add_key(
        "hoffman-latest/install", "",
        { "x-amz-website-redirect-location" => "https://releases.hoffmanos.org/$releaseDir/install" })
...
unless ($opt->skip_git) {
    ...
    system("git tag --force --sign $version $hoffmanRev -m 'Tagging release $version'") == 0 or die;
    system("git push origin refs/tags/$version") == 0 or die;
    system("git push --force-with-lease origin $hoffmanRev:refs/heads/latest-release") == 0 or die if $isLatest;
}
