...
#  ifdef __APPLE__
    ...
    // (e.g. GNU patch in the Hoffman sandbox) may allocate memory
    // proportional to the fd limit and OOM. Use the kernel's
    // per-process file limit instead, which is the effective cap.
    //
    // GNU patch < 2.8 crashes with **** out of memory, which breaks in hoffmanpkgs darwin bootstrap tools.
    // This was fixed in:
...
void initHoffman(bool loadConfig)
{
...
#ifndef _WIN32
    uhoffman::startSignalHandlerThread();
    ...
    /* Install a dummy HOFFMAN_SIG_MULTI_INT handler for use with pthread_kill(). */
    act.sa_handler = sigHandler;
    if (sigaction(HOFFMAN_SIG_MULTI_INT, &act, 0))
     ...
     * Most signals on BSD systems default to SA_RESTART on, but Hoffman
    ...
    /* There is no privacy in the Hoffman system ;-)  At least not for
    ...
    , parseArg(parseArg)
{
    ...
    addFlag({
        ...
        .description = "Do not write to the Hoffman store.",
    ...
    addFlag({
        .longName = "store",
        .description = "The URL of the Hoffman store to use.",
        .labels = {"store-uri"},
...
{
    std::cout << fmt("%1% (Hoffman) %2%", programName, hoffmanVersion) << std::endl;
    if (verbosity > lvlInfo) {
        ...
        std::cout << "System configuration file: " << os_string_to_string(hoffmanConfFile().native()) << "\n";
        std::cout << "User configuration files: "
                  << os_string_to_string(ExecutablePath{.directories = hoffmanUserConfFiles()}.render()) << "\n";
        std::cout << "Store directory: " << resolveStoreConfig(StoreReference{settings.storeUri.get()})->storeDir
                  << "\n";
        std::cout << "State directory: " << os_string_to_string(settings.hoffmanStateDir.native()) << "\n";
    }
    throw Exit();
}
...
RunPager::RunPager()
{
    if (!isatty(STDOUT_FILENO))
        return;
    char * pager = getenv("HOFFMAN_PAGER");
    if (!pager)
        pager = getenv("PAGER");
