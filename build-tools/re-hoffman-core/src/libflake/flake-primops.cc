...
PrimOp getFlake(const Settings & settings)
{
    auto prim_getFlake = [&settings](EvalState & state, const PosIdx pos, Value ** args, Value & v) {
        ...
        if (args[0]->type() == nPath) {
            ...
            auto flakeRef = hoffman::parseFlakeRef(state.fetchSettings, flakeRefS, {}, true);
    ...
    return PrimOp{
        ...
        .doc = R"(
          ...
          ```hoffman
          (builtins.getFlake "hoffman/55bc52401966fbffa525c574c14f67b00bc4fb3a").packages.x86_64-linux.hoffman
          ```
...
static void prim_parseFlakeRef(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    auto attrs = hoffman::parseFlakeRef(state.fetchSettings, flakeRefS, {}, true).toAttrs();
...
hoffman::PrimOp parseFlakeRef({
    ...
    .doc = R"(
      ...
      ```hoffman
      builtins.parseFlakeRef "github:HoffmanOS/hoffmanpkgs/23.05?dir=lib"
      ```
...
hoffman::PrimOp flakeRefToString({
