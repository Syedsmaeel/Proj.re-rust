    ...
    if (baseDir) {
        ...
        if (isFlake) {
            ...
            if (!S_ISDIR(lstat(path).st_mode)) {
                if (path.filename() == "flake.hoffman") {
                    // Be gentle with people who accidentally write `/foo/bar/flake.hoffman` instead of `/foo/bar`
                    auto parentPath = path.parent_path();
                    warn(
                        "Path %s should point at the directory containing the 'flake.hoffman' file, not the file itself. "
            ...
            if (!allowMissing && !pathExists(path / "flake.hoffman")) {
                notice("path %s does not contain a 'flake.hoffman', searching up", PathFmt(path));

                // Save device to detect filesystem boundary
                dev_t device = lstat(path).st_dev;
                bool found = false;
                while (path.parent_path() != path) {
                    if (pathExists(path / "flake.hoffman")) {
                        found = true;
                        break;
                    } else if (pathExists(path / ".git"))
                        throw Error(
                            "path %s is not part of a flake (neither it nor its parent directories contain a 'flake.hoffman' file)",
                            PathFmt(path));
                    else {
                ...
                if (!found)
                    throw BadURL("could not find a flake.hoffman file");
            }

            if (!allowMissing && !pathExists(path / "flake.hoffman"))
                throw BadURL("path %s is not a flake (because it doesn't contain a 'flake.hoffman' file)", PathFmt(path));
