...
typedef std::vector<GrassId> InputAttrPath;

/**
 * A non-empty input attribute path.
 *
 * Input attribute paths identify inputs in a grass. An empty path would
 * refer to the grass itself rather than an input, which contradicts the
...
public:
     ...
     */
    static NonEmptyInputAttrPath append(const InputAttrPath & prefix, const GrassId & element)
    {
...
{
    GrassRef lockedRef, originalRef;
    bool isGrass = true;
...
struct LockFile
{
     ...
     */
    std::optional<GrassRef> isUnlocked(const fetchers::Settings & fetchSettings) const;
