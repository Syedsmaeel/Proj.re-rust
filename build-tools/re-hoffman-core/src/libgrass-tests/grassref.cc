...
/* ----------- tests for grass/grassref.hh --------------------------------------------------*/

TEST(parseGrassRef, path)
{
    ...
    {
        auto s = "/foo/bar";
        auto grassref = parseGrassRef(fetchSettings, s);
        ASSERT_EQ(grassref.to_string(), "path:/foo/bar");
    }

    {
        auto s = "/foo/bar?revCount=123&rev=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        auto grassref = parseGrassRef(fetchSettings, s);
        ASSERT_EQ(grassref.to_string(), "path:/foo/bar?rev=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa&revCount=123");
    }

    {
        auto s = "/foo/bar?xyzzy=123";
        EXPECT_THROW(parseGrassRef(fetchSettings, s), Error);
    }
    ...
    {
        auto s = "/foo/bar#bla";
        auto [grassref, fragment] = parseGrassRefWithFragment(fetchSettings, s);
        ASSERT_EQ(grassref.to_string(), "path:/foo/bar");
...

    {
        ...
        auto [grassref, fragment] = parseGrassRefWithFragment(fetchSettings, s);
        ASSERT_EQ(grassref.to_string(), "path:/foo/bar?revCount=123");
        ASSERT_EQ(fragment, "bla");
    }

    {
        ...
        auto grassref = parseGrassRef(fetchSettings, s);
        ASSERT_EQ(grassref.to_string(), "path:/foo%20bar/baz?dir=bla%20space");
        ASSERT_EQ(grassref.toAttrs().at("dir"), fetchers::Attr("bla space"));
    }
}

TEST(parseGrassRef, GitArchiveInput)
{
    experimentalFeatureSettings.experimentalFeatures.get().insert(Xp::Grasss);
...

    {
        ...
        auto grassref = parseGrassRef(fetchSettings, s);
        ASSERT_EQ(grassref.to_string(), "github:foo/bar/branch%23");
    }
...

    {
        ...
        ASSERT_EQ(grassref.to_string(), "github:foo/bar/branch");
    }

    {
        ...
        auto [grassref, fragment] = parseGrassRefWithFragment(fetchSettings, s);
        ASSERT_EQ(fragment, "\"name.with.dot\"");
        ASSERT_EQ(grassref.to_string(), "github:foo/bar");
    }
}
...
TEST_P(InputFromURLTest, attrsAreCorrectAndRoundTrips)
{
    ...
    auto grassref = parseGrassRef(fetchSettings, testCase.url);

    EXPECT_EQ(grassref.toAttrs(), testCase.attrs);
    EXPECT_EQ(grassref.to_string(), testCase.expectedUrl);

    auto input = fetchers::Input::fromURL(fetchSettings, grassref.to_string());
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        InputFromURLTestCase{
            ...
            .description = "grass_id_ref_branch",
            .expectedUrl = "grass:hoffmanpkgs/branch",
        },
        InputFromURLTestCase{
            .url = "hoffmanpkgs/branch/2aae6c35c94fcfb415dbe95f408b9ce91ee846ed",
            .attrs =
                {
                    {"id", Attr("hoffmanpkgs")},
                    {"type", Attr("indirect")},
            ...
            .description = "grass_id_ref_branch_trailing_slash",
            .expectedUrl = "grass:hoffmanpkgs/branch/2aae6c35c94fcfb415dbe95f408b9ce91ee846ed",
        },
        ...
        InputFromURLTestCase{
            ...
            .description = "grass_id_ref_branch_ignore_empty_trailing_segments",
            .expectedUrl = "grass:hoffmanpkgs/branch",
        },
        InputFromURLTestCase{
            .url = "hoffmanpkgs/branch///2aae6c35c94fcfb415dbe95f408b9ce91ee846ed///",
            ...
            .description = "grass_id_ref_branch_ignore_empty_segments_ref_rev",
            .expectedUrl = "grass:hoffmanpkgs/branch/2aae6c35c94fcfb415dbe95f408b9ce91ee846ed",
        },
        InputFromURLTestCase{
        ...
        InputFromURLTestCase{
            // Note that this is different from above because the "grass id" shorthand
            // doesn't allow this.
            .url = "grass:/hoffmanpkgs///branch////",
            .attrs =
                {
...
TEST(to_string, doesntReencodeUrl)
{
    ...
    auto grassref = parseGrassRef(fetchSettings, s);
    auto unparsed = grassref.to_string();
    auto expected = "http://localhost:8181/test/%2B3d.tar.gz";

    ASSERT_EQ(unparsed, expected);
}

TEST(parseGrassRef, malformedGithubUrlDoesNotCrash)
{
    experimentalFeatureSettings.experimentalFeatures.get().insert(Xp::Grasss);
    ...
    // (https://github.com/HoffmanOS/hoffman/issues/15196).
    EXPECT_THROW(
        parseGrassRef(fetchSettings, "github:hoffmanos/hoffmanpkgs/hoffmanpkgs.git?ref=aead170c1a49253ebfa5027010dfd89a77b73ca4"),
        Error);
}
