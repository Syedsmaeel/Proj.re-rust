...
TEST_F(hoffman_api_store_test, hoffman_api_init_getFlake_exists)
{
    ...
    auto settings = hoffman_flake_settings_new(ctx);
    assert_ctx_ok();
    ...
    hoffman_err err = hoffman_expr_eval_from_string(ctx, state, "builtins.getFlake", ".", value);
...
TEST_F(hoffman_api_store_test, hoffman_api_flake_reference_not_absolute_no_basedir_fail)
{
    hoffman_libstore_init(ctx);
    assert_ctx_ok();
    hoffman_libexpr_init(ctx);
    ...
    auto parseFlags = hoffman_flake_reference_parse_flags_new(ctx, settings);

    std::string str(".#legacyPackages.aarch127-unknown...orion");
    std::string fragment;
    hoffman_flake_reference * flakeReference = nullptr;
    auto r = hoffman_flake_reference_and_fragment_from_string(
    ...
    hoffman_flake_reference_parse_flags_free(parseFlags);
}

TEST_F(hoffman_api_store_test, hoffman_api_load_flake)
{
    auto tmpDir = hoffman::createTempDir();
    hoffman::AutoDelete delTmpDir(tmpDir, true);

    hoffman::writeFile(tmpDir / "flake.hoffman", R"(
        {
            outputs = { ... }: {
                hello = "potato";
            };
    ...
    auto r0 = hoffman_flake_reference_parse_flags_set_base_directory(
    ...
    auto lockFlags = hoffman_flake_lock_flags_new(ctx, settings);
    assert_ctx_ok();
    ASSERT_NE(nullptr, lockFlags);

    auto lockedFlake = hoffman_flake_lock(ctx, fetchSettings, settings, state, lockFlags, flakeReference);
    assert_ctx_ok();
    ASSERT_NE(nullptr, lockedFlake);

    hoffman_flake_lock_flags_free(lockFlags);

    auto value = hoffman_locked_flake_get_output_attrs(ctx, settings, state, lockedFlake);
    assert_ctx_ok();
    ASSERT_NE(nullptr, value);
    ...
    hoffman_locked_flake_free(lockedFlake);
    hoffman_flake_reference_free(flakeReference);
    hoffman_state_free(state);
    hoffman_flake_settings_free(settings);
}

TEST_F(hoffman_api_store_test, hoffman_api_load_flake_with_flags)
{
    hoffman_libstore_init(ctx);
    assert_ctx_ok();
    hoffman_libexpr_init(ctx);
    ...
    hoffman::writeFile(tmpDir / "b" / "flake.hoffman", R"(
        {
            outputs = { ... }: {
                hello = "BOB";
            };
    ...
    hoffman::writeFile(tmpDir / "a" / "flake.hoffman", R"(
        {
    ...
    hoffman::writeFile(tmpDir / "c" / "flake.hoffman", R"(
        {
            outputs = { ... }: {
                hello = "Claire";
            };
    ...
    hoffman_flake_lock_flags_set_mode_check(ctx, lockFlags);
    assert_ctx_ok();
    ...
    hoffman_flake_lock_flags_set_mode_virtual(ctx, lockFlags);
    assert_ctx_ok();

    lockedFlake = hoffman_flake_lock(ctx, fetchSettings, settings, state, lockFlags, flakeReference);
    assert_ctx_ok();
    ASSERT_NE(nullptr, lockedFlake);
    ...
    hoffman_flake_lock_flags_set_mode_write_as_needed(ctx, lockFlags);
    assert_ctx_ok();
    ...
    // Get the output attrs
    value = hoffman_locked_flake_get_output_attrs(ctx, settings, state, lockedFlake);
    assert_ctx_ok();
    ASSERT_NE(nullptr, value);
    ...
    hoffman_flake_reference * overrideFlakeReference = nullptr;
    hoffman_flake_reference_and_fragment_from_string(
    ...
    hoffman_flake_lock_flags_add_input_override(ctx, lockFlags, "b", overrideFlakeReference);
    assert_ctx_ok();
...
TEST_F(hoffman_api_store_test, hoffman_api_flake_lock_flags_add_input_override_empty_path)
{
    auto tmpDir = hoffman::createTempDir();
    hoffman::AutoDelete delTmpDir(tmpDir, true);
    ...
    auto r = hoffman_flake_lock_flags_add_input_override(ctx, lockFlags, "", flakeReference);
