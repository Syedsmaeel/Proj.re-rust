...
#include "hoffman/util/fs-sink.hh"
...
std::pair<StorePath, Input> Input::fetchToStore(const Settings & settings, Store & store) const
{
    ...
    auto [storePath, input] = [&]() -> std::pair<StorePath, Input> {
        try {
            ...
            auto storePath =
                hoffman::fetchToStore(settings, store, SourcePath(accessor), FetchMode::Copy, result.getName());
...
std::pair<ref<SourceAccessor>, Input> Input::getAccessorUnchecked(const Settings & settings, Store & store) const
{
    ...
    /* The tree may already be in the Hoffman store, or it could be
    ...
    */
            ...
            if (accessor->fingerprint) {
                settings.getCache()->upsert(
                    makeSourcePathToHashCacheKey(
                        *accessor->fingerprint, ContentAddressMethod::Raw::HoffmanArchive, CanonPath::root),
    ...
    PathLocks lock(
        {lockFilePath.string()}, fmt("waiting for another Hoffman process to finish fetching input '%s'...", to_string()));
    lock.setDeletion(true);

    static auto inTest = getEnv("_HOFFMAN_TEST_CONCURRENT_FETCHES") == "1";
    if (inTest)
