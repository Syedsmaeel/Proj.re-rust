...
struct Settings : public Config
{
    ...
    Setting<StringMap> accessTokens{
        ...
        R"(
          ...
          * Gitlab: the token value is either the OAuth2 token or the
          ...
          Example `~/.config/hoffman/hoffman.conf`:
    ...
    Setting<bool> allowDirtyLocks{
        ...
        R"(
          ...
          practice since Hoffman has no way to obtain such inputs if they
    ...
    Setting<bool> trustTarballsFromGitForges{
        ...
        R"(
          If enabled (the default), Hoffman considers tarballs from
          GitHub and similar Git forges to be locked if a Git revision
          is specified,
          e.g. `github:HoffmanOS/patchelf/7c2f768bf9601268a4e71c2ebe91e2011918a70f`.
          This requires Hoffman to trust that the provider returns the
    ...
    Setting<unsigned int> tarballTtl{
        ...
        R"(
          ...
          the cached tarball is stale, Hoffman checks whether it is still up
          to date using the ETag header. Hoffman downloads a new version if
          the ETag header is unsupported, or the cached ETag doesn't match.

          Setting the TTL to `0` forces Hoffman to always check if the tarball is
          up to date.

          Hoffman caches tarballs in `$XDG_CACHE_HOME/hoffman/tarballs`.

          Files fetched via `HOFFMAN_PATH`, `fetchGit`, `fetchMercurial`,
