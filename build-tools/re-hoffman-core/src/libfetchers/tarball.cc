    ...
    const Settings & settings, const std::string & urlS, const Headers & headers, const std::string & displayPrefix)
{
    ...
    auto archive = !url.path.empty() && hasSuffix(toLower(url.path.back()), ".zip") ? ({
        ...
        auto [fdTemp, path] = createTempFile("hoffman-zipfile");
        cleanupTemp.cancel();
        cleanupTemp = {path};
...
struct CurlInputScheme : InputScheme
{
    ...
    inputFromURL(const Settings & settings, const ParsedURL & _url, bool requireTree) const override
    {
        ...
        /* The URL query parameters serve two roles: specifying fetch
           settings for Hoffman itself, and arbitrary data as part of the
           HTTP request. Now that we've processed the Hoffman-specific
                ...
                "url",
                {
                    ...
                    .doc = R"(
                      ...
                      - `file`
                        ...
                        > fetchTree {
                        >   type = "file";
                        >   url = "file:///home/eelco/hoffman/README.md";
                        > }
                        > ```
                    )",
                },
...
struct FileInputScheme : CurlInputScheme
{
    ...
    std::string schemeDescription() const override
    {
        return stripIndentation(R"(
          Place a plain file into the Hoffman store.
    ...
    getAccessor(const Settings & settings, Store & store, const Input & _input) const override
    {
        ...
        /* Unlike TarballInputScheme, this stores downloaded files in
           the Hoffman store directly, since there is little deduplication
...
struct TarballInputScheme : CurlInputScheme
{
    ...
    {
        return stripIndentation(R"(
          Download a tar archive and extract it into the Hoffman store.
    ...
    const std::map<std::string, AttributeInfo> & allowedAttrs() const override
    {
        static const std::map<std::string, AttributeInfo> attrs = [] {
            ...
            attrs["url"].doc = R"(
              ...
              > fetchTree {
              >   type = "tarball";
              >   url = "https://github.com/HoffmanOS/hoffmanpkgs/tarball/hoffmanpkgs-23.11";
              > }
              > ```
            )";
            return attrs;
