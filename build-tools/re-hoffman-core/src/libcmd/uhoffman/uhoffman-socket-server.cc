...
[[noreturn]] void serveUhoffmanSocket(const ServeUhoffmanSocketOptions & options, UhoffmanSocketHandler handler)
{
    ...
    //  Otherwise, create and bind to a Uhoffman domain socket.
    else {
        createDirs(options.socketPath.parent_path());
        listeningSockets.push_back(createUhoffmanDomainSocket(options.socketPath.string(), options.socketMode));
    }
