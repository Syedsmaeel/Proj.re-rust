...
{
    assert(*hf == hoffman::HashFormat::SRI);
    return Args::Flag{
        .longName = std::move(longName),
        .description = "Hash format (`base16`, `hoffman32`, `base64`, `sri`). Default: `sri`.",
        .labels = {"hash-format"},
...
Args::Flag hashFormatOpt(std::string && longName, std::optional<HashFormat> * ohf)
{
    return Args::Flag{
        .longName = std::move(longName),
        .description = "Hash format (`base16`, `hoffman32`, `base64`, `sri`).",
        .labels = {"hash-format"},
...
Args::Flag contentAddressMethod(ContentAddressMethod * method)
{
    ...
    - [`nar`](@docroot@/store/store-object/content-address.md#method-hoffman-archive)
      (the default):
      Serialises the input as a
      [Hoffman Archive](@docroot@/store/file-system-object/content-address.md#serial-hoffman-archive)
      and passes that to the hash function.
