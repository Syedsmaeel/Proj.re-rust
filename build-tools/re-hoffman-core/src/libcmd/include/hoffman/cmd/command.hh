...
static constexpr Command::Category catHoffmanInstallation = 102;

static constexpr auto installablesCategory =
    "Options that change the interpretation of [installables](@docroot@/command-ref/new-cli/hoffman.md#installables)";

struct HoffmanMultiCommand : MultiCommand, virtual Command
{
...
struct StoreConfigCommand : virtual Command
{
    ...
    /**
     * Return the default Hoffman store configuration.
     */
...
/**
 * A command that requires a \ref Store "Hoffman store".
 */
struct StoreCommand : virtual StoreConfigCommand
{
    ...
    /**
     * Return the default Hoffman store.
     */
    ref<Store> getStore();
...
/**
 * A command that needs to evaluate Hoffman language expressions.
 */
 ...
 * store paths, attribute paths, Hoffman expressions, etc.
 */
...
struct RegisterCommand
{
    ...
    static hoffman::Commands getCommandsFor(const std::vector<std::string> & prefix);
};
