...
namespace hoffman::uhoffman {

/**
 * Information about the identity of the peer on a Uhoffman domain socket connection.
 */
struct PeerInfo
{
 ...
 */
using UhoffmanSocketHandler = fun<void(AutoCloseFD socket, std::function<void()> closeListeners)>;
 ...
 */
struct ServeUhoffmanSocketOptions
{
    /**
     * The Uhoffman domain socket path to create and listen on.
     */
     ...
     * Ordinarily the name of the socket unit, e.g. `hoffman-daemon.socket`
 ...
 * - Creating and binding a Uhoffman domain socket if no activation is detected
 ...
 */
[[noreturn]] void serveUhoffmanSocket(const ServeUhoffmanSocketOptions & options, UhoffmanSocketHandler handler);
