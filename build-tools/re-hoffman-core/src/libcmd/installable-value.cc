...
{
    auto evalCache =
        std::make_shared<hoffman::eval_cache::EvalCache>(std::nullopt, state, [&]() { return toValue(state).first; });
...
{
    return UsageError("installable '%s' does not correspond to a Hoffman language value", installable.what());
}
