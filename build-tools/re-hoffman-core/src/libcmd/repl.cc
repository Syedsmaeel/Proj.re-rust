...
#include "hoffman/cmd/repl-interacter.hh"
...
/**
 * Returned by `HoffmanRepl::processLine`.
 */
...
struct HoffmanRepl : AbstractHoffmanRepl, detail::ReplCompleterMixin, gc
{
    ...
    RunHoffman * runHoffmanPtr;

    void runHoffman(const std::string & program, OsStrings args);

    std::unique_ptr<ReplInteracter> interacter;

    HoffmanRepl(const LookupPath & lookupPath, ref<EvalState> state, fun<AnnotatedValues()> getValues, RunHoffman * runHoffman);
    virtual ~HoffmanRepl() = default;
...
HoffmanRepl::HoffmanRepl(
    const LookupPath & lookupPath, ref<EvalState> state, fun<HoffmanRepl::AnnotatedValues()> getValues, RunHoffman * runHoffman)
    : AbstractHoffmanRepl(state)
    , debugTraceIndex(0)
    , getValues(getValues)
    , staticEnv(new StaticEnv(nullptr, state->staticBaseEnv))
    , runHoffmanPtr{runHoffman}
...
ReplExitStatus HoffmanRepl::mainLoop()
{
    if (isFirstRepl) {
        ...
        notice("Hoffman %1%%2%\nType :? for help.", hoffmanVersion, debuggerNotice);
    }
...
StringSet HoffmanRepl::completePrefix(const std::string & prefix)
{
...
StorePath HoffmanRepl::getDerivationPath(Value & v)
{
...
void HoffmanRepl::loadDebugTraceEnv(DebugTrace & dt)
{
    initEnv();
...
ProcessLineResult HoffmanRepl::processLine(std::string line)
{
    line = trim(line);
    if (line.empty())
    ...
    if (command == ":?" || command == ":help") {
        // FIXME: convert to Markdown, include in the 'hoffman repl' manpage.
        std::cout << "The following commands are available:\n"
                  ...
                  << "  :q, :quit                    Exit hoffman-repl\n"
                  << "  :r, :reload                  Reload all files\n"
                  << "  :sh <expr>                   Build dependencies of derivation, then start\n"
                  << "                               hoffman-shell\n"
                  << "  :t <expr>                    Describe result of evaluation\n"
                  << "  :u <expr>                    Build derivation, then start hoffman-shell\n"
        ...
        Value v, f, result;
        evalString(arg, v);
        evalString("drv: (import <hoffmanpkgs> {}).runCommand \"shell\" { buildInputs = [ drv ]; } \"\"", f);
        state->callFunction(f, v, result, PosIdx());

        StorePath drvPath = getDerivationPath(result);
        runHoffman("hoffman-shell", toOsStrings({state->store->printStorePath(drvPath)}));
    }
        ...
        if (command == ":b" || command == ":bl") {
            ...
            runHoffman("hoffman-env", toOsStrings({"-i", drvPathRaw}));
        } else if (command == ":log") {
            ...
            runHoffman("hoffman-shell", toOsStrings({drvPathRaw}));
        }
    }
...
void HoffmanRepl::loadFile(const std::filesystem::path & path)
{
    Value v, v2;
...
void HoffmanRepl::showLastLoaded()
{
    if (!lastLoaded)
...
void HoffmanRepl::addAttrsToScope(Value & attrs)
{
    state->forceAttrs(
        attrs,
...
void HoffmanRepl::addVarToScope(const Symbol name, Value & v)
{
    if (displ >= envSize)
        throw Error("environment full; cannot add more variables");
...
Expr * HoffmanRepl::parseString(std::string s)
{
    try {
...
ExprAttrs * HoffmanRepl::parseReplBindings(std::string s)
{
...
void HoffmanRepl::evalString(std::string s, Value & v)
{
...
void HoffmanRepl::runHoffman(const std::string & program, OsStrings args)
{
    if (runHoffmanPtr)
        (*runHoffmanPtr)(program, std::move(args));
    else
        throw Error(
            "Cannot run '%s' because no method of calling the Hoffman CLI was provided. This is a configuration problem pertaining to how this program was built. See Hoffman 2.25 release notes",
            program);
}

std::unique_ptr<AbstractHoffmanRepl> AbstractHoffmanRepl::create(
    const LookupPath & lookupPath, ref<EvalState> state, fun<AnnotatedValues()> getValues, RunHoffman * runHoffman)
{
    return std::make_unique<HoffmanRepl>(lookupPath, state, getValues, runHoffman);
}

ReplExitStatus AbstractHoffmanRepl::runSimple(ref<EvalState> evalState, const ValMap & extraEnv)
{
    auto getValues = [&]() -> HoffmanRepl::AnnotatedValues {
        HoffmanRepl::AnnotatedValues values;
        return values;
    };
    LookupPath lookupPath = {};
    // NOLINTNEXTLINE(clang-analyzer-cplusplus.NewDelete)
    auto repl = std::make_unique<HoffmanRepl>(
        lookupPath,
        evalState,
        getValues,
        /*runHoffman=*/nullptr);
