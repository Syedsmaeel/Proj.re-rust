...
hoffman::Commands RegisterCommand::getCommandsFor(const std::vector<std::string> & prefix)
{
    hoffman::Commands res;
...
nlohmann::json HoffmanMultiCommand::toJSON()
{
    // FIXME: use Command::toJSON() as well.
    return MultiCommand::toJSON();
}

void HoffmanMultiCommand::run()
{
    if (!command) {
        ...
        std::string markdownError =
            fmt("`hoffman %s` requires a sub-command. Available sub-commands:\n\n%s\n",
                commandName,
                concatStringsSep("\n", subCommandTextLines));
...
CopyCommand::CopyCommand()
{
    addFlag({
        .longName = "from",
        .description = "URL of the source Hoffman store.",
        .labels = {"store-uri"},
    ...
    addFlag({
        .longName = "to",
        .description = "URL of the destination Hoffman store.",
        .labels = {"store-uri"},
...
ref<EvalState> EvalCommand::getEvalState()
{
    if (!evalState) {
        ...
        if (startReplOnEvalErrors) {
            evalState->debugRepl = &AbstractHoffmanRepl::runSimple;
        };
    }
...
void MixProfile::updateProfile(Store & store_, const StorePath & storePath)
{
    ...
    if (!store)
        throw Error("'--profile' is not supported for this Hoffman store");
