...
FlakeRef InstallableFlake::hoffmanpkgsFlakeRef() const
{
    auto lockedFlake = getLockedFlake();

    if (auto hoffmanpkgsInput = lockedFlake->lockFile.findInput({"hoffmanpkgs"})) {
        if (auto lockedNode = std::dynamic_pointer_cast<const flake::LockedNode>(hoffmanpkgsInput)) {
            if (lockedNode->isFlake) {
                debug("using hoffmanpkgs flake '%s'", lockedNode->lockedRef);
                return std::move(lockedNode->lockedRef);
            }
        }
    }

    return defaultHoffmanpkgsFlakeRef();
}
