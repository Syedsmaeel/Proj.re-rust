...
namespace {
// Used to communicate to HoffmanRepl::getLine whether a signal occurred in ::readline.
...
{
    // Allow hoffman-repl specific settings in .inputrc
    rl_readline_name = "hoffman-repl";
    try {
...
bool ReadlineLikeInteracter::getLine(std::string & input, ReplPromptType promptType)
{
    ...
    if (auto e = getEnv("_HOFFMAN_TEST_REPL_ECHO"); s && e && *e == "1") {
#if !USE_READLINE
