...
void ExprLambda::setDocComment(DocComment docComment)
{
    ...
    // See https://github.com/HoffmanOS/rfcs/blob/master/rfcs/0145-doc-strings.md#ambiguous-placement
    if (!this->docComment) {
 ...
 * two below functions.
 *
 * To be removed by https://github.com/HoffmanOS/hoffman/pull/11121
 */
...
void ExprCall::warnIfCursedOr(const SymbolTable & symbols, const PosTable & positions)
{
    if (cursedOrEndPos.has_value()) {
        ...
        out << "at " << positions[pos]
            << ": "
               "This expression uses `or` as an identifier in a way that will change in a future Hoffman release.\n"
               "Wrap this entire expression in parentheses to preserve its current meaning:\n"
               "    ("
            << positions[pos].getSnippetUpTo(positions[*cursedOrEndPos]).value_or("could not read expression")
            << ")\n"
               "Give feedback at https://github.com/HoffmanOS/hoffman/pull/11121";
        warn(out.str());
    }
}
