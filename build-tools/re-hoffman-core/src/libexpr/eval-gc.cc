...
static inline void initGCReal()
{
    ...
    /* Set the initial heap size to something fairly big (25% of
       ...
       should probably also provide a hoffman.conf setting for this.  Note
...
void initGC()
{
    ...
    // HOFFMAN_PATH must override the regular setting
    // See the comment in applyConfig
    if (auto hoffmanPathEnv = getEnv("HOFFMAN_PATH")) {
        globalConfig.set("hoffman-path", concatStringsSep(" ", EvalSettings::parseHoffmanPath(hoffmanPathEnv.value())));
    }
