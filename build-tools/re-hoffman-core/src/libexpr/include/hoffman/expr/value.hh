...
#include "hoffman/util/checked-arithmetic.hh"
...
using HoffmanInt = checked::Checked<int64_t>;
using HoffmanFloat = double;

/**
 * External values must descend from ExternalValueBase, so that
 * type-agnostic hoffman functions (e.g. showType) can be implemented
 */
...
public:
    ...
    printValueAsJSON(EvalState & state, bool strict, HoffmanStringContext & context, bool copyToStore = true) const;
...
struct ValueBase
{
    ...
    struct StringWithContext
    {
         ...
         * context element.
         *
         * @See HoffmanStringContext for an more easily understood type,
        ...
        public:
             ...
             */
            static Context * fromBuilder(const HoffmanStringContext & context, EvalMemory & mem);
        };
 ...
 * InternalType <-> C++ type.
 */
#define HOFFMAN_VALUE_STORAGE_FOR_EACH_FIELD(MACRO)                     \
    MACRO(HoffmanInt, integer, tInt)                                    \
    ...
    MACRO(HoffmanFloat, fpoint, tFloat)

#define HOFFMAN_VALUE_PAYLOAD_TYPE(T, FIELD_NAME, DISCRIMINATOR) \
    template<>                                               \
...
HOFFMAN_VALUE_STORAGE_FOR_EACH_FIELD(HOFFMAN_VALUE_PAYLOAD_TYPE)

#undef HOFFMAN_VALUE_PAYLOAD_TYPE
...
protected:
    using Payload = union
    {
#define HOFFMAN_VALUE_STORAGE_DEFINE_FIELD(T, FIELD_NAME, DISCRIMINATOR) T FIELD_NAME;
        HOFFMAN_VALUE_STORAGE_FOR_EACH_FIELD(HOFFMAN_VALUE_STORAGE_DEFINE_FIELD)
#undef HOFFMAN_VALUE_STORAGE_DEFINE_FIELD
    };
...
protected:
#define HOFFMAN_VALUE_STORAGE_GET_IMPL(K, FIELD_NAME, DISCRIMINATOR) \
...
#define HOFFMAN_VALUE_STORAGE_SET_IMPL(K, FIELD_NAME, DISCRIMINATOR) \
    ...
    HOFFMAN_VALUE_STORAGE_FOR_EACH_FIELD(HOFFMAN_VALUE_STORAGE_GET_IMPL)
    HOFFMAN_VALUE_STORAGE_FOR_EACH_FIELD(HOFFMAN_VALUE_STORAGE_SET_IMPL)

#undef HOFFMAN_VALUE_STORAGE_SET_IMPL
#undef HOFFMAN_VALUE_STORAGE_GET_IMPL
#undef HOFFMAN_VALUE_STORAGE_FOR_EACH_FIELD
...
protected:
    ...
    InternalType getInternalType() const noexcept
    {
        ...
        [[unlikely]] default:
            hoffmanUnreachableWhenHardened();
        }
    }

#define HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS(TYPE, MEMBER_A, MEMBER_B)                                   \
                                                                                                       \
    ...
    HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS(SmallList, [0], [1])
    HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS(PrimOpApplicationThunk, .left, .right)
    HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS(FunctionApplicationThunk, .left, .right)
    HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS(ClosureThunk, .env, .expr)
    HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS(Lambda, .env, .fun)

#undef HOFFMAN_VALUE_STORAGE_DEF_PAIR_OF_PTRS

    void getStorage(HoffmanInt & integer) const noexcept
    {
        Payload payload = loadPayload();
    ...
    void getStorage(HoffmanFloat & fpoint) const noexcept
    {
        Payload payload = loadPayload();
        fpoint = std::bit_cast<HoffmanFloat>(payload[1]);
    }
    ...
    void setStorage(HoffmanInt integer) noexcept
    {
    ...
    void setStorage(HoffmanFloat fpoint) noexcept
    {
...
public:
    ...
    inline void mkInt(HoffmanInt::Inner n) noexcept
    {
        mkInt(HoffmanInt{n});
    }

    inline void mkInt(HoffmanInt n) noexcept
    {
        setStorage(HoffmanInt{n});
    }
    ...
    void mkString(std::string_view s, const HoffmanStringContext & context, EvalMemory & mem);

    void mkStringMove(const StringData & s, const HoffmanStringContext & context, EvalMemory & mem);
    ...
    inline void mkFloat(HoffmanFloat n) noexcept
    {
        setStorage(n);
    }
    ...
    HoffmanInt integer() const noexcept
    {
        return getStorage<HoffmanInt>();
    }

    HoffmanFloat fpoint() const noexcept
    {
        return getStorage<HoffmanFloat>();
    }
