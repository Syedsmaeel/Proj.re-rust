...
#include "hoffman/expr/eval-profiler.hh"
...
#include "hoffman/util/position.hh"
#include "hoffman/util/pos-table.hh"
#include "hoffman/util/source-accessor.hh"
#include "hoffman/expr/search-path.hh"
#include "hoffman/expr/repl-exit-status.hh"
#include "hoffman/util/ref.hh"
#include "hoffman/expr/counter.hh"

// For `HOFFMAN_USE_BOEHMGC`, and if that's set, `GC_THREADS`
    ...
    static constexpr auto preallocate()
    {
        ...
        StaticEvalSymbols staticSymbols = {
            ...
            .exprSymbols = {
                ...
                .hoffmanPath = alloc.create("__hoffmanPath"),
...
public:
    ...
    /**
     * If set, force copying files to the Hoffman store even if they
     * already exist there.
     */
    RepairFlag repair;
    ...
    /**
     * The in-memory filesystem for <hoffman/...> paths.
     */
    const ref<MemorySourceAccessor> corepkgsFS;

    /**
     * In-memory filesystem for internal, non-user-callable Hoffman
     * expressions like `derivation.hoffman`.
     */
...
private:
    ...
    /**
     * A cache that maps paths to "resolved" paths for importing Hoffman
     * expressions, i.e. `/foo` to `/foo/default.hoffman`.
     */
...
public:
     ...
     * @param buildStore     The store to use for builds ("import from derivation", C API `hoffman_string_realise`)
     */
    EvalState(
...
public:
     ...
     */
    HoffmanInt forceInt(Value & v, const PosIdx pos, std::string_view errorCtx);
    HoffmanFloat forceFloat(Value & v, const PosIdx pos, std::string_view errorCtx);
...
public:
    ...
    std::optional<std::string> tryAttrsToString(
        const PosIdx pos, Value & v, HoffmanStringContext & context, bool coerceMore = false, bool copyToStore = true);
     ...
     * evaluator does is just evaluate hoffman expressions from those locations.
    ...
    /**
     * Ensure that all HoffmanStringContextElem::Opaque context elements get fetched
     * to the store.
     */
    void ensureLazyPathsCopied(const HoffmanStringContext & context);
     ...
     * referenced paths are copied to the Hoffman store as a side effect.
     */
    ...
    StorePath copyPathToStore(HoffmanStringContext & context, const SourcePath & path);
     ...
     */
    SourcePath coerceToPath(const PosIdx pos, Value & v, HoffmanStringContext & context, std::string_view errorCtx);

    /**
     * Like coerceToPath, but the result must be a store path.
     */
    StorePath coerceToStorePath(const PosIdx pos, Value & v, HoffmanStringContext & context, std::string_view errorCtx);
     ...
     * element, which is either a `HoffmanStringContextElem::Opaque` or
     * `HoffmanStringContextElem::Built`. (`HoffmanStringContextEleme::DrvDeep`
     * is not permitted).
     *
...
private:
    ...
    /**
     * Current Hoffman call stack depth, used with `max-call-depth` setting to throw stack overflow hopefully before we run
     * out of system stack.
     */
    size_t callDepth = 0;
...
public:
     ...
     * single `HoffmanStringContextElem::Opaque` element of that store path.
     */
     ...
     *
     * The string is the printed store path with a context containing a
     * single `HoffmanStringContextElem::Built` element of the drv path and
     * output name.
     *
     ...
     *
     * @return true if Hoffman was built with GC and a GC was performed, false if not.
     ...
     */
    [[nodiscard]] StringMap
    realiseContext(const HoffmanStringContext & context, StorePathSet * maybePaths = nullptr, bool isIFD = true);
...
/**
 * If `path` refers to a directory, then append "/default.hoffman".
 *
 * @param addDefaultHoffman Whether to append "/default.hoffman" after resolving symlinks.
 */
SourcePath resolveExprPath(SourcePath path, bool addDefaultHoffman = true);
