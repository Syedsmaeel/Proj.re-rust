...
struct EvalSettings : Config
{
    ...
    static Strings getDefaultHoffmanPath();

    static bool isPseudoUrl(std::string_view s);

    static Strings parseHoffmanPath(const std::string & s);
    ...
    Setting<bool> enableNativeCode{this, false, "allow-unsafe-native-code-during-evaluation", R"(
        ...
        - `builtins.importNative` *path* *symbol*
          ...
          The [Hoffman C++ API documentation](@docroot@/development/documentation.md#api-documentation) has more details on evaluator internals.

        - `builtins.exec` *arguments*

          Execute a program, where *arguments* are specified as a list of strings, and parse its output as a Hoffman expression.
    )"};

    Setting<Strings> hoffmanPath{
        this,
        {},
        "hoffman-path",
        R"(
          ...
          [`builtins.hoffmanPath`](@docroot@/language/builtins.md#builtins-hoffmanPath) and can be used with [`builtins.findFile`](@docroot@/language/builtins.md#builtins-findFile).

          - The configuration setting is overridden by the [`HOFFMAN_PATH`](@docroot@/command-ref/env-common.md#env-HOFFMAN_PATH)
          environment variable.
          - `HOFFMAN_PATH` is overridden by [specifying the setting as the command line flag](@docroot@/command-ref/conf-file.md#command-line-flags) `--hoffman-path`.
          - Any current value is extended by the [`-I` option](@docroot@/command-ref/opt-common.md#opt-I) or `--extra-hoffman-path`.

          If the respective paths are accessible, the default values are:

          - `$HOME/.hoffman-defexpr/channels`

            The [user channel link](@docroot@/command-ref/files/default-hoffman-expression.md#user-channel-link), pointing to the current state of [channels](@docroot@/command-ref/files/channels.md) for the current user.

          - `hoffmanpkgs=$HOFFMAN_STATE_DIR/profiles/per-user/root/channels/hoffmanpkgs`

            The current state of the `hoffmanpkgs` channel for the `root` user.

          - `$HOFFMAN_STATE_DIR/profiles/per-user/root/channels`

            The current state of all channels for the `root` user.

          These files are set up by the [Hoffman installer](@docroot@/installation/installing-binary.md).
          See [`HOFFMAN_STATE_DIR`](@docroot@/command-ref/env-common.md#env-HOFFMAN_STATE_DIR) for details on the environment variable.

          > **Note**
          >
          > If [restricted evaluation](@docroot@/command-ref/conf-file.md#conf-restrict-eval) is enabled, the default value is empty.
          >
          > If [pure evaluation](#conf-pure-eval) is enabled, `builtins.hoffmanPath` *always* evaluates to the empty list `[ ]`.
        )",
        {},
        false};
    ...
    Setting<std::string> currentSystem{
        ...
        R"(
          ...
          in the Hoffman language if it is set as a non-empty string.
          ...
          This is useful for evaluating Hoffman code on one system to produce derivations to be built on another type of system.
        )"};
    ...
    Setting<bool> restrictEval{
        ...
        "restrict-eval",
        R"(
          If set to `true`, the Hoffman evaluator doesn't allow access to any
          files outside of
          [`builtins.hoffmanPath`](@docroot@/language/builtins.md#builtins-hoffmanPath),
    ...
    Setting<bool> pureEval{
        ...
        "pure-eval",
        R"(
          Pure evaluation mode ensures that the result of Hoffman expressions is fully determined by explicitly declared inputs, and not influenced by external state:
    ...
    Setting<bool> traceImportFromDerivation{
        ...
        R"(
          By default, Hoffman allows [Import from Derivation](@docroot@/language/import-from-derivation.md).

          When this setting is `true`, Hoffman logs a warning indicating that it performed such an import.
    ...
    Setting<bool> enableImportFromDerivation{
        ...
        R"(
          ...
          With this option set to `false`, Hoffman throws an error when evaluating an expression that uses this feature,
    ...
    Setting<Strings> allowedUris{
        ...
        R"(
          ...
          `https://github.com/HoffmanOS`, builtin functions such as `fetchGit` are
          allowed to access `https://github.com/HoffmanOS/patchelf.git`.
    ...
    Setting<bool> traceFunctionCalls{
        ...
        R"(
          If set to `true`, the Hoffman evaluator traces every function call.
          Hoffman prints a log message at the "vomit" level for every function
          entrance and function exit.

              function-trace entered undefined position at 1565795816999559622
              function-trace exited undefined position at 1565795816999581277
              function-trace entered /hoffman/store/.../example.hoffman:226:41 at 1565795253249935150
              function-trace exited /hoffman/store/.../example.hoffman:226:41 at 1565795253249941684

          The `undefined position` means the function call is a builtin.

          Use the `contrib/stack-collapse.py` script distributed with the Hoffman
    ...
    Setting<bool> ignoreExceptionsDuringTry{
        ...
        "ignore-try",
        R"(
          If set to true, ignore exceptions inside 'tryEval' calls when evaluating Hoffman expressions in
    ...
    Setting<bool> builtinsTraceDebugger{
        ...
        R"(
          ...
          * [`builtins.warn`](@docroot@/language/builtins.md#builtins-warn)
          ...
          This is useful for debugging warnings in third-party Hoffman code.
        )"};
    ...
    Setting<bool> builtinsAbortOnWarn{
        ...
        R"(
          ...
          This is useful for finding information about warnings in third-party Hoffman code when you can not start the interactive debugger, such as when Hoffman is called from a non-interactive script. See [`debugger-on-warn`](#conf-debugger-on-warn).

          Currently, a stack trace can only be produced when the debugger is enabled, or when evaluation is aborted.

          This option can be enabled by setting `HOFFMAN_ABORT_ON_WARN=1` in the environment.
        )"};
    ...
    Setting<Diagnose> lintAbsolutePathLiterals{
        ...
        R"(
          ...
          such paths often make Hoffman expressions less portable and reproducible,
    ...
    Setting<Diagnose> lintUrlLiterals{
        ...
        "lint-url-literals",
        R"(
          Controls handling of unquoted URLs as part of the Hoffman language syntax.
          The Hoffman language allows for URL literals, like so:

          ```
          $ hoffman repl
          hoffman-repl> http://foo
          "http://foo"
          ```

          Setting this to `warn` or `fatal` will cause the Hoffman parser to
          warn or throw an error when encountering a URL literal:

          ```
          $ hoffman repl --lint-url-literals fatal
          hoffman-repl> http://foo
...
/**
 * Conventionally part of the default hoffman path in impure mode.
 */
