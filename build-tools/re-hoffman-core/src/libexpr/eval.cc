...
#include "hoffman/expr/print-options.hh"
...
#include "hoffman/expr/function-trace.hh"
#include "hoffman/store/profiles.hh"
#include "hoffman/expr/print.hh"
#include "hoffman/fetchers/filtering-source-accessor.hh"
...
EvalState::EvalState(
    ...
    , storeFS(makeMountedSourceAccessor({
          ...
          /* In the pure eval case, we can simply require
             ...
             another Hoffman store there), but the relocated
             store does not.
    ...
    , rootFS([&] {
        /* In pure eval mode, we provide a filesystem that only
           ...
           /hoffman/store while using a chroot store, and also for lazy
           mounted fetchTree. */
    ...
    , derivationInternal{internalFS->addFile(
          CanonPath("derivation-internal.hoffman"),
#include "primops/derivation.hoffman.gen.hh"
          )}
    , store(store)
    ...
    , staticBaseEnv{std::make_shared<StaticEnv>(nullptr, nullptr)}
{
    ...
    std::call_once(stackSizeBumped, []() {
        ...
        hoffman::ensureStackSizeAtLeast(60 * 1024 * 1024);
    });
#endif

    corepkgsFS->setPathDisplay("<hoffman", ">");
    internalFS->setPathDisplay("«hoffman-internal»", "");

    countCalls = getEnv("HOFFMAN_COUNT_CALLS").value_or("0") != "0";

    static_assert(sizeof(Env) <= 16, "environment must be <= 16 bytes");

    /* Construct the Hoffman expression search path. */
    assert(lookupPath.elements.empty());
    if (!settings.pureEval) {
        for (auto & i : lookupPathFromArguments.elements) {
            lookupPath.elements.emplace_back(LookupPath::Elem{i});
        }
        /* $HOFFMAN_PATH overriding regular settings is implemented as a hack in `initGC()` */
        for (auto & i : settings.hoffmanPath.get()) {
            lookupPath.elements.emplace_back(LookupPath::Elem::parse(i));
        }
        if (!settings.restrictEval) {
            for (auto & i : EvalSettings::getDefaultHoffmanPath()) {
                lookupPath.elements.emplace_back(LookupPath::Elem::parse(i));
            }
        }
    }
    ...
    corepkgsFS->addFile(
        CanonPath("fetchurl.hoffman"),
#include "fetchurl.hoffman.gen.hh"
    );
...
Value::StringWithContext::Context::fromBuilder(const HoffmanStringContext & context, EvalMemory & mem)
{
    if (context.empty())
        return nullptr;
    ...
    std::ranges::transform(
        context, ctx->elems, [&](const HoffmanStringContextElem & elt) { return &StringData::make(mem, elt.to_string()); });
    return ctx;
}

void Value::mkString(std::string_view s, const HoffmanStringContext & context, EvalMemory & mem)
{
    mkStringNoCopy(StringData::make(mem, s), Value::StringWithContext::Context::fromBuilder(context, mem));
}

void Value::mkStringMove(const StringData & s, const HoffmanStringContext & context, EvalMemory & mem)
{
...
void EvalState::mkStorePathString(const StorePath & p, Value & v)
{
    v.mkString(
        ...
        HoffmanStringContext{
            HoffmanStringContextElem::Opaque{.path = p},
        },
        mem);
}
...
void EvalState::mkOutputString(
    ...
    const ExperimentalFeatureSettings & xpSettings)
{
    value.mkString(mkOutputStringRaw(b, optStaticOutputPath, xpSettings), HoffmanStringContext{b}, mem);
}
...
void EvalState::mkSingleDerivedPathString(const SingleDerivedPath & p, Value & v)
{
    v.mkString(
        ...
        HoffmanStringContext{
            std::visit([](auto && v) -> HoffmanStringContextElem { return v; }, p),
        },
        mem);
}
...
/**
 * A helper `Expr` class to lets us parse and evaluate Hoffman expressions
...
void EvalState::evalFile(const SourcePath & path, Value & v, bool mustBeTrivial)
{
    ...
    // https://github.com/HoffmanOS/hoffman/pull/13930 is merged. That will ensure
...
void ExprCall::eval(EvalState & state, Env & env, Value & v)
{
    ...
    // Empirical arity of Hoffmanpkgs lambdas by regex e.g. ([a-zA-Z]+:(\s|(/\*.*\/)|(#.*\n))*){5}
    // 2: over 4000
    // 3: about 300
    // 4: about 60
    // 5: under 10
...
Hoffman attempted to evaluate a function as a top level expression; in
this case it must have its arguments supplied either by default
values, or passed explicitly with '--arg' or '--argstr'. See
https://hoffman.dev/manual/hoffman/stable/language/syntax.html#functions.)",
                    symbols[i.name])
                    .atPos(i.pos)
...
HoffmanInt EvalState::forceInt(Value & v, const PosIdx pos, std::string_view errorCtx)
{
    try {
        forceValue(v, pos);
        if (v.type() != nInt)
...
HoffmanFloat EvalState::forceFloat(Value & v, const PosIdx pos, std::string_view errorCtx)
{
    try {
        forceValue(v, pos);
        if (v.type() == nInt)
...
void copyContext(const Value & v, HoffmanStringContext & context, const ExperimentalFeatureSettings & xpSettings)
{
    if (auto * ctx = v.context())
        for (auto * elem : *ctx)
            context.insert(HoffmanStringContextElem::parse(elem->view(), xpSettings));
}
...
std::string_view EvalState::forceStringNoCtx(Value & v, const PosIdx pos, std::string_view errorCtx)
{
    auto s = forceString(v, pos, errorCtx);
    if (v.context()) {
        auto ctxElem = HoffmanStringContextElem::parse((*v.context()->begin())->view());
        error<EvalError>(
...
EvalState::tryAttrsToString(const PosIdx pos, Value & v, HoffmanStringContext & context, bool coerceMore, bool copyToStore)
{
...
StorePath EvalState::copyPathToStore(HoffmanStringContext & context, const SourcePath & path)
{
    if (hoffman::isDerivation(path.path.abs()))
    ...
    context.insert(HoffmanStringContextElem::Opaque{.path = dstPath});
    return dstPath;
}

SourcePath EvalState::coerceToPath(const PosIdx pos, Value & v, HoffmanStringContext & context, std::string_view errorCtx)
{
    try {
        forceValue(v, pos);
    } catch (Error & e) {
...
StorePath
EvalState::coerceToStorePath(const PosIdx pos, Value & v, HoffmanStringContext & context, std::string_view errorCtx)
{
    auto path = coerceToString(pos, v, context, errorCtx, false, false, true).toOwned();
    if (auto storePath = store->maybeParseStorePath(path))
        return *storePath;
    error<EvalError>("path '%1%' is not in the Hoffman store", path).withTrace(pos, errorCtx).debugThrow();
}
    ...
    const PosIdx pos, Value & v, std::string_view errorCtx, const ExperimentalFeatureSettings & xpSettings)
{
    ...
    auto derivedPath = std::visit(
        overloaded{
            [&](HoffmanStringContextElem::Opaque && o) -> SingleDerivedPath { return std::move(o); },
            [&](HoffmanStringContextElem::DrvDeep &&) -> SingleDerivedPath {
                error<EvalError>(
            ...
            [&](HoffmanStringContextElem::Built && b) -> SingleDerivedPath { return std::move(b); },
        },
        ((HoffmanStringContextElem &&) *context.begin()).raw);
    return {
...
bool Counter::enabled = getEnv("HOFFMAN_SHOW_STATS").value_or("0") != "0";
...
void EvalState::printStatistics()
{
    ...
    auto outPath = getEnv("HOFFMAN_SHOW_STATS_PATH").value_or("-");
    std::fstream fs;
    if (outPath != "-")
    ...
    if (getEnv("HOFFMAN_SHOW_SYMBOLS").value_or("0") != "0") {
...
SourcePath resolveExprPath(SourcePath path, bool addDefaultHoffman)
{
    ...
    /* If `path' refers to a directory, append `/default.hoffman'. */
    if (addDefaultHoffman && path.resolveSymlinks().lstat().type == SourceAccessor::tDirectory)
        return path / "default.hoffman";
...
SourcePath EvalState::findFile(const LookupPath & lookupPath, const std::string_view path, const PosIdx pos)
{
    ...
    if (hasPrefix(path, "hoffman/"))
        return {corepkgsFS, CanonPath(path.substr(3))};

    error<ThrownError>(
        settings.pureEval ? "cannot look up '<%s>' in pure evaluation mode (use '--impure' to override)"
                          : "file '%s' was not found in the Hoffman search path (add it using $HOFFMAN_PATH or -I)",
        path)
        .atPos(pos)
        .debugThrow();
}
...
EvalState::resolveLookupPathPath(const LookupPath::Path & value0, bool initAccessControl)
{
    ...
    if (EvalSettings::isPseudoUrl(value)) {
        try {
            ...
            logWarning({.msg = HintFmt("Hoffman search path entry '%1%' cannot be downloaded, ignoring", value)});
        }
    }
...

    {
        ...
        if (path.resolveSymlinks().pathExists())
            ...
            logWarning({.msg = HintFmt("Hoffman search path entry '%1%' does not exist, ignoring", value)});
        }
    }
...
std::string ExternalValueBase::coerceToString(
    EvalState & state, const PosIdx & pos, HoffmanStringContext & context, bool copyMore, bool copyToStore) const
{
...
void forceNoNullByte(std::string_view s, std::function<Pos()> pos)
{
    if (s.find('\0') != s.npos) {
        ...
        Error error("input string '%s' cannot be represented as Hoffman string because it contains null bytes", str);
        if (pos) {
            error.atPos(pos());
        }
