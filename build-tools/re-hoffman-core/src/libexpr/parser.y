%skeleton "lalr1.cc"
%define api.location.type { ::hoffman::ParserLocation }
%define api.namespace { ::hoffman::parser }
...
%parse-param { hoffman::ParserState * state }
%lex-param { void * scanner }
%lex-param { hoffman::ParserState * state }
%expect 0
...
#include "hoffman/expr/parser-state.hh"

#define YY_DECL                                    \
    int yylex(                                     \
        hoffman::Parser::value_type * yylval_param,    \
        hoffman::Parser::location_type * yylloc_param, \
        yyscan_t yyscanner,                        \
        hoffman::ParserState * state)
...
%token <HoffmanInt> INT_LIT "integer"
%token <HoffmanFloat> FLOAT_LIT "floating-point literal"
...
expr_select
  ...
  | /* Backwards compatibility: because Hoffmanpkgs has a function named ‘or’,
       allow stuff like ‘map or [...]’. This production is problematic (see
       https://github.com/HoffmanOS/hoffman/issues/11118) and will be refactored in the
...
expr_simple
  ...
  | SPATH {
      ...
      $$ = state->exprs.add<ExprCall>(CUR_POS,
          ...
          {state->exprs.add<ExprVar>(state->s.hoffmanPath),
