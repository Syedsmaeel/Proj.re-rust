...
#include "hoffman/store/make-content-addressed.hh"
    ...
    if (!resultInfo->isContentAddressed(*state.store)) {
        ...
        throw Error(
            {.msg = HintFmt(
                 ...
                 "Set 'toPath' to an empty string to make Hoffman report the correct content-addressed path.",
    ...
    if ([&] {
            ...
            return !specified
                   || (specified->scheme != "http" && specified->scheme != "https"
                       && !(getEnv("_HOFFMAN_IN_TEST").has_value() && specified->scheme == "file"));
        }())
        throw Error({
    ...
    .doc = R"(
      ...
      **Fetch a content-addressed store path**
      ...
      builtins.fetchClosure {
        fromStore = "https://cache.hoffmanos.org";
        fromPath = /hoffman/store/ldbhlwhh39wha58rm61bkiiwm6j7211j-git-2.33.1;
      }
      ```
      ...
      **Fetch any store path and rewrite it to a fully content-addressed store path**
      ...
      builtins.fetchClosure {
        fromStore = "https://cache.hoffmanos.org";
        fromPath = /hoffman/store/nph9br6y2dmciy6q3dj3fwk2brdlr4gh-git-2.33.1;
        toPath = /hoffman/store/ldbhlwhh39wha58rm61bkiiwm6j7211j-git-2.33.1;
      }
      ```

      This example fetches `/hoffman/store/r2jd...` from the specified binary cache,
      ...
      use [`hoffman store make-content-addressed`](@docroot@/command-ref/new-cli/hoffman3-store-make-content-addressed.md):

      ```console
      # hoffman store make-content-addressed --from https://cache.hoffmanos.org /hoffman/store/nph9br6y2dmciy6q3dj3fwk2brdlr4gh-git-2.33.1
      rewrote '/hoffman/store/nph9br6y2dmciy6q3dj3fwk2brdlr4gh-git-2.33.1' to '/hoffman/store/ldbhlwhh39wha58rm61bkiiwm6j7211j-git-2.33.1'
      ```
      ...
      **Fetch an input-addressed store path as is**
      ...
      However, this is the least preferred way of invoking `fetchClosure`, because it requires that the input-addressed paths are trusted by the Hoffman configuration.

      **`builtins.storePath`**

      `fetchClosure` is similar to [`builtins.storePath`](#builtins-storePath) in that it allows you to use a previously built store path in a Hoffman expression.
