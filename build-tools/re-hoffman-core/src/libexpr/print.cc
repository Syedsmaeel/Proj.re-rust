...
#include "hoffman/util/english.hh"
...
private:
    ...
    {
        ::hoffman::printElided(output, value, single, plural, options.ansiColors);
    }
...
public:
    Printer(std::ostream & output, EvalState & state, PrintOptions options, HoffmanStringContext * context)
        : output(output)
        , state(state)
        , options(options)
        , context(context)
...
void printValue(EvalState & state, std::ostream & output, Value & v, PrintOptions options, HoffmanStringContext * context)
{
