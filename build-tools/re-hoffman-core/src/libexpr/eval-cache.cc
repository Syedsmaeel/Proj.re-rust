        ...
        case AttrType::String: {
            ...
            if (!queryAttribute.isNull(3))
                for (auto & s : tokenizeString<std::vector<std::string>>(queryAttribute.getStr(3), " "))
                    context.insert(HoffmanStringContextElem::parse(s));
            return {{rowId, string_t{queryAttribute.getStr(2), context}}};
        }
        case AttrType::Bool:
            return {{rowId, queryAttribute.getInt(2) != 0}};
        case AttrType::Int:
            return {{rowId, int_t{HoffmanInt{queryAttribute.getInt(2)}}}};
            ...
            if (auto s = std::get_if<string_t>(&cachedValue->second)) {
                ...
                for (auto & c : s->second) {
                    const StorePath & path = std::visit(
                        overloaded{
                            [&](const HoffmanStringContextElem::DrvDeep & d) -> const StorePath & { return d.drvPath; },
                            [&](const HoffmanStringContextElem::Built & b) -> const StorePath & {
                                return b.drvPath->getBaseStorePath();
                            },
                            [&](const HoffmanStringContextElem::Opaque & o) -> const StorePath & { return o.path; },
                        },
                        c.raw);
