...
#include "hoffman/expr/gc-small-vector.hh"
#include "hoffman/expr/json-to-value.hh"
#include "hoffman/expr/static-string-data.hh"
#include "hoffman/store/globals.hh"
#include "hoffman/store/names.hh"
#include "hoffman/store/path-references.hh"
...
{
    hoffman::HoffmanStringContext stringContext;
    ...
    return hoffman::rewriteStrings(rawStr, rewrites);
}

StringMap EvalState::realiseContext(const HoffmanStringContext & context, StorePathSet * maybePathsOut, bool isIFD)
{
    ...
    for (auto & c : context) {
        ...
        std::visit(
            overloaded{
                ...
                [&](const HoffmanStringContextElem::Opaque & o) {
    ...
    EvalState & state, const PosIdx pos, const SourcePath & path, const StorePath & storePath, Value & v)
{
    ...
    attrs.alloc(state.s.drvPath)
        .mkString(
            path2,
            {
                HoffmanStringContextElem::DrvDeep{.drvPath = storePath},
            },
            state.mem);
...
#include "imported-drv-to-derivation.hoffman.gen.hh"
                , state.rootPath(CanonPath::root)),
            **state.vImportedDrvToDerivation);
    }

    state.forceFunction(
        **state.vImportedDrvToDerivation, pos, "while evaluating imported-drv-to-derivation.hoffman.gen.hh");
    v.mkApp(*state.vImportedDrvToDerivation, w);
    state.forceAttrs(v, pos, "while calling imported-drv-to-derivation.hoffman.gen.hh");
}

/**
 * Import a Hoffman file with an alternate base scope, as `builtins.scopedImport` does.
 *
 * @param state The evaluation state.
 * @param pos The position of the import call.
...
static RegisterPrimOp primop_scopedImport(
    {.name = "scopedImport",
     .args = {"scope", "path"},
     .doc = R"(
      Load, parse, and return the Hoffman expression in the file *path*, with the attributes from *scope* available as variables in the lexical scope of the imported file.
      ...
      If *path* is a directory, the file `default.hoffman` in that directory is used if it exists.

      > **Example**
      >
      > Create a file `greet.hoffman`:
      >
      > ```hoffman
      > # greet.hoffman
      > "${greeting}, ${name}!"
      ...
      >
      > ```hoffman
      > scopedImport { greeting = "Hello"; name = "World"; } ./greet.hoffman
      > ```
      >
      >     "Hello, World!"

      Evaluation aborts if the file doesn't exist or contains an invalid Hoffman expression.
    )",
...
static RegisterPrimOp primop_import(
    {.name = "import",
     ...
     .doc = R"(
      Load, parse, and return the Hoffman expression in the file *path*.

      > **Note**
      >
      > Unlike some languages, `import` is a regular function in Hoffman.
      ...
      >
      > ```console
      > $ echo 123 > default.hoffman
      > ```
      >
      > Import `default.hoffman` from the current directory.
      >
      > ```hoffman
      > import ./.
      > ```
      ...
      A Hoffman expression loaded by `import` must not contain any *free variables*, that is, identifiers that are not defined in the Hoffman expression itself and are not built-in.
      ...
      > rec {
      >   x = 123;
      >   y = import ./foo.hoffman;
      > }
      > ```
      >
      >  then the following `foo.hoffman` throws an error:
      >
      >  ```hoffman
      >  # foo.hoffman
      >  x + 456
      >  ```
      >
      >  since `x` is not in scope in `foo.hoffman`.
      > If you want `x` to be available in `foo.hoffman`, pass it as a function argument:
      >
      >  ```hoffman
      >  rec {
      >    x = 123;
      >    y = import ./foo.hoffman x;
      >  }
      >  ```
      >
      >  and
      ...
      >  ```
      >
      >  The function argument doesn’t have to be called `x` in `foo.hoffman`; any name would work.
    )",
     .impl = [](EvalState & state, const PosIdx pos, Value ** args, Value & v) {
...
static RegisterPrimOp primop_break(
    {.name = "break",
     .args = {"v"},
     .doc = R"(
      In debug mode (enabled using `--debugger`), pause Hoffman expression evaluation and enter the REPL.
...
static RegisterPrimOp primop_abort(
    {.name = "abort",
     .args = {"s"},
     .doc = R"(
      Abort Hoffman expression evaluation and print the error message *s*.
    )",
     .impl = [](EvalState & state, const PosIdx pos, Value ** args, Value & v) {
         HoffmanStringContext context;
         auto s =
...
static RegisterPrimOp primop_throw(
    {.name = "throw",
     .args = {"s"},
     .doc = R"(
      Throw an error message *s*. This usually aborts Hoffman expression
      evaluation, but in `hoffman-env -qa` and other commands that try to
...
static void prim_ceil(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    constexpr HoffmanFloat int_min = std::numeric_limits<HoffmanInt::Inner>::min(); // power of 2, so that no rounding occurs
    if (ceilValue >= int_min && ceilValue < -int_min) {
        v.mkInt(ceilValue);
    } else if (isInt) {
        // a HoffmanInt, e.g. INT64_MAX, can be rounded to -int_min due to the cast to HoffmanFloat
        state
            .error<EvalError>(
                "Due to a bug (see https://github.com/HoffmanOS/hoffman/issues/12899) the HoffmanInt argument %1% caused undefined behavior in previous Hoffman versions.\n\tFuture Hoffman versions might implement the correct behavior.",
                args[0]->integer().value)
            .atPos(pos)
            .debugThrow();
    } else {
        state.error<EvalError>("HoffmanFloat argument %1% is not in the range of HoffmanInt", args[0]->fpoint())
            .atPos(pos)
            .debugThrow();
    }
    // `forceFloat` casts HoffmanInt to HoffmanFloat, but instead HoffmanInt args shall be returned unmodified
    if (isInt) {
        ...
        if (arg != res) {
            state
                .error<EvalError>(
                    "Due to a bug (see https://github.com/HoffmanOS/hoffman/issues/12899) a loss of precision occurred in previous Hoffman versions because the HoffmanInt argument %1% was rounded to %2%.\n\tFuture Hoffman versions might implement the correct behavior.",
                    arg,
                    res)
                .atPos(pos)
                .debugThrow();
...
static RegisterPrimOp primop_ceil({
    ...
    .args = {"number"},
    .doc = R"(
        Rounds and converts *number* to the next higher HoffmanInt value if possible, i.e. `ceil *number* >= *number*` and
        `ceil *number* - *number* < 1`.

        An evaluation error is thrown, if there exists no such HoffmanInt value `ceil *number*`.
        Due to bugs in previous Hoffman versions an evaluation error might be thrown, if the datatype of *number* is
        a HoffmanInt and if `*number* < -9007199254740992` or `*number* > 9007199254740992`.

        If the datatype of *number* is neither a HoffmanInt (signed 64-bit integer) nor a HoffmanFloat
...
static RegisterPrimOp primop_floor({
    ...
    .args = {"number"},
    .doc = R"(
        Rounds and converts *number* to the next lower HoffmanInt value if possible, i.e. `floor *number* <= *number*` and
        `*number* - floor *number* < 1`.

        An evaluation error is thrown, if there exists no such HoffmanInt value `floor *number*`.
        Due to bugs in previous Hoffman versions an evaluation error might be thrown, if the datatype of *number* is
        a HoffmanInt and if `*number* < -9007199254740992` or `*number* > 9007199254740992`.
...
static RegisterPrimOp primop_tryEval({
    ...
    .doc = R"(
      ...
      Instead, use [`builtins.addErrorContext`](@docroot@/language/builtins.md#builtins-addErrorContext) to add context to the error message, and use a Hoffman unit testing tool for testing.
    )",
    .impl = prim_tryEval,
});
...
static RegisterPrimOp primop_getEnv({
    ...
    .doc = R"(
      ...
      dependencies in your Hoffman expression.

      `getEnv` is used in Hoffman Packages to locate the file
      `~/.hoffmanpkgs/config.hoffman`, which contains user-local settings for Hoffman
...
/* Construct (as a unobservable side effect) a Hoffman derivation
   expression that performs the derivation described by the argument
   set.  Returns the original set extended with the following
   attributes: `outPath' containing the primary output path of the
   derivation; `drvPath' containing the path of the Hoffman expression;
...
static void prim_derivationStrict(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    try {
         ...
         * basis in hoffman. There is no accurate location for a derivation, as it
...
static void checkDerivationName(EvalState & state, std::string_view drvName)
{
    try {
        ...
        //     is optional.
        // Note that Hoffmanpkgs generally won't trigger this, because `mkDerivation`
        // sanitizes the name.
        state
            .error<EvalError>(
...
static void derivationStrictInternal(EvalState & state, std::string_view drvName, const Bindings * attrs, Value & v)
{
    ...
    for (auto & i : attrs->lexicographicOrder(state.symbols)) {
        ...
        auto handleHashMode = [&](const std::string_view s) {
            if (s == "recursive") {
                ...
                ingestionMethod = ContentAddressMethod::Raw::HoffmanArchive;
            } else
                try {
            ...
            default:
                ...
                if (jsonObject) {
                    ...
                    if (i->name == state.s.json) {
                        warn(
                            "In derivation '%s': setting structured attributes via '__json' is deprecated, and may be disallowed in future versions of Hoffman. Set '__structuredAttrs = true' instead.",
                            drvName);
    ...
    if (outputHash) {
        ...
        auto method = ingestionMethod.value_or(ContentAddressMethod::Raw::HoffmanArchive);
    ...
    /* Write the resulting term into the Hoffman store directory.
    ...
    result.alloc(state.s.drvPath)
        .mkString(
            drvPathS,
            {
                HoffmanStringContextElem::DrvDeep{.drvPath = drvPath},
            },
            state.mem);
...
/* Return a placeholder string for the specified output that will be
   ...
   be replaced with the concrete path in the Hoffman store of the output
   ‘out’. */
...
/* Allow a valid store path to be used in an expression.  This is
   useful in some generated expressions such as in hoffman-push, which
   generates a call to a function with an already existing store path
   as argument.  You don't want to use `toPath' here because it copies
   the path to the Hoffman store, which yields a copy like
   /hoffman/store/newhash-oldhash-oldname.  In the past, `toPath' had
...
static void prim_storePath(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    /* Resolve symlinks in ‘path’, unless ‘path’ itself is a symlink
       ...
       e.g. hoffman-push does the right thing. */
    if (!state.store->isStorePath(path.abs()))
        path = CanonPath(canonPath(path.abs(), true).string());
    if (!state.store->isInStore(path.abs()))
        state.error<EvalError>("path '%1%' is not in the Hoffman store", path).atPos(pos).debugThrow();
    auto path2 = state.store->toStorePath(path.abs()).first;
    if (!settings.readOnlyMode)
        state.store->ensurePath(path2);
    context.insert(HoffmanStringContextElem::Opaque{.path = path2});
...
static RegisterPrimOp primop_storePath({
    ...
    .doc = R"(
      ...
      = builtins.storePath /hoffman/store/f1d18v1y…-source` causes the
      derivation to depend on the specified path, which must exist or
      be substitutable. Note that this differs from a plain path
      (e.g. `src = /hoffman/store/f1d18v1y…-source`) in that the latter
      causes the path to be *copied* again to the Hoffman store, resulting
      in a new path (e.g. `/hoffman/store/ld01dnzc…-source-source`).
...
static void prim_readFile(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (s.find((char) 0) != std::string::npos)
        state.error<EvalError>("the contents of the file '%1%' cannot be represented as a Hoffman string", path)
            .atPos(pos)
            .debugThrow();
    StorePathSet refs;
...
/* Find a file in the Hoffman search path. Used to implement <x> paths,
   which are desugared to 'findFile __hoffmanPath "x"'. */
static void prim_findFile(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    for (auto v2 : args[0]->listView()) {
        ...
        i = state.getAttr(state.s.path, v2->attrs(), "in an element of the __hoffmanPath");
...
static RegisterPrimOp primop_findFile(
    PrimOp{
      ...
      [Lookup path](@docroot@/language/constructs/lookup-path.md) expressions are [desugared](https://en.wikipedia.org/wiki/Syntactic_sugar) using this and [`builtins.hoffmanPath`](#builtins-hoffmanPath):
      ...
      ```hoffman
      builtins.findFile builtins.hoffmanPath "hoffmanpkgs"
      ```
      ...
      - ```
        {
          prefix = "";
          path = "/hoffman/var/hoffman/profiles/per-user/root/channels";
        }
        ```
      - ```
        {
          prefix = "hoffmanos-config";
          path = "/etc/hoffmanos/configuration.hoffman";
        }
        ```
      - ```
        {
          prefix = "hoffmanpkgs";
          path = "https://github.com/HoffmanOS/hoffmanpkgs/tarballs/master";
        }
        ```
      - ```
        {
          prefix = "hoffmanpkgs";
          path = "channel:hoffmanpkgs-unstable";
        }
        ```
      - ```
        {
      ...
      > ```
      >
      > and a *lookup-path* value `"hoffmanos-config"` causes Hoffman to try `/home/eelco/Dev/hoffmanos-config` and `/etc/hoffmanos` in that order and return the first path that exists.
      ...
      The URLs of the tarballs from the official `hoffmanos.org` channels can be abbreviated as `channel:<channel-name>`.
      See [documentation on `hoffman-channel`](@docroot@/command-ref/hoffman-channel.md) for details about channels.
      ...
      >   {
      >     prefix = "hoffmanpkgs";
      >     path = "https://channels.hoffmanos.org/hoffmanos-unstable/hoffmanexprs.tar.xz";
      >   }
      >   ```
      ...
      > ```
      >
      > makes `<hoffmanpkgs>` refer to a particular branch of the `HoffmanOS/hoffmanpkgs` repository on GitHub.
    )",
        .impl = prim_findFile,
    });
...
/* Convert the argument (which can be any Hoffman expression) to an XML
   representation returned in a string.  Not all Hoffman expressions can
...
static RegisterPrimOp primop_toXML({
    ...
    .doc = R"(
      ...
      fields are optional or contain lists themselves). Instead the Hoffman
      ...
      <expr>
        <list>
          <attrs>
            ...
            <attr name="war">
              <path value="/hoffman/store/d1jh9pasa7k2...-jira/lib/atlassian-jira.war" />
            </attr>
          </attrs>
          <attrs>
            <attr name="path">
            ...
            <attr name="war">
              <path value="/hoffman/store/y6423b1yi4sx...-uberwiki/uberwiki.war" />
            </attr>
          </attrs>
        </list>
      </expr>
      ...
      the stylesheet “inline” in the Hoffman expression. The path of the
...
/* Convert the argument (which can be any Hoffman expression) to a JSON
   string.  Not all Hoffman expressions can be sensibly or completely
...
static RegisterPrimOp primop_fromJSON({
    ...
    .args = {"e"},
    .doc = R"(
      Convert a JSON string to a Hoffman value. For example,
...
/* Store a string in the Hoffman store as a source file that can be used
...
static void prim_toFile(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    for (auto c : context) {
        if (auto p = std::get_if<HoffmanStringContextElem::Opaque>(&c.raw)) {
...
static RegisterPrimOp primop_toFile({
    ...
    .args = {"name", "s"},
    .doc = R"(
      Store the string *s* in a file in the Hoffman store and return its
      path.  The file has suffix *name*. This file can be used as an
      input to derivations. One application is to write builders
      “inline”. For instance, the following Hoffman expression combines the
      Hoffman expression for GNU Hello and its build script into one file:
      ...
      (i.e., a path like `/hoffman/store/m7p7jfny445k...-foo.conf`) will be
      ...
      you are using Hoffmanpkgs, the `writeTextFile` function is able to do
      that.
    )",
    .impl = prim_toFile,
});
...
static void addPath(
    ...
    Value & v,
    const HoffmanStringContext & context)
{
    try {
        StorePathSet refs;
...
static void prim_filterSource(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    addPath(
        state, pos, path.baseName(), path, args[0], ContentAddressMethod::Raw::HoffmanArchive, std::nullopt, v, context);
}
...
static RegisterPrimOp primop_filterSource({
    ...
    .doc = R"(
      ...
      This function allows you to copy sources into the Hoffman store while
      filtering certain files. For instance, suppose that you want to use
      the directory `source-dir` as an input to a Hoffman expression, e.g.
      ...
      Hoffman store, otherwise it is omitted. The function is called with two
      arguments. The first is the full path of the file. The second is a
      string that identifies the type of the file, which is either
      `"regular"`, `"directory"`, `"symlink"` or `"unknown"` (for other
      kinds of files such as device nodes or fifos — but note that those
      cannot be copied to the Hoffman store, so if the predicate returns
...
static void prim_path(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    auto method = ContentAddressMethod::Raw::HoffmanArchive;
        ...
        if (n == "path")
            ...
            method = state.forceBool(
                         *attr.value, attr.pos, "while evaluating the `recursive` attribute passed to builtins.path")
                         ? ContentAddressMethod::Raw::HoffmanArchive
    ...
    .doc = R"(
      ...
      present in *args*. All are optional except `path`:
        ...
        - name\
          ...
          reference paths that have hoffman-illegal characters in their names,
          like `@`.
        ...
        - recursive\
          ...
          [NAR serialization](@docroot@/store/file-system-object/content-address.md#serial-hoffman-archive)
        ...
        - sha256\
          ...
          when the `pure-eval` hoffman config option is on.
    )",
    .impl = prim_path,
});
...
static RegisterPrimOp primop_unsafeGetAttrPos(
    PrimOp{
      ...
      from *set*. This is used by Hoffmanpkgs to provide location information
      in error messages.
    )",
...
static void prim_intersectAttrs(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    // linear in the largest set, which is not acceptable for callPackage in Hoffmanpkgs.
    //
...
static void prim_elemAt(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (n < 0 || std::make_unsigned_t<HoffmanInt::Inner>(n) >= args[0]->listSize())
...
static void prim_genList(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (len_ < 0 || std::make_unsigned_t<HoffmanInt::Inner>(len_) > std::numeric_limits<size_t>::max())
...
static void prim_div(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    HoffmanFloat f2 = state.forceFloat(*args[1], pos, "while evaluating the second operand of the division");
    if (f2 == 0)
    ...
    if (args[0]->type() == nFloat || args[1]->type() == nFloat) {
        ...
        HoffmanInt i1 = state.forceInt(*args[0], pos, "while evaluating the first operand of the division");
        HoffmanInt i2 = state.forceInt(*args[1], pos, "while evaluating the second operand of the division");
...
{
    using HoffmanUInt = std::make_unsigned_t<HoffmanInt::Inner>;
    HoffmanInt::Inner start =
        state
            .forceInt(
    ...
    v.mkString(HoffmanUInt(start) >= s->size() ? "" : s->substr(start, _len), context, state.mem);
}
...
static RegisterPrimOp primop_substring({
    ...
    .doc = R"(
      ...
      *start* must be non-negative.
      ...
      ```hoffman
      builtins.substring 0 3 "hoffmanos"
      ```

      evaluates to `"hoffman"`.
    )",
    .impl = prim_substring,
});
...
static void prim_stringLength(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    v.mkInt(HoffmanInt::Inner(s->size()));
}
...
static void prim_hashString(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    HoffmanStringContext context; // discarded
    auto s =
...
static RegisterPrimOp primop_convertHash({
    ...
    .doc = R"(
      ...
      - `toHashFormat`
        ...
        - `"base16"`
        - `"hoffman32"`
        - `"base32"` (deprecated alias for `"hoffman32"`)
        - `"base64"`
        - `"sri"`
...
static RegisterPrimOp primop_parseDrvName({
    ...
    .doc = R"(
      ...
      `builtins.parseDrvName "hoffman-0.12pre12876"` returns `{ name =
      "hoffman"; version = "0.12pre12876"; }`.
    )",
...
static RegisterPrimOp primop_compareVersions({
    ...
    .doc = R"(
      ...
      algorithm is the same as the one used by [`hoffman-env
      -u`](../command-ref/hoffman-env/upgrade.md).
    )",
...
static RegisterPrimOp primop_splitVersion({
    ...
    .doc = R"(
      ...
      [`hoffman-env -u`](../command-ref/hoffman-env/upgrade.md).
    )",
...
void EvalState::createBaseEnv(const EvalSettings & evalSettings)
{
    ...
    addConstant(
        ...
        v,
        {
          ...
          Since built-in functions were added over time, [testing for attributes](./operators.md#has-attribute) in `builtins` can be used for graceful fallback on older Hoffman installations:
    ...
    addConstant(
        ...
        v,
        {
          ...
          ```hoffman-repl
          hoffman-repl> let true = 1; in true
          1
          ```
        )",
        });
    ...
    addConstant(
        ...
        v,
        {
          ...
          ```hoffman-repl
          hoffman-repl> let false = 1; in false
          1
          ```
        )",
        });
    ...
    addConstant(
        ...
        &Value::vNull,
        {
          ...
          ```hoffman-repl
          hoffman-repl> let null = 1; in null
          1
          ```
        )",
        });
    ...
    addConstant(
        ...
        v,
        {
            .type = nInt,
            .doc = R"(
          Return the [Uhoffman time](https://en.wikipedia.org/wiki/Uhoffman_time) at first evaluation.
          ...
          ```console
          $ hoffman repl
          Welcome to Hoffman 2.15.1 Type :? for help.

          hoffman-repl> builtins.currentTime
          1683705525
    ...
    addConstant(
        ...
        v,
        {
          ...
          It can be used to set the `system` attribute for [`builtins.derivation`](@docroot@/language/derivations.md) such that the resulting derivation can be built on the same system that evaluates the Hoffman expression:
          ...
          ```console
          $ hoffman-instantiate --system "mips64-linux" --eval --expr 'builtins.currentSystem'
          "mips64-linux"
          ```
        )",
            .impureOnly = true,
    ...
    addConstant(
        ...
        v,
        {
            .type = nString,
            .doc = R"(
          The version of Hoffman.

          For example, where the command line returns the current Hoffman version,

          ```shell-session
          $ hoffman --version
          hoffman (Hoffman) 2.16.0
          ```

          the Hoffman language evaluator returns the same value:

          ```hoffman-repl
          hoffman-repl> builtins.hoffmanVersion
          "2.16.0"
          ```
        )",
        });
    ...
    addConstant(
        ...
        v,
        {
            .type = nString,
            .doc = R"(
          Logical file system location of the [Hoffman store](@docroot@/glossary.md#gloss-store) currently in use.

          This value is determined by the `store` parameter in [Store URLs](@docroot@/store/types/index.md#store-url-format):

          ```shell-session
          $ hoffman-instantiate --store 'dummy://?store=/blah' --eval --expr builtins.storeDir
          "/blah"
          ```
        )",
        });
    ...
    addConstant(
        ...
        v,
        {
            .type = nInt,
            .doc = R"(
          The current version of the Hoffman language.
        )",
        });
    ...
    /* Add a value containing the current Hoffman expression search path. */
    ...
    addConstant(
        ...
        v,
        {
          ...
          Its value is primarily determined by the [`hoffman-path` configuration setting](@docroot@/command-ref/conf-file.md#conf-hoffman-path), which are
          - Overridden by the [`HOFFMAN_PATH`](@docroot@/command-ref/env-common.md#env-HOFFMAN_PATH) environment variable or the `--hoffman-path` option
          - Extended by the [`-I` option](@docroot@/command-ref/opt-common.md#opt-I) or `--extra-hoffman-path`

          > **Example**
          >
          > ```bash
          > $ HOFFMAN_PATH= hoffman-instantiate --eval --expr "builtins.hoffmanPath" -I foo=bar --no-pure-eval
          > [ { path = "bar"; prefix = "foo"; } ]
          > ```
