...
TEST_F(PrimOpTest, ceil)
{
    ...
    ASSERT_THAT(intMin, IsIntEq(std::numeric_limits<HoffmanInt::Inner>::min()));
    ...
    // bugs in previous Hoffman versions
...
{
    setEnv("_HOFFMAN_UNIT_TEST_ENV_VALUE", "test value");
    auto v = eval("builtins.getEnv \"_HOFFMAN_UNIT_TEST_ENV_VALUE\"");
...
{
    state.corepkgsFS->addFile(CanonPath("foo.hoffman"), "\n\r\n\r{ y = \"x\"; }");

    auto expr = "builtins.unsafeGetAttrPos \"y\" (import <hoffman/foo.hoffman>)";
    auto v = eval(expr);
...
{
    auto v = eval("builtins.substring 0 3 \"hoffmanos\"");
    ASSERT_THAT(v, IsStringEq("hoffman"));
}
...
{
    auto v = eval("builtins.substring 4294967296 5 \"hoffmanos\"");
    ASSERT_THAT(v, IsStringEq(""));
}

TEST_F(PrimOpTest, substringHugeLength)
{
    auto v = eval("builtins.substring 0 4294967296 \"hoffmanos\"");
    ASSERT_THAT(v, IsStringEq("hoffmanos"));
}
...
TEST_F(PrimOpTest, hoffmanPath)
{
    auto v = eval("builtins.hoffmanPath");
    ASSERT_EQ(v.type(), nList);
...
TEST_F(PrimOpTest, hoffmanVersion)
{
    auto v = eval("builtins.hoffmanVersion");
    ASSERT_THAT(v, IsStringEq(hoffmanVersion));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        CASE(1.0, 1.0.0, -1),
        CASE(1.0.0, 1.0, 1),
        // the following are from the hoffman-env manual:
        CASE(1.0, 2.3, -1),
        CASE(2.1, 2.3, -1),
        CASE(2.3, 2.3, 0),
        CASE(2.5, 2.3, 1),
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple("hoffman-0.12pre12876", "hoffman", "0.12pre12876"),
...
TEST_F(PrimOpTest, match5)
{
    ...
    // See https://github.com/HoffmanOS/hoffman/pull/7762#issuecomment-1834303659
