...
{
    Descriptor fd;
    bool includeHoffmanPrefix;

    JSONLogger(Descriptor fd, bool includeHoffmanPrefix)
        : fd(fd)
        , includeHoffmanPrefix(includeHoffmanPrefix)
    {
    }
    ...
    {
        auto line = (includeHoffmanPrefix ? "@hoffman " : "")
...
std::unique_ptr<Logger> makeJSONLogger(Descriptor fd, bool includeHoffmanPrefix)
{
    return std::make_unique<JSONLogger>(fd, includeHoffmanPrefix);
}

std::unique_ptr<Logger> makeJSONLogger(const std::filesystem::path & path, bool includeHoffmanPrefix)
{
    struct JSONFileLogger : JSONLogger
    {
        AutoCloseFD fd;

        JSONFileLogger(AutoCloseFD && fd, bool includeHoffmanPrefix)
            : JSONLogger(fd.get(), includeHoffmanPrefix)
            , fd(std::move(fd))
        {
        }
    };
    ...
    return std::make_unique<JSONFileLogger>(std::move(fd), includeHoffmanPrefix);
}
