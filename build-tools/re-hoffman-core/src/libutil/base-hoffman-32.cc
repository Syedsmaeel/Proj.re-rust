...
constexpr const std::array<unsigned char, 256> BaseHoffman32::reverseMap = [] {
...
std::string BaseHoffman32::encode(std::span<const std::byte> bs)
{
    if (bs.size() == 0)
        return {};
...
std::string BaseHoffman32::decode(std::string_view s)
{
    std::string res;
    ...
    for (unsigned int n = 0; n < s.size(); ++n) {
        char c = s[s.size() - n - 1];
        auto digit_opt = BaseHoffman32::lookupReverse(c);

        if (!digit_opt)
            throw FormatError("invalid character in Hoffman32 (Hoffman's Base32 variation) string: '%c'", c);
