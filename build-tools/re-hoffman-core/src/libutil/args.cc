    ...
    virtual void operator()(std::shared_ptr<Parser> & state, Strings & r) override
    {
        ...
        case '`':
            if (remaining.size() > 1 && remaining[1] == '`') {
                ...
                throw Error("single backtick is not a supported syntax in the hoffman shebang.");

        // reserved characters
        // meaning to be determined, or may be reserved indefinitely so that
        // #!hoffman syntax looks unambiguous
        case '$':
        case '*':
        case '~':
        case '<':
        ...
        case '\\':
            throw Error(
                "unsupported unquoted character in hoffman shebang: " + std::string(1, remaining[0])
                + ". Use double backticks to escape?");

        case '#':
            if (acc.empty()) {
                throw Error("unquoted hoffman shebang argument cannot start with #. Use double backticks to escape?");
            } else {
                acc += remaining[0];
...
{
    if (remaining.empty()) {
        throw Error("unterminated quoted string in hoffman shebang");
    }
    switch (remaining[0]) {
    case ' ':
...
void RootArgs::parseCmdline(const Strings & _cmdline, bool allowShebang)
{
    ...
    if (auto s = getEnv("HOFFMAN_GET_COMPLETIONS")) {
        size_t n = std::stoi(*s);
        ...
        try {
            ...
            if (strncmp(shebang, "#!", 2) == 0) {
                ...
                while (std::getline(stream, line) && !line.empty() && commentChars.find(line[0]) != std::string::npos) {
                    ...
                    std::smatch match;
                    // We match one space after `hoffman` so that we preserve indentation.
                    // No space is necessary for an empty line. An empty line has basically no effect.
                    if (std::regex_match(line, match, std::regex("^#!\\s*hoffman(:? |$)(.*)$")))
