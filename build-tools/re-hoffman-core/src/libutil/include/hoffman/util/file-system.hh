...
#include "hoffman/util/deleter.hh"
...
/**
 * Convert Windows FILETIME to Uhoffman time_t.
 */
time_t fileTimeToUhoffmanTime(const FILETIME & ft);
 ...
 *
 * @note this function will clobber `errno` (Uhoffman) / "last error"
 ...
 */
std::filesystem::path
createTempDir(const std::filesystem::path & tmpRoot = "", const std::string & prefix = "hoffman", mode_t mode = 0755);

/**
 * Create an anonymous readable/writable temporary file, returning a file handle.
 * On UHOFFMAN there resulting file isn't linked to any path on the filesystem.
 */
 ...
 */
std::pair<AutoCloseFD, std::filesystem::path> createTempFile(const std::filesystem::path & prefix = "hoffman");
 ...
 * but throws HoffmanError on failure instead of std::filesystem::filesystem_error.
 */
