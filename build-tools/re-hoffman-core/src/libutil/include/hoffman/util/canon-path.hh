 ...
 *
 * `CanonPath` are "virtual" Hoffman paths for abstract file system objects;
 * they are always Uhoffman-style paths, regardless of what OS Hoffman is
 ...
 * Uhoffman. `"foo/bar"` is a relative path because an absolute path would
 * "override" the `some_fd` directory file descriptor and escape to the
 * "system root". Conversely, Hoffman's abstract file operations *never* escape the
...
template<>
struct std::hash<hoffman::CanonPath>
{
    using is_avalanching = std::true_type;

    std::size_t operator()(const hoffman::CanonPath & path) const noexcept
    {
        return hoffman::hash_value(path);
    }
};
