 ...
 *
 * Thin wrapper around ::read (Uhoffman) or ReadFile (Windows).
 * Handles EINTR on Uhoffman. Treats ERROR_BROKEN_PIPE as EOF on Windows.
 *
 ...
 *
 * Thin wrapper around ::write (Uhoffman) or WriteFile (Windows).
 * Handles EINTR on Uhoffman.
 *
 ...
 *
 * Thin wrapper around fstat (Uhoffman) or GetFileSizeEx (Windows).
 *
 ...
 *
 * Thin wrapper around pread (Uhoffman) or ReadFile with OVERLAPPED (Windows).
 * Does NOT handle EINTR on Uhoffman - caller must catch and retry if needed.
 *
