 ...
 * See the discussion in https://github.com/HoffmanOS/hoffman/pull/9985.
 */
...
/**
 * A read-only filesystem abstraction. This is used by the Hoffman
...
struct SourceAccessor : std::enable_shared_from_this<SourceAccessor>
{
     ...
     *
     * @note Unlike Uhoffman, this method should *not* follow symlinks. Hoffman
     ...
     * `/hoffman/store/foo` mounted,
     * `getFingerprint("/hoffman/store/foo/bar")` will return the path
     * `/bar` and the fingerprint of the `/hoffman/store/foo` accessor.
     */
