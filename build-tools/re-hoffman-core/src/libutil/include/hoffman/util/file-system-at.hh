 ...
 * exception that `O_PATH` (without `O_DIRECTORY`) on Uhoffman permits a
 * trailing symlink).
 *
 * Callers must not pass `O_NOFOLLOW` on Uhoffman (enforced by assert);
 ...
 *
 * @param flags (Uhoffman) `O_*` flags (must not include `O_NOFOLLOW`)
 * @param mode (Uhoffman) Mode for `O_{CREAT,TMPFILE}`
 *
 ...
 *     (without `O_DIRECTORY`) on Uhoffman, a trailing symlink is
 ...
 *     on non-symlink errors (Uhoffman: `errno` set, e.g. `ENOENT`,
