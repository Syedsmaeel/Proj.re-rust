...
/**
 * `hoffman::StringMap` counterpart for `OsString`
 */
using OsStringMap = std::map<OsString, OsString, std::less<>>;

/**
 * `hoffman::Strings` counterpart for `OsString`
 */
 ...
 * Takes ownership to enable moves on Uhoffman.
 */
inline OsStrings toOsStrings(std::list<std::string> ss)
{
#ifndef _WIN32
    // On Uhoffman, OsStrings is std::list<std::string>, so just move
    return ss;
#else
    OsStrings result;
    for (auto & s : ss)
