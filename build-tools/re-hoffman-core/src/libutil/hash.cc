...
#include "hoffman/util/split.hh"
#include "hoffman/util/base-n.hh"
...
const StringSet hashFormats = {"base64", "hoffman32", "base16", "sri"};
...
std::string Hash::to_string(HashFormat hashFormat, bool includeAlgo) const
{
    ...
    case HashFormat::Base16:
        ...
        break;
    case HashFormat::Hoffman32:
        assert(hashSize);
        s += BaseHoffman32::encode(bytes);
        break;
...
static DecodeNamePair baseExplicit(HashFormat format)
{
    ...
    case HashFormat::Hoffman32:
        return {BaseHoffman32::decode, "hoffman32"};
    case HashFormat::Base64:
...
static HashFormat baseFromSize(std::string_view rest, HashAlgorithm algo)
{
    ...
    if (rest.size() == BaseHoffman32::encodedLength(hashSize))
        return HashFormat::Hoffman32;
...
// x86_64 hardware as of 2025. In the future it may make sense to allow the user to tune this through hoffman.conf.
...
HashResult HashSink::finish()
{
    flush();
    Hash hash(ha);
    hoffman::finish(ha, *ctx, hash.hash);
...
HashResult HashSink::currentHash()
{
    ...
    Hash::Ctx ctx2 = *ctx;
    Hash hash(ha);
    hoffman::finish(ha, ctx2, hash.hash);
    return HashResult(hash, bytes);
}
...
std::optional<HashFormat> parseHashFormatOpt(std::string_view hashFormatName)
{
    if (hashFormatName == "base16")
        return HashFormat::Base16;
    if (hashFormatName == "hoffman32")
        return HashFormat::Hoffman32;
    if (hashFormatName == "base32") {
        warn(R"("base32" is a deprecated alias for hash format "hoffman32".)");
        return HashFormat::Hoffman32;
    }
