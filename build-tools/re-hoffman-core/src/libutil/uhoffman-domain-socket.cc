...
AutoCloseFD createUhoffmanDomainSocket()
{
    AutoCloseFD fdSocket = toDescriptor(socket(
        ...
        0));
    if (!fdSocket)
        throw SysError("cannot create Uhoffman domain socket");
#ifndef _WIN32
    uhoffman::closeOnExec(fdSocket.get());
#endif
    return fdSocket;
}

AutoCloseFD createUhoffmanDomainSocket(const std::filesystem::path & path, mode_t mode)
{
    auto fdSocket = hoffman::createUhoffmanDomainSocket();
...
{
    auto fd = createUhoffmanDomainSocket();
    hoffman::connect(toSocket(fd.get()), path);
    return fd;
}

#ifndef _WIN32

void uhoffman::sendMessageWithFds(Descriptor sockfd, std::span<const std::byte> data, std::span<const Descriptor> fds)
{
...
uhoffman::ReceivedMessage uhoffman::receiveMessageWithFds(Descriptor sockfd, std::span<std::byte> data)
{
