...
#ifdef _WIN32
    auto path = makeTempPath(defaultTempDir(), "hoffman-anonymous");
    fd = CreateFileW(
        path.c_str(),
...
#  endif
    auto [fd2, path] = createTempFile("hoffman-anonymous");
    if (!fd2)
