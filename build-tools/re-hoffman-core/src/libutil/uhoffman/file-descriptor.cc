...
#include "util-uhoffman-config-private.hh"

namespace hoffman {

std::make_unsigned_t<off_t> getFileSize(Descriptor fd)
{
    auto st = hoffman::fstat(fd);
    return st.st_size;
}
...
static int uhoffman_close_range(unsigned int first, unsigned int last, int flags)
{
#  if !HAVE_CLOSE_RANGE
...
void uhoffman::closeExtraFDs()
{
    ...
    if (uhoffman_close_range(MAX_KEPT_FD + 1, ~0U, 0) == 0) {
        return;
    }
#endif
...
void uhoffman::closeOnExec(int fd)
{
    int prev;
...
void uhoffman::SelfPipe::create()
{
    pipe.create(/*nonBlocking=*/true);
}

void uhoffman::SelfPipe::notify()
{
