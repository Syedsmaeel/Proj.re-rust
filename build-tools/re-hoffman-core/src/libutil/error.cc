...
void throwExceptionSelfCheck()
{
    ...
    throw Error(
        "C++ exception handling is broken. This would appear to be a problem with the way Hoffman was compiled and/or linked and/or loaded.");
}
 ...
 *
 *     _HOFFMAN_EVAL_SHOW_UNKNOWN_LOCATIONS=1 _HOFFMAN_TEST_ACCEPT=1 make tests/lang.sh.test
 *     git diff -U20 tests
 *
 */
static bool printUnknownLocations = getEnv("_HOFFMAN_EVAL_SHOW_UNKNOWN_LOCATIONS").has_value();
    ...
    std::set<Trace> tracesSeen)
{
    if (skippedTraces.size() > 0) {
        ...
        if (skippedTraces.size() <= 5) {
            ...
            // - (9 duplicate frames omitted)
            //
            // See: `tests/functional/lang/eval-fail-mutual-recursion.hoffman`
            tracesSeen.clear();
        }
    }
