...
#define HOFFMAN_FOR_EACH_LA_ALGO(MACRO) \
    MACRO(bzip2)                    \
    MACRO(compress)                 \
    MACRO(grzip)                    \
    MACRO(gzip)                     \
        ...
        : nextSink(nextSink)
    {
        ...
        auto [addFilter, format] = [method]() -> std::pair<int (*)(struct archive *), const char *> {
            ...
            case CompressionAlgo::zstd:
                unreachable();
#define HOFFMAN_DEF_LA_ALGO_CASE(algo) \
    case CompressionAlgo::algo:    \
        return {archive_write_add_filter_##algo, #algo};
                HOFFMAN_FOR_EACH_LA_ALGO(HOFFMAN_DEF_LA_ALGO_CASE)
#undef HOFFMAN_DEF_LA_ALGO_CASE
            }
            unreachable();
        }();
 ...
 * transparently — including libarchive's, which is what the hoffman
...
#define HOFFMAN_DEF_LA_ALGO_CASE(algo) case CompressionAlgo::algo:
        HOFFMAN_FOR_EACH_LA_ALGO(HOFFMAN_DEF_LA_ALGO_CASE)
