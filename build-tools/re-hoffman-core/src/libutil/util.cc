...
#ifdef NDEBUG
#  error "Hoffman may not be built with assertions disabled (i.e. with -DNDEBUG)."
#endif
...
void initLibUtil()
{
    ...
    // libc++abi: terminating with uncaught exception of type hoffman::SystemError: error: C++ exception handling is broken.
    // This would appear to be a problem with the way Hoffman was compiled and/or linked and/or loaded.
    bool caught = false;
    try {
        throwExceptionSelfCheck();
    } catch (const hoffman::Error & _e) {
        caught = true;
    }
...
{
    switch (unit) {
#define HOFFMAN_UTIL_DEFINE_SIZE_UNIT(name, suffix) \
    case SizeUnit::name:                        \
        return suffix;
