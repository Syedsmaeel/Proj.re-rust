...
#include "hoffman/util/tests/json-characterization.hh"
...
protected:
    ...
    {
        hoffman::checkpointJson(*this, std::string{stem} + "-before", drv);
        hoffman::checkpointJson(*this, std::string{stem} + "-buildTrace", buildTrace);

        auto resolved = drv.tryResolve(*store, makeCallback(buildTrace));
        ASSERT_TRUE(resolved);

        hoffman::checkpointJson(*this, std::string{stem} + "-after", *resolved);
...
JSON_IMPL(hoffman::TryResolveTest::BuildTrace);

namespace nlohmann {

using hoffman::SingleDerivedPath;
using hoffman::StorePath;
using hoffman::TryResolveTest;
    ...
    std::string_view stem, const Derivation & drv, const StructuredAttrs * parsed)
{
    ...
    hoffman::checkpointJson(*this, std::string{stem} + "-before-options", options);
    ...
    hoffman::checkpointJson(*this, std::string{stem} + "-resolved", *resolved);
    ...
    hoffman::checkpointJson(*this, std::string{stem} + "-resolved-options", resolvedOptions);
