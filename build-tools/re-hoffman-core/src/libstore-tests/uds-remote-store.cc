...
{
    EXPECT_THROW(UDSRemoteStoreConfig("", {{"store", (std::filesystem::path{"hoffman"} / "store").string()}}), UsageError);
}
...
TEST(UDSRemoteStore, constructConfigWithParams)
{
    ...
    EXPECT_EQ(storeReference.to_string(), "uhoffman:///tmp/socket?max-connections=1");
    EXPECT_EQ(storeReference.render(/*withParams=*/false), "uhoffman:///tmp/socket");
