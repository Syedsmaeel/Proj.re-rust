...
TEST_F(hoffman_api_util_context, hoffman_libstore_init)
{
    auto ret = hoffman_libstore_init(ctx);
    ASSERT_EQ(HOFFMAN_OK, ret);
}

TEST_F(hoffman_api_store_test, hoffman_store_get_uri)
{
    std::string str;
    auto ret = hoffman_store_get_uri(ctx, store, OBSERVE_STRING(str));
    ASSERT_EQ(HOFFMAN_OK, ret);
    auto expectedStoreURI = "local://?"
                            + hoffman::encodeQuery({
                                {"log", hoffmanLogDir},
                                {"state", hoffmanStateDir},
                                {"store", hoffmanStoreDir},
                            });
    ASSERT_EQ(expectedStoreURI, str);
}

TEST_F(hoffman_api_util_context, hoffman_store_get_storedir_default)
{
    hoffman_libstore_init(ctx);
    Store * store = hoffman_store_open(ctx, nullptr, nullptr);
    assert_ctx_ok();
    ASSERT_NE(store, nullptr);

    std::string str;
    auto ret = hoffman_store_get_storedir(ctx, store, OBSERVE_STRING(str));
    assert_ctx_ok();
    ASSERT_EQ(HOFFMAN_OK, ret);

    // These tests run with a unique storeDir, but not a relocated store
    ASSERT_STREQ(HOFFMAN_STORE_DIR, str.c_str());

    hoffman_store_free(store);
}

TEST_F(hoffman_api_store_test, hoffman_store_get_storedir)
{
    std::string str;
    auto ret = hoffman_store_get_storedir(ctx, store, OBSERVE_STRING(str));
    assert_ctx_ok();
    ...
    // These tests run with a unique storeDir, but not a relocated store
    ASSERT_STREQ(hoffmanStoreDir.c_str(), str.c_str());
}

TEST_F(hoffman_api_store_test, InvalidPathFails)
{
    hoffman_store_parse_path(ctx, store, "invalid-path");
    ASSERT_EQ(hoffman_err_code(ctx), HOFFMAN_ERR_HOFFMAN_ERROR);
}

TEST_F(hoffman_api_store_test, ReturnsValidStorePath)
{
    StorePath * result = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str());
    ASSERT_NE(result, nullptr);
    ASSERT_EQ("name", result->path.name());
    ASSERT_EQ(PATH_SUFFIX.substr(1), result->path.to_string());
    hoffman_store_path_free(result);
}

TEST_F(hoffman_api_store_test, SetsLastErrCodeToHoffmanOk)
{
    StorePath * path = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str());
    ASSERT_EQ(hoffman_err_code(ctx), HOFFMAN_OK);
    hoffman_store_path_free(path);
}

TEST_F(hoffman_api_store_test, DoesNotCrashWhenContextIsNull)
{
    StorePath * path = nullptr;
    ASSERT_NO_THROW(path = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str()));
    hoffman_store_path_free(path);
}

// Verify it's 20 bytes
static_assert(sizeof(hoffman_store_path_hash_part::bytes) == 20);
static_assert(sizeof(hoffman_store_path_hash_part::bytes) == sizeof(hoffman_store_path_hash_part));

TEST_F(hoffman_api_store_test, hoffman_store_path_hash)
{
    StorePath * path = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str());
    ASSERT_NE(path, nullptr);

    hoffman_store_path_hash_part hash;
    auto ret = hoffman_store_path_hash(ctx, path, &hash);
    assert_ctx_ok();
    ASSERT_EQ(ret, HOFFMAN_OK);
...
TEST_F(hoffman_api_store_test, hoffman_store_create_from_parts_roundtrip)
{
    // Parse a path
    StorePath * original = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str());
    EXPECT_NE(original, nullptr);

    // Get its hash
    hoffman_store_path_hash_part hash;
    auto ret = hoffman_store_path_hash(ctx, original, &hash);
    assert_ctx_ok();
    ASSERT_EQ(ret, HOFFMAN_OK);

    // Get its name
    std::string name;
    hoffman_store_path_name(original, OBSERVE_STRING(name));

    // Reconstruct from parts
    StorePath * reconstructed = hoffman_store_create_from_parts(ctx, &hash, name.c_str(), name.size());
    assert_ctx_ok();
    ...
    hoffman_store_path_free(original);
    hoffman_store_path_free(reconstructed);
}

TEST_F(hoffman_api_store_test, hoffman_store_create_from_parts_invalid_name)
{
    hoffman_store_path_hash_part hash = {};
    // Invalid name with spaces
    StorePath * path = hoffman_store_create_from_parts(ctx, &hash, "invalid name", 12);
    ASSERT_EQ(path, nullptr);
    ASSERT_EQ(hoffman_err_code(ctx), HOFFMAN_ERR_HOFFMAN_ERROR);
}

TEST_F(hoffman_api_store_test, get_version)
{
    std::string str;
    auto ret = hoffman_store_get_version(ctx, store, OBSERVE_STRING(str));
    ASSERT_EQ(HOFFMAN_OK, ret);
    ASSERT_STREQ(PACKAGE_VERSION, str.c_str());
}

TEST_F(hoffman_api_util_context, hoffman_store_open_dummy)
{
    hoffman_libstore_init(ctx);
    Store * store = hoffman_store_open(ctx, "dummy://", nullptr);
    ASSERT_EQ(HOFFMAN_OK, hoffman_err_code(ctx));
    ASSERT_STREQ("dummy://", store->ptr->config.getReference().render(/*withParams=*/true).c_str());

    std::string str;
    hoffman_store_get_version(ctx, store, OBSERVE_STRING(str));
    ASSERT_STREQ("", str.c_str());

    hoffman_store_free(store);
}

TEST_F(hoffman_api_util_context, hoffman_store_open_invalid)
{
    hoffman_libstore_init(ctx);
    Store * store = hoffman_store_open(ctx, "invalid://", nullptr);
    ASSERT_EQ(HOFFMAN_ERR_HOFFMAN_ERROR, hoffman_err_code(ctx));
    ASSERT_EQ(nullptr, store);
    hoffman_store_free(store);
}

TEST_F(hoffman_api_store_test, hoffman_store_is_valid_path_not_in_store)
{
    StorePath * path = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str());
    ASSERT_EQ(false, hoffman_store_is_valid_path(ctx, store, path));
    hoffman_store_path_free(path);
}

TEST_F(hoffman_api_store_test, hoffman_store_real_path)
{
    StorePath * path = hoffman_store_parse_path(ctx, store, (hoffmanStoreDir + PATH_SUFFIX).c_str());
    std::string rp;
    auto ret = hoffman_store_real_path(ctx, store, path, OBSERVE_STRING(rp));
    assert_ctx_ok();
    ASSERT_EQ(HOFFMAN_OK, ret);
    // Assumption: we're not testing with a relocated store
    ASSERT_STREQ((hoffmanStoreDir + PATH_SUFFIX).c_str(), rp.c_str());

    hoffman_store_path_free(path);
}

TEST_F(hoffman_api_util_context, hoffman_store_real_path_relocated)
{
#ifdef _WIN32
    GTEST_SKIP() << "Wine/Windows does not support symlinks needed for local store gcroots";
#endif
    auto tmp = hoffman::createTempDir();
    ...
    Store * store = hoffman_store_open(ctx, "local", kvs);
    assert_ctx_ok();
    ASSERT_NE(store, nullptr);

    std::string hoffmanStoreDir;
    auto ret = hoffman_store_get_storedir(ctx, store, OBSERVE_STRING(hoffmanStoreDir));
    ASSERT_EQ(HOFFMAN_OK, ret);
    ASSERT_STREQ(HOFFMAN_STORE_DIR, hoffmanStoreDir.c_str());
    ...
    std::string rp;
    ret = hoffman_store_real_path(ctx, store, path, OBSERVE_STRING(rp));
    assert_ctx_ok();
    ASSERT_EQ(HOFFMAN_OK, ret);

    // Assumption: we're not testing with a relocated store
    ASSERT_STREQ((storeRoot + HOFFMAN_STORE_DIR + PATH_SUFFIX).c_str(), rp.c_str());

    hoffman_store_path_free(path);
}

TEST_F(hoffman_api_util_context, hoffman_store_real_path_binary_cache)
{
    auto tmpDir = hoffman::createTempDir() / "binary-cache";
    auto url = hoffman::ParsedURL{.scheme = "file", .path = hoffman::pathToUrlPath(tmpDir)};
    Store * store = hoffman_store_open(ctx, url.to_string().c_str(), nullptr);
    assert_ctx_ok();
    ASSERT_NE(store, nullptr);

    std::string path_raw = std::string(HOFFMAN_STORE_DIR) + PATH_SUFFIX;
    StorePath * path = hoffman_store_parse_path(ctx, store, path_raw.c_str());
    assert_ctx_ok();
    ASSERT_NE(path, nullptr);
...
class HoffmanApiStoreTestWithRealisedPath : public hoffman_api_store_test_base
{
public:
    std::optional<hoffman::EnableExperimentalFeature> enableCA;
    StorePath * drvPath = nullptr;
    hoffman_derivation * drv = nullptr;
    Store * store = nullptr;
    StorePath * outPath = nullptr;

    void SetUp() override
    {
        hoffman_api_store_test_base::SetUp();
#ifdef _WIN32
        GTEST_SKIP() << "Wine does not support symlinks needed for local store gcroots";
#endif

        enableCA.emplace("ca-derivations");
        hoffman::settings.getWorkerSettings().substituters = {};

        store = open_local_store();

        std::filesystem::path unitTestData = hoffman::getUnitTestData();
        std::ifstream t{unitTestData / "derivation/ca/self-contained.json"};
        std::stringstream buffer;
        buffer << t.rdbuf();

        // Replace the hardcoded system with the current system
        std::string jsonStr = hoffman::replaceStrings(buffer.str(), "x86_64-linux", hoffman::settings.thisSystem.get());

        drv = hoffman_derivation_from_json(ctx, store, jsonStr.c_str());
        assert_ctx_ok();
        ASSERT_NE(drv, nullptr);

        drvPath = hoffman_add_derivation(ctx, store, drv);
        assert_ctx_ok();
        ...
        auto cb = LambdaAdapter{.fun = [&](const char * outname, const StorePath * outPath_) {
            ...
            auto is_valid_path = hoffman_store_is_valid_path(ctx, store, outPath_);
            ...
            outPath = hoffman_store_path_clone(outPath_);
        }};

        auto ret = hoffman_store_realise(
    ...
    {
        if (drvPath)
            hoffman_store_path_free(drvPath);
        if (outPath)
            hoffman_store_path_free(outPath);
        if (drv)
            hoffman_derivation_free(drv);
        if (store)
            hoffman_store_free(store);

        hoffman_api_store_test_base::TearDown();
    }
};

TEST_F(hoffman_api_store_test_base, build_from_json)
{
#ifdef _WIN32
    GTEST_SKIP() << "Wine does not support symlinks needed for local store gcroots";
#endif
    // FIXME get rid of these
    hoffman::EnableExperimentalFeature enableCA{"ca-derivations"};
    hoffman::settings.getWorkerSettings().substituters = {};
    ...
    auto * drv = hoffman_derivation_from_json(ctx, store, jsonStr.c_str());
    assert_ctx_ok();
    ASSERT_NE(drv, nullptr);

    auto * drvPath = hoffman_add_derivation(ctx, store, drv);
    assert_ctx_ok();
    ASSERT_NE(drv, nullptr);
    ...
    auto cb = LambdaAdapter{.fun = [&](const char * outname, const StorePath * outPath) {
        ...
        auto is_valid_path = hoffman_store_is_valid_path(ctx, store, outPath);
        ASSERT_EQ(is_valid_path, true);
        callbackCount++;
    }};
...
TEST_F(hoffman_api_store_test_base, hoffman_store_realise_invalid_system)
{
#ifdef _WIN32
    GTEST_SKIP() << "Wine does not support symlinks needed for local store gcroots";
#endif
    // Test that hoffman_store_realise properly reports errors when the system is invalid
    hoffman::EnableExperimentalFeature enableCA{"ca-derivations"};
    hoffman::settings.getWorkerSettings().substituters = {};
    ...
    std::string jsonStr = hoffman::replaceStrings(buffer.str(), "x86_64-linux", "bogus65-bogusos");
...
TEST_F(hoffman_api_store_test_base, hoffman_store_realise_builder_fails)
{
#ifdef _WIN32
    GTEST_SKIP() << "Wine does not support symlinks needed for local store gcroots";
#endif
    // Test that hoffman_store_realise properly reports errors when the builder fails
    hoffman::EnableExperimentalFeature enableCA{"ca-derivations"};
    hoffman::settings.getWorkerSettings().substituters = {};
    ...
    std::string jsonStr = hoffman::replaceStrings(buffer.str(), "x86_64-linux", hoffman::settings.thisSystem.get());
    jsonStr = hoffman::replaceStrings(jsonStr, "echo $name foo > $out", "exit 1");
...
TEST_F(hoffman_api_store_test_base, hoffman_store_realise_builder_no_output)
{
#ifdef _WIN32
    GTEST_SKIP() << "Wine does not support symlinks needed for local store gcroots";
#endif
    // Test that hoffman_store_realise properly reports errors when builder succeeds but produces no output
    hoffman::EnableExperimentalFeature enableCA{"ca-derivations"};
    hoffman::settings.getWorkerSettings().substituters = {};
    ...
    std::string jsonStr = hoffman::replaceStrings(buffer.str(), "x86_64-linux", hoffman::settings.thisSystem.get());
    jsonStr = hoffman::replaceStrings(jsonStr, "echo $name foo > $out", "true");
...
TEST_F(HoffmanApiStoreTestWithRealisedPath, hoffman_store_get_fs_closure_with_outputs)
{
    ...
    auto ret = hoffman_store_get_fs_closure(
        ctx,
        store,
        ...
        &data,
        [](hoffman_c_context * context, void * userdata, const StorePath * path) {
            auto * data = static_cast<CallbackData *>(userdata);
            std::string path_str;
            hoffman_store_path_name(path, OBSERVE_STRING(path_str));
    ...
    hoffman_store_path_name(outPath, OBSERVE_STRING(outPathName));
    ASSERT_EQ(closure_paths.count(outPathName), 1);
}

TEST_F(HoffmanApiStoreTestWithRealisedPath, hoffman_store_get_fs_closure_without_outputs)
{
...
TEST_F(HoffmanApiStoreTestWithRealisedPath, hoffman_store_get_fs_closure_flip_direction)
{
...
TEST_F(HoffmanApiStoreTestWithRealisedPath, hoffman_store_get_fs_closure_include_derivers)
{
    ...
    hoffman_store_path_name(drvPath, OBSERVE_STRING(drvPathName));
    ASSERT_EQ(closure_paths.count(drvPathName), 1) << "Derivation should be in closure when include_derivers=true";
}

TEST_F(HoffmanApiStoreTestWithRealisedPath, hoffman_store_realise_output_ordering)
{
    // Test that hoffman_store_realise returns outputs in alphabetical order by output name.
    ...
    auto outa_ph = hoffman::hashPlaceholder("outa");
    auto outb_ph = hoffman::hashPlaceholder("outb");
    auto outc_ph = hoffman::hashPlaceholder("outc");
    auto outd_ph = hoffman::hashPlaceholder("outd");
    auto oute_ph = hoffman::hashPlaceholder("oute");
    auto outf_ph = hoffman::hashPlaceholder("outf");
    auto outg_ph = hoffman::hashPlaceholder("outg");
    auto outh_ph = hoffman::hashPlaceholder("outh");
    auto outi_ph = hoffman::hashPlaceholder("outi");
    auto outj_ph = hoffman::hashPlaceholder("outj");

    std::string drvJson = R"({
        "version": 4,
        "name": "multi-output-test",
        "system": ")" + hoffman::settings.thisSystem.get()
                          + R"(",
        "builder": "/bin/sh",
    ...
    auto * drv = hoffman_derivation_from_json(ctx, store, drvJson.c_str());
    assert_ctx_ok();
    ASSERT_NE(drv, nullptr);
    ...
    std::map<std::string, hoffman::StorePath> outputs;
    ...
    ret = hoffman_store_get_fs_closure(
        ctx,
        store,
        drvPath,
        false, // flip_direction
    ...
    for (const auto & [outname, outPath] : outputs) {
        ...
        for (const auto & p : closure_paths) {
            // hoffman_store_path_name returns just the name part, so match against full path name
...
TEST_F(HoffmanApiStoreTestWithRealisedPath, hoffman_store_get_fs_closure_error_propagation)
{
    ...
    auto ret = hoffman_store_get_fs_closure(
        ...
        [](hoffman_c_context * context, void * userdata, const StorePath * path) {
            ...
            hoffman_set_err_msg(context, HOFFMAN_ERR_UNKNOWN, "Test error");
        });

    // Should have aborted with error
    ASSERT_EQ(ret, HOFFMAN_ERR_UNKNOWN);
 ...
 *
 * @param filename Relative path from _HOFFMAN_TEST_UNIT_DATA
...
TEST_F(hoffman_api_store_test, hoffman_derivation_to_json_roundtrip)
{
    ...
    // Parse to derivation
    auto * drv = hoffman_derivation_from_json(ctx, store, originalJson.c_str());
    assert_ctx_ok();
    ASSERT_NE(drv, nullptr);

    // Convert back to JSON
    std::string convertedJson;
    auto ret = hoffman_derivation_to_json(ctx, drv, OBSERVE_STRING(convertedJson));
    assert_ctx_ok();
    ASSERT_EQ(ret, HOFFMAN_OK);
...
TEST_F(hoffman_api_store_test, hoffman_derivation_store_round_trip)
{
    ...
    auto * drv = hoffman_derivation_from_json(ctx, store, json.c_str());
    assert_ctx_ok();
    ASSERT_NE(drv, nullptr);
    ...
    // Retrieve from store
    auto * drv2 = hoffman_store_drv_from_store_path(ctx, store, drvPath);
    assert_ctx_ok();
    ...
    hoffman_store_path_free(drvPath);
    hoffman_derivation_free(drv);
    hoffman_derivation_free(drv2);
}

TEST_F(hoffman_api_store_test, hoffman_derivation_clone)
{
    // Load a derivation from JSON
    auto json = load_json_from_test_data("derivation/invariants/filled-in-deferred-empty-env-var-pre.json");
    auto * drv = hoffman_derivation_from_json(ctx, store, json.c_str());
    ...
    auto * drv2 = hoffman_derivation_clone(drv);
    ASSERT_NE(drv2, nullptr);
