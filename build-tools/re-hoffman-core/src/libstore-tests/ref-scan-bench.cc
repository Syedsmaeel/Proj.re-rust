...
{
    auto dist = std::uniform_int_distribution<std::size_t>(0, BaseHoffman32::characters.size() - 1);
    dist(urng);
    std::generate_n(outIter, StorePath::HashLen, [&]() { return BaseHoffman32::characters[dist(urng)]; });
}
