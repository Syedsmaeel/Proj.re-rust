...
#include "hoffman/store/nar-info.hh"
...
{
    auto cacheInfo = getHoffmanCacheInfo();
    if (!cacheInfo) {
        upsertFile(cacheInfoFile, "StoreDir: " + storeDir + "\n", "text/x-hoffman-cache-info");
    } else {
        for (auto & line : tokenizeString<Strings>(*cacheInfo, "\n")) {
            ...
            if (name == "StoreDir") {
                if (value != storeDir)
                    throw Error(
                        "binary cache '%s' is for Hoffman stores with prefix '%s', not '%s'",
...
std::optional<std::string> BinaryCacheStore::getHoffmanCacheInfo()
{
...
void BinaryCacheStore::writeNarInfo(ref<NarInfo> narInfo)
{
    ...
    upsertFile(narInfoFile, narInfo->to_string(*this), "text/x-hoffman-narinfo");
    ...
    Source & narSource, RepairFlag repair, CheckSigsFlag checkSigs, fun<ValidPathInfo(HashResult)> mkInfo)
{
    ...
    narInfo->url = "nar/" + narInfo->fileHash->to_string(HashFormat::Hoffman32, false) + ".nar"
    ...
    if (repair || !fileExists(narInfo->url)) {
        ...
        stats.narWrite++;
        upsertFile(narInfo->url, source, "application/x-hoffman-nar", narInfo->fileSize);
    } else
    ...
    RepairFlag repair)
{
    ...
    if (auto * dump2p = dynamic_cast<StringSource *>(&dump)) {
        ...
        switch (dumpMethod) {
        case FileSerialisationMethod::HoffmanArchive:
