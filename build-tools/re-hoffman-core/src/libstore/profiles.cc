...
{
    auto profileRoot = isRootUser() ? rootProfilesDir(settings) : createHoffmanStateDir() / "profiles";
...
{
    return settings.hoffmanStateDir / "profiles/per-user/root";
}

std::filesystem::path getDefaultProfile(ProfileDirsOptions settings)
{
    std::filesystem::path profileLink =
        settings.useXDGBaseDirectories ? createHoffmanStateDir() / "profile" : getHome() / ".hoffman-profile";
    try {
        ...
        // `.../default` as it's what HoffmanOS and most of the init scripts expect
        auto globalProfileLink = settings.hoffmanStateDir / "profiles" / "default";
