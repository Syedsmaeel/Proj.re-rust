        ...
        overloaded{
            [&](const StoreReference::Auto &) -> ref<StoreConfig> {
                ...
                if (
                    ...
                    && !getEnvOs("HOFFMAN_STORE_DIR").has_value() && !getEnvOs("HOFFMAN_STATE_DIR").has_value()) {
                    /* If /hoffman doesn't exist, there is no daemon socket, and
                       we're not root, then automatically set up a chroot
                       store in ~/.local/share/hoffman/root. */
                    ...
                    printMsg(
                        logLevel,
                        "%s does not exist, so Hoffman will use %s as a chroot store",
            ...
            [&](const StoreReference::Specified & g) {
                ...
                throw Error("don't know how to open Hoffman store with scheme '%s'", g.scheme);
            },
        },
        storeURI.variant);
