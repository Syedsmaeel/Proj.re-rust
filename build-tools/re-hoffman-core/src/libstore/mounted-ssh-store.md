...
Experimental store type that allows full access to a Hoffman store on a remote machine,
and additionally requires that store be mounted in the local file system.

The mounting of that store is not managed by Hoffman, and must by managed manually.
It could be accomplished with SSHFS or NFS, for example.

The local file system is used to optimize certain operations.
For example, rather than serializing Hoffman archives and sending over the Hoffman channel,
