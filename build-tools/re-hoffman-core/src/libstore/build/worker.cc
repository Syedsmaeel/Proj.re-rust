...
#include "hoffman/store/build/substitution-goal.hh"
#include "hoffman/store/build/drv-output-substitution-goal.hh"
#include "hoffman/store/build/derivation-goal.hh"
#include "hoffman/store/build/derivation-resolution-goal.hh"
#include "hoffman/store/build/derivation-building-goal.hh"
#include "hoffman/store/build/derivation-trampoline-goal.hh"
...
Worker::Worker(Store & store, Store & evalStore)
    ...
    , store(store)
    , evalStore(evalStore)
    , settings(hoffman::settings.getWorkerSettings())
    , getSubstituters{[] {
        return hoffman::settings.getWorkerSettings().useSubstitutes ? getDefaultSubstituters() : std::list<ref<Store>>{};
    }}
{
#ifdef _WIN32
    if (!ioport)
...
void Worker::run(const Goals & _topGoals)
{
    ...
    while (1) {
        ...
        if (!children.empty() || !waitingForAWhile.empty() || !waitingForCompletion.empty())
            ...
            if (Machine::parseConfig({hoffman::settings.thisSystem}, hoffman::settings.getWorkerSettings().builders).empty())
                throw Error(
                    "Unable to start any build; either increase '--max-jobs' or enable remote builds.\n"
                    "\n"
                    "For more information run 'man hoffman.conf' and search for '/machines'.");
            else
                throw Error(
...
bool Worker::pathContentsGood(const StorePath & path)
{
    ...
    if (auto accessor = store.getFSAccessor(path, /*requireValidPath=*/false)) {
        auto current = hashPath({ref{accessor}}, FileIngestionMethod::HoffmanArchive, info->narHash.algo).first;
