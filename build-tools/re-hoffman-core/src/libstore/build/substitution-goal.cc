...
Goal::Co PathSubstitutionGoal::init()
{
    ...
    if (worker.store.config.getReadOnly())
        throw Error(
            "cannot substitute path '%s' - no write access to the Hoffman store", worker.store.printStorePath(storePath));
...
Goal::Co PathSubstitutionGoal::tryToRun(
    StorePath subPath, hoffman::ref<Store> sub, std::shared_ptr<const ValidPathInfo> info, bool & substituterFailed)
{
