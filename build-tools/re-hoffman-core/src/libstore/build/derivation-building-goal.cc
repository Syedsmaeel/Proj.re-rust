#include "hoffman/store/build/derivation-building-goal.hh"
#include "hoffman/store/build/derivation-env-desugar.hh"
#ifndef _WIN32 // TODO enable build hook on Windows
#  include "hoffman/store/build/hook-instance.hh"
...
#include "hoffman/store/build/worker.hh"
#include "hoffman/util/util.hh"
#include "hoffman/util/compression.hh"
#include "hoffman/store/common-protocol.hh"
#include "hoffman/store/common-protocol-impl.hh"
#include "hoffman/store/local-store.hh" // TODO remove, along with remaining downcasts
...
static BuildError reject(const LocalBuildRejection & rejection, std::string_view thingCannotBuild)
{
    if (std::get_if<LocalBuildRejection::NoLocalStore>(&rejection.rejection))
        return BuildError(
            ...
            "For more information check 'man hoffman.conf' and search for '/machines'.");
        ...
        if (wrongStore.badPlatform && wrongStore.badPlatform->derivation == "x86_64-darwin"
            ...
            msg +=
                fmt("\nNote: run `%s` to run programs for x86_64-darwin",
                    Magenta("/usr/sbin/softwareupdate --install-rosetta && launchctl stop org.hoffmanos.hoffman-daemon"));
    }
    ...
    PathLocks outputLocks)
{
                ...
                for (auto c : data)
                    if (c == '\n') {
                        ...
                        if (json) {
                            ...
                            // are also available to `hoffman log`.
                            if (s && logFile->sink) {
                                ...
                                if (type == resBuildLogLine) {
                                    ...
                                    if (!phase.is_null()) {
                                        // hoffmanpkgs' stdenv produces lines in the log to signal
                                        // phase changes.
                                        // We want to get the same lines in case of remote builds.
                                        // The format is:
                                        //   @hoffman { "action": "setPhase", "phase": "$curPhase" }
                                        const auto logLine =
    ...
    auto builtOutputs =
        /* When using a build hook, the build hook can register the output
           as valid (by doing `hoffman-store --import').  If so we don't have
           to do anything here.
    ...
    const StorePathSet & outputPaths)
{
    ...
    hookEnvironment.emplace(OS_STR("HOFFMAN_CONFIG"), string_to_os_string(globalConfig.toKeyValue()));
...
BuildError DerivationBuildingGoal::fixupBuilderFailureErrorMessage(BuilderFailureError e, BuildLog & buildLog)
{
    ...
    if (!logger->isVerbose() && !logTail.empty()) {
        ...
        auto hoffmanLogCommand = experimentalFeatureSettings.isEnabled(Xp::HoffmanCommand) ? "hoffman log" : "hoffman-store -l";
