...
#include "hoffman/store/remote-fs-accessor.hh"
...
void RemoteStore::initConnection(Connection & conn)
{
    ...
    try {
        conn.from.endOfFileError = "Hoffman daemon disconnected unexpectedly (maybe it crashed?)";
        ...
        try {
            ...
            if (conn.protoVersion.number < WorkerProto::minimum.number)
                throw Error("the Hoffman daemon version is too old");
...
void RemoteStore::setOptions(Connection & conn)
{
    ...
    std::map<std::string, hoffman::Config::SettingInfo> overrides;
    ...
    RepairFlag repair)
{
    ...
    if (conn->protoVersion >= WorkerProto::Version{.number = {1, 25}}) {
        ...
        if (repair)
            throw Error("repairing is not supported when building through the Hoffman daemon protocol < 1.25");
        ...
        case ContentAddressMethod::Raw::HoffmanArchive:
        case ContentAddressMethod::Raw::Git:
        default: {
            auto fim = caMethod.getFileIngestionMethod();
            conn->to << WorkerProto::Op::AddToStore << name
                     << ((hashAlgo == HashAlgorithm::SHA256 && fim == FileIngestionMethod::HoffmanArchive)
                             ? 0
                             : 1) /* backwards compatibility hack */
                     << (fim == FileIngestionMethod::HoffmanArchive ? 1 : 0) << printHashAlgo(hashAlgo);
            ...
            try {
                ...
                {
                    Finally cleanup([&]() { connections->decCapacity(); });
                    if (fim == FileIngestionMethod::HoffmanArchive) {
...
{
    auto conn(getConnection());
    return conn->daemonHoffmanVersion;
}
