...
OsStrings getHoffmanSshOpts()
{
    std::string sshOpts = getEnv("HOFFMAN_SSHOPTS").value_or("");

    try {
        return toOsStrings(shellSplitString(sshOpts));
    } catch (Error & e) {
        e.addTrace({}, "while splitting HOFFMAN_SSHOPTS '%s'", sshOpts);
        throw;
    }
}
...
SSHMaster::SSHMaster(
    ...
    , compress(compress)
    , logFD(logFD)
    , tmpDir(make_ref<AutoDelete>(createTempDir("", "hoffman", 0700)))
{
...
{
    auto sshArgs = getHoffmanSshOpts();
...
Strings createSSHEnv()
{
    ...
    // solved; refer to the development history of hoffmanExePath in libstore/globals.cc.
...
std::unique_ptr<SSHMaster::Connection> SSHMaster::startCommand(OsStrings && command, OsStrings && extraSshArgs)
{
    ...
    conn->sshPid = startProcess(
        [&]() {
            ...
            hoffman::execvpe(args.begin()->c_str(), stringsToCharPtrs(args).data(), stringsToCharPtrs(env).data());
