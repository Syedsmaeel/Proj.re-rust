...
#include "hoffman/util/lru-cache.hh"
#include "hoffman/util/sync.hh"
#include "hoffman/util/configuration.hh"
#include "hoffman/store/path-info.hh"
...
protected:
     ...
     * `LocalFSStore`, and `Uhoffman` is appropriate for any store which is *not* a
     * `LocalFSStore`.
     *
    ...
    enum struct FilePathType {
        /**
         * Uhoffman-style path (e.g., `/hoffman/store`)
         *
         * Uhoffman paths use forward slashes, and do not have any root "drive" or
         * other such notion.
         *
         ...
         * `HOFFMAN_STORE_DIR`, which is probably `/hoffman/store`.
         *
         * @todo At some point we will have to adjust this further, to support
         * the case of Uhoffman SSHing to Windows.
         */
        Uhoffman,

        /**
         * Native path for the current platform. On Uhoffman this is a Uhoffman Path,
         ...
         *
         * - On Uhoffman, is based on a hard-coded constant, `HOFFMAN_STORE_DIR`, which
         * is probably `/hoffman/store`.
         *
         * - On Windows, it is based on `%PROGRAMDATA%\hoffman\store`.
         *
...
public:
    ...
    /**
     * A setting for the Hoffman store directory. Automatically canonicalises the
     * path and rejects the empty string. Stored as `std::string` because
     * store directory are valid file paths on *some* OS, but not neccessarily the OS of this build of Hoffman.
     *
    ...
    class StoreDirSetting : public BaseSetting<std::string>
    {
        ...
        /**
         * Compute the default Hoffman store directory from environment variables
         * (`HOFFMAN_STORE_DIR`, `HOFFMAN_STORE`) or the compile-time default.
         *
         * @param pathType Whether to return a Uhoffman-style or native path. Different
...
public:
     ...
     *
     * By default, returns the global `settings.hoffmanStateDir`. Subclasses
...
public:
    ...
    /**
     * Follow symlinks until we end up with a path in the Hoffman store.
     */
...
public:
     ...
     * implements hoffman-copy-closure's --use-substitutes
     * flag.
     */
     ...
     * (i.e. you'll get `/hoffman/store/<hash>-x` rather than
     * `/hoffman/store/<hash>-<name>`). Use queryPathInfo() to obtain the
...
public:
     ...
     * `FileSerialisationMethod::HoffmanArchive`.
     *
    ...
    virtual StorePath addToStoreFromDump(
        Source & dump,
        std::string_view name,
        FileSerialisationMethod dumpMethod = FileSerialisationMethod::HoffmanArchive,
        ContentAddressMethod hashMethod = ContentAddressMethod::Raw::HoffmanArchive,
        HashAlgorithm hashAlgo = HashAlgorithm::SHA256,
        const StorePathSet & references = StorePathSet(),
     ...
     * can be loaded into the database using `hoffman-store --load-db` or
     * `hoffman-store --register-validity`.
     */
    std::string makeValidityRegistration(const StorePathSet & paths, bool showDerivers, bool showHash);

    /**
     * Optimise the disk space usage of the Hoffman store by hard-linking files
     * with the same contents.
     */
    virtual void optimiseStore() {};

    /**
     * Check the integrity of the Hoffman store.
     *
    ...
    /**
     * @return An object to access files in the Hoffman store, across all
     * store objects.
     */
    ...
    /**
     * Write a derivation to the Hoffman store, and return its path.
     */
