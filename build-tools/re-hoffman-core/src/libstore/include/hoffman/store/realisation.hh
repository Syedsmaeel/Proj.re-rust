...
#include "hoffman/util/std-hash.hh"
...
template<>
struct std::hash<hoffman::DrvOutput>
{
    std::size_t operator()(const hoffman::DrvOutput & id) const noexcept
    {
        std::size_t h = 0;
        hoffman::hash_combine(h, id.drvPath, id.outputName);
        return h;
    }
};
...
JSON_IMPL(hoffman::DrvOutput)
JSON_IMPL(hoffman::UnkeyedRealisation)
JSON_IMPL(hoffman::Realisation)
