...
public:
    ...
    /* Do not set this too low. On glibc, getaddrinfo() contains fallback code
       ...
       in resolv.conf(5). Also see https://github.com/HoffmanOS/hoffman/pull/13985 for
    ...
    Setting<unsigned long> stalledDownloadTimeout{
        ...
        R"(
          ...
          during download. Hoffman cancels idle downloads after this
          timeout's duration.
        )"};
    ...
    Setting<uint32_t> tries{
        ...
        R"(
          The number of times Hoffman attempts a file transfer (download
    ...
    Setting<uint32_t> retryDelayRateLimitedMs{
        ...
        R"(
          ...
          when present, Hoffman respects the larger of the two values.
        )"};
    ...
    Setting<size_t> downloadBufferSize{
        ...
        R"(
          The size of Hoffman's internal download buffer in bytes during `curl` transfers. If data is
    ...
    Setting<unsigned int> downloadSpeed{
        ...
        R"(
          ...
          Hoffman to use for downloads.
        )"};

    Setting<AbsolutePath> netrcFile{
        this,
        hoffmanConfDir() / "netrc",
        "netrc-file",
        R"(
          If set to an absolute path to a `netrc` file, Hoffman uses the HTTP
          authentication credentials in this file when trying to download from
          a remote host through HTTP or HTTPS. Defaults to
          `$HOFFMAN_CONF_DIR/netrc`.
    ...
    Setting<std::optional<AbsolutePath>> caFile{
        ...
        R"(
          ...
          authenticate `https://` downloads. Hoffman by default uses
          the first of the following files that exists:

          1. `/etc/ssl/certs/ca-certificates.crt`
          2. `/hoffman/var/hoffman/profiles/default/etc/ssl/certs/ca-bundle.crt`

          The path can be overridden by the following environment
          variables, in order of precedence:

          1. `HOFFMAN_SSL_CERT_FILE`
          2. `SSL_CERT_FILE`
        )",
        {},
