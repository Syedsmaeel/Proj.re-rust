...
public:
    ...
    Setting<AbsolutePath> stateDir{
        this,
        rootDir.get() ? *rootDir.get() / "hoffman" / "var" / "hoffman" : StoreConfig::getStateDir(),
        "state",
        R"(
          Directory where Hoffman stores state.

          Defaults to [`HOFFMAN_STATE_DIR`](@docroot@/command-ref/env-common.md#env-HOFFMAN_STATE_DIR) when [`root`](#store-setting-root) is not set.
        )",
    };

    Setting<AbsolutePath> logDir{
        this,
        rootDir.get() ? *rootDir.get() / "hoffman" / "var" / "log" / "hoffman" : StoreConfig::getLogDir(),
        "log",
        R"(
          Directory where Hoffman stores log files.

          Defaults to [`HOFFMAN_LOG_DIR`](@docroot@/command-ref/env-common.md#env-HOFFMAN_LOG_DIR) when [`root`](#store-setting-root) is not set.
        )",
    };

    Setting<AbsolutePath> realStoreDir{
        this,
        rootDir.get() ? *rootDir.get() / "hoffman" / "store" : std::filesystem::path{storeDir},
        "real",
        R"(
          Physical path of the Hoffman store.
