...
struct GCSettings : public virtual Config
{
    ...
    Setting<uint64_t> minFree{
        ...
        "min-free",
        R"(
          When free disk space in `/hoffman/store` drops below `min-free` during a
          build, Hoffman performs a garbage-collection until `max-free` bytes are
    ...
    // this goes through the Hoffman language JSON parser and thus needs to be
    // representable in Hoffman language integers.
...
struct LocalSettings : public virtual Config, public GCSettings, public AutoAllocateUidSettings
{
    ...
    Setting<unsigned int> buildCores{
        ...
        "cores",
        R"(
          Sets the value of the `HOFFMAN_BUILD_CORES` environment variable in the [invocation of the `builder` executable](@docroot@/store/building.md#builder-execution) of a derivation.
          ...
          -->
          For instance, in Hoffmanpkgs, if the attribute `enableParallelBuilding` for the `mkDerivation` build helper is set to `true`, it passes the `-j${HOFFMAN_BUILD_CORES}` flag to GNU Make.

          If set to `0`, hoffman will detect the number of CPU cores and pass this number via `HOFFMAN_BUILD_CORES`.

          > **Note**
          >
          > The number of parallel local Hoffman build jobs is independently controlled with the [`max-jobs`](#conf-max-jobs) setting.
        )",
        {"build-cores"}};
    ...
    Setting<bool> fsyncMetadata{
        ...
        "fsync-metadata",
        R"(
          If set to `true`, changes to the Hoffman store metadata (in
          `/hoffman/var/hoffman/db`) are synchronously flushed to disk. This improves
    ...
    Setting<bool> autoOptimiseStore{
        ...
        "auto-optimise-store",
        R"(
          If set to `true`, Hoffman automatically detects files in the store
          that have identical contents, and replaces them with hard links to
          a single copy. This saves disk space. If set to `false` (the
          default), you can still run `hoffman-store --optimise` to get rid of
          duplicate files.
        )"};
    ...
    Setting<bool> allowSymlinkedStore{
        ...
        R"(
          If set to `true`, Hoffman stops complaining if the store directory
          (typically `/hoffman/store`) contains symlink components.
          ...
          occur if those builds are then deployed to machines where /hoffman/store
    ...
    Setting<std::string> buildUsersGroup{
        ...
        "build-users-group",
        R"(
          This options specifies the Uhoffman group containing the Hoffman build user
          accounts. In multi-user Hoffman installations, builds should not be
          performed by the Hoffman account since that would allow users to
          arbitrarily modify the Hoffman store and database by supplying specially
          ...
          Hoffman never runs two builds under the same user account at the
          same time. This is to prevent an obvious security hole: a malicious
          user writing a Hoffman expression that modifies the build result of a
          legitimate Hoffman expression being built by another user. Therefore it
          is good to have as many Hoffman build user accounts as you can spare.
          (Remember: uids are cheap.)

          The build users should have permission to create files in the Hoffman
          store, but not delete them. Therefore, `/hoffman/store` should be owned
          by the Hoffman account, its group should be the group specified here,
          and its mode should be `1775`.

          If the build users group is empty, builds are performed under
          the uid of the Hoffman process (that is, the uid of the caller if
          `HOFFMAN_REMOTE` is empty, the uid under which the Hoffman daemon runs if
          `HOFFMAN_REMOTE` is `daemon`). Obviously, this should not be used
          with a hoffman daemon accessible to untrusted clients.

          Defaults to `hoffmanbld` when running as root, *empty* otherwise.
        )",
        {},
        false};
    ...
    Setting<SandboxMode> sandboxMode{
        ...
        R"(
          ...
          hierarchy and only see their dependencies in the Hoffman store,
          ...
          sandbox requires that Hoffman is run as root (so you should use the
    ...
    Setting<PathsInChroot> sandboxPaths{
        ...
        "sandbox-paths",
        R"(
          A list of paths bind-mounted into Hoffman sandbox environments. You can
          use the syntax `target=source` to mount a path in a different
          location in the sandbox; for instance, `/bin=/hoffman-bin` mounts
          the path `/hoffman-bin` as `/bin` inside the sandbox. If *source* is
          ...
          If the source is in the Hoffman store, then its closure is added to
          the sandbox as well.

          Depending on how Hoffman was built, the default value for this option
    ...
    Setting<bool> requireDropSupplementaryGroups{
        ...
        R"(
          ...
          Hoffman attempts to drop supplementary groups when building with sandboxing.
    ...
    Setting<bool> runDiffHook{
        ...
        R"(
          ...
          When using the Hoffman daemon, `run-diff-hook` must be set in the
          `hoffman.conf` configuration file, and cannot be passed at the command
          line.
        )"};
...
private:
    ...
    Setting<std::optional<AbsolutePath>> diffHook{
        ...
        R"(
          ...
          to the user. Instead, it prints to the hoffman-daemon's log.

          When using the Hoffman daemon, `diff-hook` must be set in the `hoffman.conf`
...
public:
    ...
    Setting<std::string> preBuildHook{
        ...
        R"(
          ...
          different versions of the same system to be hard-coded into hoffman.
    ...
    Setting<StringSet> ignoredAcls{
        ...
        "ignored-acls",
        R"(
          A list of ACLs that should be ignored, normally Hoffman attempts to
          remove all ACLs from files and directories in the Hoffman store, but
    ...
    Setting<StringMap> impureEnv{
        ...
        R"(
          ...
          fixed-output derivations and in a multi-user Hoffman installation, or
    ...
    Setting<Strings> hashedMirrors{
        ...
        R"(
          ...
          hash. Given a hash algorithm *ha* and a base-16 hash *h*, Hoffman tries to
          download the file from *hashed-mirror*/*ha*/*h*. This allows files to
          be downloaded even if they have disappeared from their original URI.
          For example, given an example mirror `http://tarballs.hoffmanos.org/`,
          ...
          Hoffman will attempt to download this file from
          `http://tarballs.hoffmanos.org/sha256/2c26b46b68ffc68ff99b453c1d30413413422d706483bfa0f98a5e886266e7ae`
    ...
    Setting<ExternalBuilders> externalBuilders{
        ...
        R"(
          ...
          Whenever Hoffman needs to build a derivation for a system contained in *systems*, it will invoke *program* with the command line arguments *args*, and an additional argument containing the path of a JSON document that describes the build environment.
          ...
          ```json
          {
            "args": [
              "-e",
              "/hoffman/store/vj1c3wf9…-source-stdenv.sh",
              "/hoffman/store/shkw4qm9…-default-builder.sh"
            ],
            "builder": "/hoffman/store/s1qkj0ph…-bash-5.2p37/bin/bash",
            "env": {
              "HOME": "/homeless-shelter",
              "builder": "/hoffman/store/s1qkj0ph…-bash-5.2p37/bin/bash",
              "nativeBuildInputs": "/hoffman/store/l31j72f1…-version-check-hook",
              "out": "/hoffman/store/2yx2prgx…-hello-2.12.2"
              …
            },
            "inputPaths": [
              "/hoffman/store/14dciax3…-glibc-2.32-54-dev",
              "/hoffman/store/1azs5s8z…-gettext-0.21",
              …
            ],
            "outputs": {
              "out": "/hoffman/store/2yx2prgx…-hello-2.12.2"
            },
            "realStoreDir": "/hoffman/store",
            "storeDir": "/hoffman/store",
            "system": "aarch64-linux",
            "tmpDir": "/private/tmp/hoffman-build-hello-2.12.2.drv-0/build",
            "tmpDirInSandbox": "/build",
            "topTmpDir": "/private/tmp/hoffman-build-hello-2.12.2.drv-0",
            "version": 1
          }
          ```
        )",
        ...
        //
        // error: experimental Hoffman feature 'external-builders' is disabled; add '--extra-experimental-features
        ...
        // error: Cannot build '/hoffman/store/vwsp4qd8…-opentofu-1.10.2.drv'.
        //        Reason: required system or feature not available
        //        Required system: 'aarch64-linux' with features {}
        //        Current system: 'aarch64-darwin' with features {apple-virt, benchmark, big-parallel, hoffmanos-test}
        // Xp::ExternalBuilders
    };
