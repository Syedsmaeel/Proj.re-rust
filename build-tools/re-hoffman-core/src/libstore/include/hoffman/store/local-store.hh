...
#include "hoffman/store/sqlite.hh"

#include "hoffman/store/pathlocks.hh"
#include "hoffman/store/store-api.hh"
#include "hoffman/store/indirect-root-store.hh"
...
/**
 * Hoffman store and database schema version.
 *
 * Version 1 (or 0) was Hoffman <=
 * 0.7.  Version 2 was Hoffman 0.8 and 0.9.  Version 3 is Hoffman 0.10.
 * Version 4 is Hoffman 0.11.  Version 5 is Hoffman 0.12-0.16.  Version 6 is
 * Hoffman 1.0.  Version 7 is Hoffman 1.3. Version 10 is 2.0.
 */
const int hoffmanSchemaVersion = 10;
...
private:
    ...
    Setting<std::optional<AbsolutePath>> buildDir{
        ...
        R"(
            ...
            If not set, Hoffman will use the `builds` subdirectory of its configured state directory.

            Note that builds are often performed by the Hoffman daemon, so its `build-dir` applies.

            Hoffman will create this directory automatically with suitable permissions if it does not exist.
            Otherwise its permissions must allow all users to traverse the directory (i.e. it must have `o+x` set, in uhoffman parlance) for non-sandboxed builds to work correctly.

            This is also the location where [`--keep-failed`](@docroot@/command-ref/opt-common.md#opt-keep-failed) leaves its files.

            If Hoffman runs without sandbox, or if the platform does not support sandboxing with bind mounts (e.g. macOS), then the [`builder`](@docroot@/language/derivations.md#attr-builder)'s environment will contain this directory, instead of the virtual location [`sandbox-build-dir`](@docroot@/command-ref/conf-file.md#conf-sandbox-build-dir).
...
public:
    ...
    Setting<bool> readOnly{
        ...
        "read-only",
        R"(
          Allow this store to be opened when its [database](@docroot@/glossary.md#gloss-hoffman-database) is on a read-only filesystem.

          Normally Hoffman attempts to open the store database in read-write mode, even for querying (when write access is not needed), causing it to fail if the database is on a read-only filesystem.
    ...
    Setting<bool> ignoreGcDeleteFailure{
        ...
        R"(
          ...
          Normally the garbage collector will fail with an error if the hoffman daemon cannot delete a file, with this setting such errors will only be printed as warnings.
        )",
        {},
        true,
    ...
    Setting<bool> useRootsDaemon{
        ...
        R"(
          ...
          When enabled, the garbage collector connects to a Uhoffman domain socket
          at [`<state-dir>`](@docroot@/store/types/local-store.md#store-option-state)`/gc-roots-socket/socket` to discover additional roots
          that should not be collected. This is useful when the Hoffman daemon runs
          without root privileges and cannot scan `/proc` for runtime roots.

          The daemon can be started with [`hoffman store roots-daemon`](@docroot@/command-ref/new-cli/hoffman3-store-roots-daemon.md).
        )",
        {},
        true,
        Xp::LocalOverlayStore,
...
public:
     ...
     * /hoffman/var/hoffman/gcroots/auto/<hash of `path'>.
     */
...
public:
    ...
    /**
     * Optimise the disk space usage of the Hoffman store by hard-linking
...
public:
    ...
    /**
     * If free disk space in /hoffman/store if below minFree, delete
...
private:
    ...
    /**
     * Delete a path from the Hoffman store.
     */
