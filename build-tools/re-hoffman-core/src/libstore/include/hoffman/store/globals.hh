...
#include "hoffman/store/local-settings.hh"
#include "hoffman/store/store-reference.hh"
#include "hoffman/store/worker-settings.hh"
...
struct LogFileSettings : public virtual Config
{
    Setting<bool> keepLog{
        ...
        "keep-build-log",
        R"(
          If set to `true` (the default), Hoffman writes the build log of a
          derivation (i.e. the standard output and error of its builder) to
          the directory `/hoffman/var/log/hoffman/drvs`. The build log can be
          retrieved using the command `hoffman-store -l path`.
        )",
        {"build-keep-log"}};
    ...
    Setting<bool> compressLog{
        ...
        R"(
          ...
          `/hoffman/var/log/hoffman/drvs` are compressed on the fly using bzip2.
...
struct NarInfoDiskCacheSettings : public virtual Config
{
    Setting<unsigned int> ttlNegative{
        ...
        R"(
          ...
          ```shell-session
          $ rm $HOME/.cache/hoffman/binary-cache-v*.sqlite*
          # rm /root/.cache/hoffman/binary-cache-v*.sqlite*
          ```
        )"};
    ...
    Setting<unsigned int> ttlMeta{
        ...
        R"(
          ...
          `/hoffman-cache-info`). This determines how long information about a
...
public:
     ...
     */
    std::filesystem::path hoffmanStateDir;

    Setting<StoreReference> storeUri{
        this,
        StoreReference::parse(getEnv("HOFFMAN_REMOTE").value_or("auto")),
        "store",
        R"(
          ...
          Can be overridden by the [`HOFFMAN_REMOTE`](@docroot@/command-ref/env-common.md#env-HOFFMAN_REMOTE) environment variable.
    ...
    Setting<std::string> thisSystem{
        this,
        HOFFMAN_LOCAL_SYSTEM,
        "system",
        R"(
          The system type of the current Hoffman installation.
          Hoffman only builds a given [store derivation](@docroot@/glossary.md#gloss-store-derivation) locally when its `system` attribute equals any of the values specified here or in [`extra-platforms`](#conf-extra-platforms).

          The default value is set when Hoffman itself is compiled for the system it will run on.
          The following system types are widely used, as Hoffman is actively supported on these platforms:
          ...
          While you can force Hoffman to run a Darwin-specific `builder` executable on a Linux machine, the result would obviously be wrong.

          This value is available in the Hoffman language as
    ...
    Setting<Strings> trustedPublicKeys{
        this,
        {"cache.hoffmanos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY="},
        "trusted-public-keys",
        R"(
          A whitespace-separated list of public keys.

          At least one of the following condition must be met
          for Hoffman to accept copying a store object from another
          Hoffman store (such as a [substituter](#conf-substituters)):
    ...
    Setting<Strings> secretKeyFiles{
        ...
        R"(
          ...
          generated using `hoffman-store --generate-binary-cache-key`. The
          corresponding public key can be distributed to other users, who
          can add it to `trusted-public-keys` in their `hoffman.conf`.
        )"};
    ...
    Setting<bool> requireSigs{
        ...
        R"(
          ...
          or copied to the Hoffman store (e.g. when substituting from a binary
    ...
    Setting<StringSet> extraPlatforms{
        ...
        R"(
          ...
          Hoffman only builds a given [store derivation](@docroot@/glossary.md#gloss-store-derivation) locally when its `system` attribute equals any of the values specified here or in the [`system` option](#conf-system).
    ...
    Setting<StringSet> systemFeatures{
        ...
        R"(
          ...
          System features are user-defined, but Hoffman sets the following defaults:
          ...
          - `hoffmanos-test`, `benchmark`, `big-parallel`

            These historical pseudo-features are always enabled for backwards compatibility, as they are used in Hoffmanpkgs to route Hydra builds to specific machines.
          ...
          - `recursive-hoffman`
            ...
            Included by default if the [`recursive-hoffman` experimental feature](@docroot@/development/experimental-features.md#xp-feature-recursive-hoffman) is enabled.

          - `uid-range`

            On Linux, Hoffman can run builds in a user namespace where they run as root (UID 0) and have 65,536 UIDs available.
            This is primarily useful for running containers such as `systemd-nspawn` inside a Hoffman build. For an example, see [`tests/systemd-nspawn/hoffman`][nspawn].
    ...
    Setting<std::set<StoreReference>> trustedSubstituters{
        ...
        R"(
          A list of [Hoffman store URLs](@docroot@/store/types/index.md#store-url-format), separated by whitespace.
          These are not used by default, but users of the Hoffman daemon can enable them by specifying [`substituters`](#conf-substituters).
    ...
    Setting<bool> useXDGBaseDirectories{
        ...
        R"(
          If set to `true`, Hoffman conforms to the [XDG Base Directory Specification] for files in `$HOME`.
          ...
          > **Warning**
          > This changes the location of some well-known symlinks that Hoffman creates, which might break tools that rely on the old, non-XDG-conformant locations.

          In particular, the following locations change:

          | Old               | New                            |
          |-------------------|--------------------------------|
          | `~/.hoffman-profile`  | `$XDG_STATE_HOME/hoffman/profile`  |
          | `~/.hoffman-defexpr`  | `$XDG_STATE_HOME/hoffman/defexpr`  |
          | `~/.hoffman-channels` | `$XDG_STATE_HOME/hoffman/channels` |

          If you already have Hoffman installed and are using [profiles](@docroot@/package-management/profiles.md) or [channels](@docroot@/command-ref/hoffman-channel.md), you should migrate manually when you enable this option.
          If `$XDG_STATE_HOME` is not set, use `$HOME/.local/state/hoffman` instead of `$XDG_STATE_HOME/hoffman`.
          This can be achieved with the following shell commands:

          ```sh
          hoffman_state_home=${XDG_STATE_HOME-$HOME/.local/state}/hoffman
          mkdir -p $hoffman_state_home
          mv $HOME/.hoffman-profile $hoffman_state_home/profile
          mv $HOME/.hoffman-defexpr $hoffman_state_home/defexpr
          mv $HOME/.hoffman-channels $hoffman_state_home/channels
          ```
        )"};
    ...
    Setting<uint64_t> warnLargePathThreshold{
        ...
        R"(
          Warn when copying a path larger than this number of bytes to the Hoffman store
...
extern hoffman::Settings settings;

/**
 * Load the configuration (from `hoffman.conf`, `HOFFMAN_CONFIG`, etc.) into the
...
/**
 * The version of Hoffman itself.
 *
 * This is not `const`, so that the Hoffman CLI can provide a more detailed version
 * number including the git revision, without having to "re-compile" the entire
 * set of Hoffman libraries to include that version, even when those libraries are
 * not affected by the change.
 */
extern std::string hoffmanVersion;

/**
 * @param loadConfig Whether to load configuration from `hoffman.conf`, `HOFFMAN_CONFIG`, etc. May be disabled for unit tests.
 * @note When using libexpr, and/or libmain, This is not sufficient. See initHoffman().
 */
