...
struct Generation
{
     ...
     * within a profile. Hoffman tries to ensure successive generations have
...
/**
 * Optimistic locking is used by long-running operations like `hoffman-env
 * -i'.  Instead of acquiring the exclusive lock for the entire
 * duration of the operation, we just perform the operation
 * optimistically (without an exclusive lock), and check at the end
 * whether the profile changed while we were busy (i.e., the symlink
 * target changed).  If so, the operation is restarted.  Restarting is
 * generally cheap, since the build results are still in the Hoffman
...
{
    const std::filesystem::path & hoffmanStateDir;
...
/**
 * Resolve the default profile (~/.hoffman-profile by default,
 * $XDG_STATE_HOME/hoffman/profile if XDG Base Directory Support is enabled),
