...
struct RestrictionContext
{
    ...
    {
        /**
         * Paths that were added via recursive Hoffman calls.
         */
        std::map<StorePath, std::shared_future<void>> addedPaths;

        /**
         * Realisations that were added via recursive Hoffman calls.
         */
    ...
    /**
     * Recursive Hoffman calls are only allowed to build or realize paths
     * in the original input closure or added via a recursive Hoffman call
     * (so e.g. you can't do 'hoffman-store -r /hoffman/store/<bla>' where
     * /hoffman/store/<bla> is some arbitrary path in a binary cache).
     */
