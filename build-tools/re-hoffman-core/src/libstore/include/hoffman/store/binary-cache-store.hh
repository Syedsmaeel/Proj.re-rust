...
public:
    ...
    Setting<CompressionAlgo> compression{
        ...
        R"(
          ...
          To use a particular compression method Hoffman has to be built with a version of libarchive that natively supports that compression algorithm.
        )"};
    ...
    Setting<std::optional<AbsolutePath>> localNarCache{
        ...
        std::nullopt,
        "local-nar-cache",
        "Path to a local cache of NARs fetched from this binary cache, used by commands such as `hoffman store cat`."};
...
protected:
    ...
    constexpr const static std::string cacheInfoFile = "hoffman-cache-info";
...
public:
    ...
    /**
     * Get the contents of /hoffman-cache-info. Return std::nullopt if it
     * doesn't exist.
     */
    virtual std::optional<std::string> getHoffmanCacheInfo();
