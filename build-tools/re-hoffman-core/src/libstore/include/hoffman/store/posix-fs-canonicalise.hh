...
struct CanonicalizePathMetadataOptions
{
     ...
     * Normally Hoffman attempts to remove all ACLs from files and directories
     * in the Hoffman store, but some ACLs like `security.selinux` or
 ...
 */
#if HOFFMAN_SUPPORT_ACL
#  define HOFFMAN_WHEN_SUPPORT_ACLS(ARG) .ignoredAcls = ARG,
#else
#  define HOFFMAN_WHEN_SUPPORT_ACLS(ARG)
#endif
 ...
 *
 * - the owner and group are set to the Hoffman user and group, if we're
 *   running as root. (Uhoffman only.)
 */
