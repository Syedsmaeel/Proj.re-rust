...
#include "hoffman/store/global-paths.hh"
...
public:
    ...
    Setting<bool> tryFallback{
        ...
        "fallback",
        R"(
          If set to `true`, Hoffman falls back to building from source if a
    ...
    MaxBuildJobsSetting maxBuildJobs{
        ...
        "max-jobs",
        R"(
          Maximum number of jobs that Hoffman tries to build locally in parallel.

          The special value `auto` causes Hoffman to use the number of CPUs in your system.
    ...
    Setting<unsigned int> maxSubstitutionJobs{
        ...
        R"(
          This option defines the maximum number of substitution jobs that Hoffman
    ...
    Setting<Strings> buildHook{
        this,
        {"hoffman", "__build-remote"},
        "build-hook",
        R"(
          The path to the helper program that executes remote builds.

          Hoffman communicates with the build hook over `stdio` using a custom protocol to request builds that cannot be performed directly by the Hoffman daemon.
          The default value is the internal Hoffman binary that implements remote building.
    ...
    Setting<std::string> builders{
        this,
        "@" + (hoffmanConfDir() / "machines").string(),
        "builders",
        R"(
          ...
          2. A comma-separated list of [Hoffman system types](@docroot@/development/building.md#system-type).
          ...
          4. The maximum number of builds that Hoffman executes in parallel on the machine.
          ...
          5. The “speed factor”, indicating the relative speed of the machine as a positive integer.
             If there are multiple machines of the right type, Hoffman prefers the fastest, taking load into account.
          ...
          >
          > ```
          > hoffman@scratchy.labs.cs.uu.nl i686-linux /home/hoffman/.ssh/id_scratchy 8 1 kvm
          > hoffman@itchy.labs.cs.uu.nl    i686-linux /home/hoffman/.ssh/id_scratchy 8 2
          > hoffman@poochie.labs.cs.uu.nl  i686-linux /home/hoffman/.ssh/id_scratchy 1 2 kvm benchmark
          > ```
          >
          ...
          For Hoffman to use substituters, the calling user must be in the [`trusted-users`](#conf-trusted-users) list.

          > **Note**
          >
          > A build machine must be accessible via SSH and have Hoffman installed.
          > `hoffman` must be available in `$PATH` for the user connecting over SSH.

          > **Warning**
          >
          > If you are building via the Hoffman daemon (default), the Hoffman daemon user account on the local machine (that is, `root`) requires access to a user account on the remote machine (not necessarily `root`).
          >
    ...
    Setting<bool> alwaysAllowSubstitutes{
        ...
        R"(
          If set to `true`, Hoffman ignores the [`allowSubstitutes`](@docroot@/language/advanced-attributes.md) attribute in derivations and always attempt to use [available substituters](#conf-substituters).
        )"};
    ...
    Setting<bool> buildersUseSubstitutes{
        ...
        R"(
          If set to `true`, Hoffman instructs [remote build machines](#conf-builders) to use their own [`substituters`](#conf-substituters) if available.

          It means that remote build hosts fetch as many dependencies as possible from their own substituters (e.g, from `cache.hoffmanos.org`) instead of waiting for the local machine to upload them all.
    ...
    Setting<bool> useSubstitutes{
        ...
        "substitute",
        R"(
          If set to `true` (default), Hoffman uses binary substitutes if
    ...
    Setting<std::vector<StoreReference>> substituters{
        this,
        std::vector<StoreReference>{StoreReference::parse("https://cache.hoffmanos.org/")},
        "substituters",
        R"(
          A list of [URLs of Hoffman stores](@docroot@/store/types/index.md#store-url-format) to be used as substituters, separated by whitespace.
          A substituter is an additional [store](@docroot@/glossary.md#gloss-store) from which Hoffman can obtain [store objects](@docroot@/store/store-object.md) instead of building them.

          Substituters are tried based on their priority value, which each substituter can set independently.
          Lower value means higher priority.
          The default is `https://cache.hoffmanos.org`, which has a priority of 40.

          At least one of the following conditions must be met for Hoffman to use a substituter:

          - The substituter is in the [`trusted-substituters`](#conf-trusted-substituters) list
          - The user calling Hoffman is in the [`trusted-users`](#conf-trusted-users) list
    ...
    Setting<std::string> postBuildHook{
        ...
        R"(
          ...
          This option is only settable in the global `hoffman.conf`, or on the
          command line by trusted users.

          When using the hoffman-daemon, the daemon executes the hook as `root`.
          If the hoffman-daemon is not involved, the hook runs as the user
          executing the hoffman-build.
          ...
          contains the following environment variables:
            ...
            - `DRV_PATH`
              ...
              Example:
              `/hoffman/store/5nihn1a7pa8b25l9zafqaqibznlvvp3f-bash-4.4-p23.drv`
            ...
            - `OUT_PATHS`
              ...
              Example:
              `/hoffman/store/l88brggg9hpy96ijds34dlq4n8fan63g-bash-4.4-p23-dev
              /hoffman/store/vch71bhyi5akr5zs40k8h2wqxx69j80l-bash-4.4-p23-doc
              /hoffman/store/c5cxjywi66iwn9dcx5yvwjkvl559ay6p-bash-4.4-p23-info
              /hoffman/store/scz72lskj03ihkcn42ias5mlp4i4gr1k-bash-4.4-p23-man
              /hoffman/store/a724znygmd1cac856j3gfsyvih3lw07j-bash-4.4-p23`.
        )"};
};
