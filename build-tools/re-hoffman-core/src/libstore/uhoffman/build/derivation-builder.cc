...
#include "hoffman/store/build/child.hh"
#include "hoffman/util/uhoffman-domain-socket.hh"
#include "hoffman/store/posix-fs-canonicalise.hh"
#include "hoffman/util/posix-source-accessor.hh"
#include "hoffman/store/restricted-store.hh"
#include "hoffman/store/user-lock.hh"
 ...
 */
// FIXME: rename this to UhoffmanDerivationBuilder or something like that.
...
protected:
    ...
    /**
     * The recursive Hoffman daemon socket.
     */
...
protected:
    ...
    {
        return acquireUserLock(settings.hoffmanStateDir, localSettings, 1, false);
    }
     ...
     * This includes:
     *
     * * The paths specified by the `sandbox-paths` setting, and their closure in the Hoffman store.
...
private:
    ...
    /**
     * Start an in-process hoffman daemon thread for recursive-hoffman.
     */
    void startDaemon();

    /**
     * Stop the in-process hoffman daemon thread.
     * @see startDaemon
     */
    void stopDaemon();
...
SingleDrvOutputs DerivationBuilderImpl::unprepareBuild()
{
    ...
    /* Terminate the recursive Hoffman daemon. */
    stopDaemon();
...
std::optional<Descriptor> DerivationBuilderImpl::startBuild()
{
    ...
    /* Create a temporary directory where the build will take
       place. */
    topTmpDir = createTempDir(buildDir, "hoffman", 0700);
    setBuildTmpDir();
    assert(!tmpDir.empty());
    ...
    /* Fire up a Hoffman daemon to process recursive Hoffman calls from the
       builder. */
    if (drvOptions.getRequiredSystemFeatures(drv).count("recursive-hoffman"))
        startDaemon();
...
void DerivationBuilderImpl::initEnv()
{
    ...
    /* Tell the builder where the Hoffman store is.  Usually they
    ...
    /* For convenience, set an environment pointing to the top build
       directory. */
    env["HOFFMAN_BUILD_TOP"] = tmpDirInSandbox();
    ...
    /* Compatibility hack with Hoffman <= 0.7: if this is a fixed-output
       derivation, tell the builder, so that for instance `fetchurl'
       can skip checking the output.  On older Hoffmanes, this environment
       variable won't be set, so `fetchurl' will do the check. */
    if (derivationType.isFixed())
        env["HOFFMAN_OUTPUT_CHECKED"] = "1";
...
{
    experimentalFeatureSettings.require(Xp::RecursiveHoffman);
    ...
    auto socketName = ".hoffman-socket";
    std::filesystem::path socketPath = tmpDir / socketName;
    env["HOFFMAN_REMOTE"] = "uhoffman://" + (tmpDirInSandbox() / socketName).native();

    daemonSocket = createUhoffmanDomainSocket(socketPath, 0600);
...
#endif
                HOFFMAN_WHEN_SUPPORT_ACLS(localSettings.ignoredAcls)},
            inodesSeen);
