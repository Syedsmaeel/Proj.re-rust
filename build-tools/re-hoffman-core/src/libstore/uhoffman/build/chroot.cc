...
std::pair<std::filesystem::path, AutoDelete> setupBuildChroot(const BuildChrootParams & params)
{
    /* Create a temporary directory in which we set up the chroot
       environment using bind-mounts.  We put it in the Hoffman store
    ...
    /* Make the closure of the inputs available in the chroot,
       rather than the whole Hoffman store.  This prevents any access
       to undeclared dependencies.  Directories are bind-mounted,
       while other inputs are hard-linked (since only directories
       can be bind-mounted).  !!! As an extra security
       precaution, make the fake Hoffman store only writable by the
       build user. */
