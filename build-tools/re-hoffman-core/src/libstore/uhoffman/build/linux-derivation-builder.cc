...
#  include "hoffman/store/personality.hh"
...
#  if HAVE_SECCOMP
    ...
    constexpr std::string_view nativeSystem = HOFFMAN_LOCAL_SYSTEM;
    ...
    /* Prevent builders from using EAs or ACLs. Not all filesystems
       support these, and they're not allowed in the Hoffman store because
...
#  if HAVE_LANDLOCK && defined(LANDLOCK_SCOPE_ABSTRACT_UHOFFMAN_SOCKET)

#    define DO_LANDLOCK 1

/* We are using LANDLOCK_SCOPE_ABSTRACT_UHOFFMAN_SOCKET on best-effort basis. There are no glibc wrappers for now. */
...
{
    bool landlockSupportsScopeAbstractUhoffmanSocket = []() {
        int abiVersion = getLandlockAbiVersion();
        if (abiVersion >= 6)
            /* All good, we can use LANDLOCK_SCOPE_ABSTRACT_UHOFFMAN_SOCKET. See
               https://docs.kernel.org/userspace-api/landlock.html#abstract-uhoffman-socket-abi-6 */
            return true;
        ...
        debug("landlock version %d does not support LANDLOCK_SCOPE_ABSTRACT_UHOFFMAN_SOCKET", abiVersion);
        return false;
    }();

    /* Bail out early if landlock is not enabled or LANDLOCK_SCOPE_ABSTRACT_UHOFFMAN_SOCKET wouldn't work.
       TODO: Consider adding more landlock rules for filesystem access as defense-in-depth on top. */
    if (!landlockSupportsScopeAbstractUhoffmanSocket)
        return;
    ...
    ::landlock_ruleset_attr attr = {
        ...
        .scoped = LANDLOCK_SCOPE_ABSTRACT_UHOFFMAN_SOCKET,
    };
...
struct ChrootLinuxDerivationBuilder : ChrootDerivationBuilder, LinuxDerivationBuilder
{
     ...
     * paths to the sandbox as a result of recursive Hoffman calls.
     */
    ...
    std::unique_ptr<UserLock> getBuildUser() override
    {
        return acquireUserLock(
            settings.hoffmanStateDir, store.config->getLocalSettings(), drvOptions.useUidRange(drv) ? 65536 : 1, true);
    }
    ...
    void prepareUser() override
    {
        if ((buildUser && buildUser->getUIDCount() != 1) || store.config->getLocalSettings().useCgroups) {
            ...
            cgroup = rootCgroupPath
                     / (buildUser ? fmt("hoffman-build-uid-%d", buildUser->getUID())
                                  : fmt("hoffman-build-pid-%d-%d", getpid(), counter++));
            ...
            if (buildUser) {
                auto cgroupsDir = std::filesystem::path{settings.hoffmanStateDir} / "cgroups";
...
#  endif
        ...
        */
        ...
        /* Make sure that we write *something* to the child in case of
           ...
           multi-threaded Hoffman, since several child processes may have
        ...
        writeFile(
            ...
            fmt("root:x:0:0:Hoffman build user:%3%:/noshell\n"
                "hoffmanbld:x:%1%:%2%:Hoffman build user:%3%:/noshell\n"
    ...
    void enterChroot() override
    {
        ...
        /* Bind-mount the sandbox's Hoffman store onto itself so that
        ...
        if (mount(chrootStoreDir.c_str(), chrootStoreDir.c_str(), 0, MS_BIND, 0) == -1)
            throw SysError("unable to bind mount the Hoffman store at %1%", PathFmt(chrootStoreDir));
            ...
            if (systemFeatures.count("kvm")) {
                if (pathExists("/dev/kvm")) {
                    ...
                    warn(
                        ...
                        "Consider removing 'kvm' from system-features in hoffman.conf if KVM is not supported on this system.");
                }
            }
        ...
        if (!derivationType.isSandboxed()) {
            ...
            /* N.B. it is realistic that these paths might not exist. It
               happens when testing Hoffman building fixed-output derivations
...
#  endif
        ...
        /* Unshare this mount namespace. This is necessary because
           ...
           Hoffman store into the sandbox. Therefore, we save the
           pre-pivot_root namespace in
           sandboxMountNamespace. Since we made /hoffman/store a
