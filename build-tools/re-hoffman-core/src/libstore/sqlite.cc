...
SQLite::SQLite(const std::filesystem::path & path, Settings && settings)
{
    ...
    // `uhoffman-dotfile` is needed on NFS file systems and on Windows' Subsystem
    // for Linux (WSL) where useSQLiteWAL should be false by default.
    const char * vfs = settings.useWAL ? 0 : "uhoffman-dotfile";
    ...
    if (getEnv("HOFFMAN_DEBUG_SQLITE_TRACES") == "1") {
