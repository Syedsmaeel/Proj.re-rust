...
/**
 * Map AWS log level to Hoffman verbosity.
...
/**
 * Custom AWS logger that routes logs through Hoffman's logging infrastructure.
 *
 ...
 */
static int hoffmanAwsLoggerLog(
    struct aws_logger * logger, enum aws_log_level logLevel, aws_log_subject_t subject, const char * format, ...)
{
    Verbosity hoffmanLevel = awsLogLevelToVerbosity(logLevel);
    if (hoffmanLevel > verbosity)
    ...
    printMsgUsing(hoffman::logger, hoffmanLevel, "(aws:%s) %s", subjectName ? subjectName : "unknown", chomp(buffer.data()));
    return AWS_OP_SUCCESS;
}
 ...
 */
static aws_log_level hoffmanAwsLoggerGetLevel(struct aws_logger * logger, aws_log_subject_t subject)
{
    // Map Hoffman verbosity back to AWS log level (inverse of awsLogLevelToVerbosity)
...
static void initialiseAwsLogger()
{
    ...
    std::call_once(initialised, []() {
        static aws_logger_vtable hoffmanAwsLoggerVtable = {
            .log = hoffmanAwsLoggerLog,
            .get_log_level = hoffmanAwsLoggerGetLevel,
            .clean_up = [](struct aws_logger *) {}, // No resources to clean up
            .set_log_level = nullptr,
        };

        static aws_logger hoffmanAwsLogger = {
            .vtable = &hoffmanAwsLoggerVtable,
            .allocator = nullptr,
            .p_impl = nullptr,
        };

        aws_logger_set(&hoffmanAwsLogger);
    });
}
...
public:
    ...
    {
        // Install custom logger that routes AWS CRT logs through Hoffman's logging infrastructure.
        // This ensures AWS logs respect Hoffman's verbosity settings and are formatted consistently.
        initialiseAwsLogger();
