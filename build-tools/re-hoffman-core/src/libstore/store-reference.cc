...
#include "hoffman/util/file-path-impl.hh"
...
StoreReference StoreReference::parse(const std::string & uri, const StoreReference::Params & extraParams)
{
    ...
    try {
        ...
        if (baseURI == "" || baseURI == "auto") {
            ...
            return {
                .variant = Specified{.scheme = "uhoffman", .authority = ""},
                .params = std::move(params),
            };
             ...
             * Old versions of hoffman allowed that. Note that this is ambiguous and does not allow
    ...
    throw UsageError("Cannot parse Hoffman store '%s'", uri);
}
