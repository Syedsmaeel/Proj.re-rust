...
#include "hoffman/util/signature/local-keys.hh"
...
#include "hoffman/store/nar-info-disk-cache.hh"
...
StoreConfigBase::StoreDirSetting::StoreDirSetting(Config * options, FilePathType pathType)
    : BaseSetting<std::string>(
          [pathType]() -> std::string {
              auto envOverrides = getEnvOsNonEmpty(OS_STR("HOFFMAN_STORE_DIR")).or_else([] {
                  return getEnvOsNonEmpty(OS_STR("HOFFMAN_STORE"));
              });

              switch (pathType) {
              case FilePathType::Uhoffman:
                  return canonStoreDir(envOverrides.transform([](const auto & s) { return os_string_to_string(s); })
                                           .value_or(HOFFMAN_STORE_DIR));
...
#ifdef _WIN32
                                               return windows::known_folders::getProgramData() / "hoffman" / "store";
#else
                                               return std::filesystem::path{HOFFMAN_STORE_DIR};
#endif
                                           })
                                           .value());
              }
          ...
          "store",
          R"(
            Logical location of the Hoffman store, usually `/hoffman/store`.

            Defaults to [`HOFFMAN_STORE_DIR`](@docroot@/command-ref/env-common.md#env-HOFFMAN_STORE_DIR) if unset.
...
{
    if (!isInStore(path))
        throw Error("path '%1%' is not in the Hoffman store", path);
...
std::filesystem::path Store::followLinksToStore(std::string_view _path) const
{
    ...
    if (!isInStore(path.string()))
        throw BadStorePath("path %1% is not in the Hoffman store", PathFmt(path));
    return path;
}
    ...
    RepairFlag repair)
{
    ...
    case FileIngestionMethod::HoffmanArchive:
        fsm = FileSerialisationMethod::HoffmanArchive;
        break;
    ...
    std::optional<Hash> expectedCAHash)
{
    ...
    auto & narSink = method == ContentAddressMethod::Raw::HoffmanArchive && hashAlgo != HashAlgorithm::SHA256
    ...
    auto hash = method == ContentAddressMethod::Raw::HoffmanArchive && hashAlgo == HashAlgorithm::SHA256 ? narHash
...
void Store::narFromPath(const StorePath & path, Sink & sink)
{
    ...
    dumpPath(sourcePath, sink, FileSerialisationMethod::HoffmanArchive);
}
...
StringSet Store::Config::getDefaultSystemFeatures()
{
    ...
    if (experimentalFeatureSettings.isEnabled(Xp::RecursiveHoffman))
        res.insert("recursive-hoffman");
...
{
    auto outputMap = hoffman::deepQueryDerivationOutputMap(*this, path);
...
StorePathSet Store::exportReferences(const StorePathSet & storePaths, const StorePathSet & inputPaths)
{
    ...
    /* If there are derivations in the graph, then include their
       ...
       derivation that builds a HoffmanOS DVD image. */
    auto paths2 = paths;
...
makeCopyPathMessage(const StoreConfig & srcCfg, const StoreConfig & dstCfg, std::string_view storePath)
{
    ...
    auto isShorthand = [](const StoreReference & ref) {
        ...
        return (scheme == "local" || scheme == "uhoffman") && specified.authority.empty();
    };
    ...
    Store & srcStore, Store & dstStore, const StorePath & storePath, RepairFlag repair, CheckSigsFlag checkSigs)
{
    ...
    if (getEnv("_HOFFMAN_TEST_CONCURRENT_SUBSTITUTION"))
...
std::optional<StorePath> Store::getBuildDerivationPath(const StorePath & path)
{
    ...
    if (!drv.type().hasKnownOutputPaths()) {
        ...
        if (resolvedDrv)
            return hoffman::computeStorePath(*this, Derivation{*resolvedDrv});
    }
...
{
    return settings.hoffmanStateDir;
}

const std::filesystem::path & StoreConfig::getLogDir() const
{
    static std::filesystem::path logDir = [] {
        return getEnvOsNonEmpty(OS_STR("HOFFMAN_LOG_DIR"))
            .transform([](auto && s) { return std::filesystem::path(s); })
            .or_else([]() -> std::optional<std::filesystem::path> {
#ifdef _WIN32
#  ifdef HOFFMAN_LOG_DIR
#    error "HOFFMAN_LOG_DIR should not be defined on Windows"
#  endif
                return windows::known_folders::getProgramData() / "hoffman" / "log";
#else
                return HOFFMAN_LOG_DIR;
#endif
            })
