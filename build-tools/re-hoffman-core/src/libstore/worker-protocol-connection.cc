...
static Logger::Fields readFields(Source & from)
{
    ...
    for (size_t n = 0; n < size; n++) {
        ...
        if (type == Logger::Field::tInt)
            ...
            throw Error("got unsupported field type %x from Hoffman daemon", (int) type);
    }
    return fields;
}
...
WorkerProto::BasicClientConnection::processStderrReturn(Sink * sink, Source * source, bool flush, bool block)
{
    ...
    while (true) {
        ...
        if (msg == STDERR_WRITE) {
            ...
            throw Error("got unknown message type %x from Hoffman daemon", msg);
    }
    ...
    if (!ex) {
        ...
        try {
            ...
            // Hoffman versions before #4628 did not have an adequate
    ...
    BufferedSink & to, Source & from, const WorkerProto::Version & localVersion)
{
    ...
    if (magic != WORKER_MAGIC_2)
        throw Error("hoffman-daemon protocol mismatch from");
    ...
    if (daemonVersion.major != WorkerProto::latest.number.major)
        throw Error("Hoffman daemon protocol version not supported");
