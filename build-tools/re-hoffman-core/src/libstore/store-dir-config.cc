...
{
    // On Windows, `/hoffman/store` is not a canonical path. More broadly it
    ...
    if (p.parent_path() != storeDir)
        throw BadStorePath("path %s is not in the Hoffman store", PathFmt(p));
...
in the Hoffman manual. These few functions implement that specification.
...
{
    /* e.g., "source:sha256:1abc...:/hoffman/store:foo.tar.gz" */
...
StorePath StoreDirConfig::makeFixedOutputPath(std::string_view name, const FixedOutputInfo & info) const
{
    ...
    if (info.hash.algo == HashAlgorithm::SHA256 && info.method == FileIngestionMethod::HoffmanArchive) {
