    ...
    if (conn.version >= WorkerProto::Version{.number = {1, 30}}) {
        ...
        std::visit(
            overloaded{
                ...
                [&](std::monostate) {
                    throw Error(
                        "wanted to build a derivation that is itself a build product, but protocols do not support that. Try upgrading the Hoffman on the other end of this connection");
                },
            },
            sOrDrvPath);
    }
...
WorkerProto::Serialise<WorkerProto::ClientHandshakeInfo>::read(const StoreDirConfig & store, ReadConn conn)
{
    ...
    if (conn.version >= WorkerProto::Version{.number = {1, 33}}) {
        res.daemonHoffmanVersion = readString(conn.from);
    }
...
{
    if (conn.version >= WorkerProto::Version{.number = {1, 33}}) {
        assert(info.daemonHoffmanVersion);
        conn.to << *info.daemonHoffmanVersion;
    }
