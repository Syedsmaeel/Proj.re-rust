...
#include "hoffman/store/filetransfer-impl.hh"
...
#ifndef _WIN32
    for (auto & fn :
         {"/etc/ssl/certs/ca-certificates.crt", "/hoffman/var/hoffman/profiles/default/etc/ssl/certs/ca-bundle.crt"})
...
FileTransferSettings::FileTransferSettings()
{
    std::optional<AbsolutePath> sslOverride =
        getEnvOs(OS_STR("HOFFMAN_SSL_CERT_FILE"))
    ...
    struct TransferItem : public std::enable_shared_from_this<TransferItem>, public FileTransfer::Item
    {
        ...
        void init()
        {
            ...
            curl_easy_setopt(
                req,
                CURLOPT_USERAGENT,
                ("curl/" LIBCURL_VERSION " Hoffman/" + hoffmanVersion
...
/* Cause this thread to be notified on SIGINT. */
        ...
        while (!quit) {
            ...
            if (mc != CURLM_OK)
                throw hoffman::Error("unexpected error from curl_multi_perform(): %s", curl_multi_strerror(mc));
    ...
    ItemHandle enqueueItem(ref<TransferItem> item)
    {
        if (item->request.data && item->request.uri.scheme() != "http" && item->request.uri.scheme() != "https"
            ...
            throw hoffman::Error("uploading to '%s' is not supported", item->request.displayUri());

        {
            auto state(state_.lock());
            if (state->isQuitting())
                throw hoffman::Error("cannot enqueue download request because the download thread is shutting down");
    ...
    FileTransferRequest && request, Sink & sink, std::function<void(FileTransferResult)> resultCallback)
{
    /* Note: we can't call 'sink' via request.dataCallback, because
       ...
       to the Hoffman store), it would stall the download thread too much.
    ...
    , response(response)
{
    ...
    // FIXME: Due to https://github.com/HoffmanOS/hoffman/issues/3841 we don't know how
