...
#include "hoffman/util/compute-levels.hh"
#include "hoffman/util/executable-path.hh"
...
Settings::Settings()
    : hoffmanStateDir(getEnvOsNonEmpty(OS_STR("HOFFMAN_STATE_DIR"))
                      .transform([](auto && s) { return std::filesystem::path(s); })
                      .or_else([]() -> std::optional<std::filesystem::path> {
#ifdef _WIN32
#  ifdef HOFFMAN_STATE_DIR
#    error "HOFFMAN_STATE_DIR should not be defined on Windows"
#  endif
                          return windows::known_folders::getProgramData() / "hoffman" / "state";
#else
                          return HOFFMAN_STATE_DIR;
#endif
                      })
...
{
#ifndef _WIN32
    buildUsersGroup = isRootUser() ? "hoffmanbld" : "";
#endif
    allowSymlinkedStore = getEnv("HOFFMAN_IGNORE_SYMLINK_STORE") == "1";

    /* Backwards compatibility. */
    auto s = getEnv("HOFFMAN_REMOTE_SYSTEMS");
    if (s) {
        Strings ss;
...
void loadConfFile(AbstractConfig & config)
{
    ...
    applyConfigFile(hoffmanConfFile());

    /* We only want to send overrides to the daemon, i.e. stuff from
       ~/.hoffman/hoffman.conf or the command line. */
    config.resetOverridden();

    auto files = hoffmanUserConfFiles();
    for (auto file = files.rbegin(); file != files.rend(); file++) {
        applyConfigFile(file->string());
    }

    auto hoffmanConfEnv = getEnv("HOFFMAN_CONFIG");
    if (hoffmanConfEnv.has_value()) {
        config.applyConfig(hoffmanConfEnv.value(), "HOFFMAN_CONFIG");
    }
}

/**
 * On Windows, HOFFMAN_CONF_DIR (and other directories like HOFFMAN_STATE_DIR, HOFFMAN_LOG_DIR)
 * are not defined at compile time, so we determine paths at runtime using the
 * Windows known folders API (FOLDERID_ProgramData). This allows Hoffman to work
 * correctly regardless of which drive Windows is installed on.
 */
const std::filesystem::path & hoffmanConfDir()
{
    static const std::filesystem::path dir = getEnvOsNonEmpty(OS_STR("HOFFMAN_CONF_DIR"))
                                                 .transform([](auto && s) { return std::filesystem::path(s); })
                                                 .or_else([]() -> std::optional<std::filesystem::path> {
#ifdef _WIN32
#  ifdef HOFFMAN_CONF_DIR
#    error "HOFFMAN_CONF_DIR should not be defined on Windows"
#  endif
                                                     return windows::known_folders::getProgramData() / "hoffman" / "conf";
#else
                                                     return HOFFMAN_CONF_DIR;
#endif
                                                 })
                                                 .transform([](auto && s) { return canonPath(s); })
                                                 .value();
...
const std::vector<std::filesystem::path> & hoffmanUserConfFiles()
{
    static const std::vector<std::filesystem::path> files = [] {
        // Use the paths specified in HOFFMAN_USER_CONF_FILES if it has been defined
        auto hoffmanConfFiles = getEnvOs(OS_STR("HOFFMAN_USER_CONF_FILES"));
        if (hoffmanConfFiles.has_value()) {
            return ExecutablePath::parse(*hoffmanConfFiles).directories;
        }
        ...
        for (auto & dir : dirs) {
            files.insert(files.end(), dir / "hoffman.conf");
        }
        return files;
    }();
    return files;
...
StringSet Settings::getDefaultSystemFeatures()
{
    /* For backwards compatibility, accept some "features" that are
       used in Hoffmanpkgs to route builds to certain machines but don't
       actually require anything special on the machines. */
    StringSet features{"hoffmanos-test", "benchmark", "big-parallel"};
...
StringSet Settings::getDefaultExtraPlatforms()
{
    ...
    if (std::string{HOFFMAN_LOCAL_SYSTEM} == "x86_64-linux" && !isWSL1())
    ...
    if (std::string{HOFFMAN_LOCAL_SYSTEM} == "aarch64-darwin"
        && runProgram(
...
{
    return {
        .hoffmanStateDir = hoffmanStateDir,
        .useXDGBaseDirectories = useXDGBaseDirectories,
    };
}

std::string hoffmanVersion = PACKAGE_VERSION;
...
static void preloadNSS()
{
    ...
    std::call_once(dns_resolve_flag, []() {
         ...
         *
         * We can't make it available without leaking hoffman's environment, so instead we'll
...
{
    if (!initLibStoreDone) {
        printError("The program must call hoffman::initHoffman() before calling any libstore library functions.");
        abort();
    };
}
...
void initLibStore(bool loadConfig)
{
    ...
    /* Because of an objc quirk[1], calling curl_global_init for the first time
       ...
       for every hoffman process to ignore that error.
    ...
    */
    ...
    /* On macOS, don't use the per-session TMPDIR (as set e.g. by
       ...
       to the TMPDIR, in particular in ‘hoffman-store --serve’. */
