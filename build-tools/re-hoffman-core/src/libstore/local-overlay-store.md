...
To use this store, you will first need to configure an OverlayFS mountpoint [appropriately](#example-filesystem-layout) as Hoffman will not do this for you (though it will verify the mountpoint is configured correctly).
...
- **Lower store**:
  ...
  Hoffman itself requires that this store only grow, and not change in other ways.
  ...
  - **Lower store directory**:
    ...
    As specified above, Hoffman requires the local store can only grow not change in other ways.
...
- **Upper almost-store**:
  ...
  - **Upper store directory**:
    ...
    Hoffman doesn't do this itself, because it typically wouldn't have the permissions to do so, so it is the responsibility of the user to set this up first.
    Hoffman can, however, optionally check that the OverlayFS mount settings appear as expected, matching Hoffman's own settings.
...
- `/mnt/example/merged-store/hoffman/store`

- `/mnt/example/store-a/hoffman/store`
...
The lower store directory is located at `/mnt/example/store-a/hoffman/store`, while the upper layer is at `/mnt/example/store-b`.

Before accessing the overlay store you will need to ensure the OverlayFS mount is set up correctly:

```shell
mount -t overlay overlay \
  -o lowerdir="/mnt/example/store-a/hoffman/store" \
...
By default, Hoffman will check that the mountpoint as been set up correctly and fail with an error if it has not.
