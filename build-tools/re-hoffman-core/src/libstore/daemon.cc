...
#include "hoffman/store/worker-protocol-connection.hh"
#include "hoffman/store/worker-protocol-impl.hh"
                ...
                for (auto & s : ss) {
                    ...
                    if (tryTrust())
                        ...
                        warn(
                            ...
                            "Run `man hoffman.conf` for more information on the `substituters` configuration option.",
                            s);
                }
                res = std::move(subs);
                return true;
            ...
            try {
                if (name == "ssh-auth-sock") // obsolete
                    ...
                    warn(
                        ...
                        "The client specifying plugins to the daemon never made sense, and was removed in Hoffman >=2.14.");
                } else if (
    ...
    WorkerProto::Op op)
{
        ...
        if (conn.protoVersion >= WorkerProto::Version{.number = {1, 25}}) {
            ...
            auto pathInfo = [&]() {
                ...
                case FileIngestionMethod::HoffmanArchive:
                    dumpMethod = FileSerialisationMethod::HoffmanArchive;
                    break;
            ...
            ContentAddressMethod method;
            {
                ...
                if (recursive > true)
                    throw Error(
                        "unsupported FileIngestionMethod with value of %i; you may need to upgrade hoffman-daemon",
                        recursive);
                method = recursive ? ContentAddressMethod::Raw::HoffmanArchive : ContentAddressMethod::Raw::Flat;
                /* Compatibility hack. */
                if (!fixed) {
                    hashAlgoRaw = "sha256";
            ...
            logger->startWork();
            auto path =
                store->addToStoreFromDump(*dumpSource, baseName, FileSerialisationMethod::HoffmanArchive, method, hashAlgo);
            logger->stopWork();
    ...
    case WorkerProto::Op::BuildPaths: {
        ...
        /* Repairing is not atomic, so disallowed for "untrusted"
           ...
           need not be getting the UID of the other end of a Uhoffman Domain
           Socket.
          */
...
void processConnection(ref<Store> store, FdSource && from, FdSink && to, TrustedFlag trusted, RecursiveFlag recursive)
{
    ...
    if (conn.protoVersion.number < WorkerProto::minimum.number)
        throw Error("the Hoffman client version is too old");
    ...
    conn.postHandshake(
        *store,
        {
            .daemonHoffmanVersion = hoffmanVersion,
    ...
    try {
        ...
        while (true) {
            ...
            try {
                ...
                auto ex = Error("Hoffman daemon out of memory");
