...
#include "hoffman/store/worker-protocol.hh"
#include "hoffman/store/derivations.hh"
#include "hoffman/store/realisation.hh"
#include "hoffman/store/references.hh"
#include "hoffman/util/callback.hh"
#include "hoffman/util/topo-sort.hh"
#include "hoffman/util/finally.hh"
#include "hoffman/util/compression.hh"
#include "hoffman/util/signals.hh"
#include "hoffman/store/posix-fs-canonicalise.hh"
#include "hoffman/util/posix-source-accessor.hh"
#include "hoffman/store/keys.hh"
    ...
    , fnTempRoots(tempRootsDir / std::to_string(getpid()))
{
        ...
        while (path != root) {
            if (std::filesystem::is_symlink(path))
                throw Error(
                    ...
                    "this is not allowed for the Hoffman store and its parent directories",
                    PathFmt(path));
    ...
    if (!config->readOnly) {
        ...
        try {
            ...
            if (e.is(std::errc::permission_denied) || e.is(std::errc::operation_not_permitted)) {
                e.addTrace(
                    {},
                    "This command may have been run as non-root in a single-user Hoffman installation,\n"
                    "or the Hoffman daemon may have crashed.");
            }
            throw;
        }
    }
    ...
    if (!config->readOnly && !lockFile(globalLock.get(), ltRead, false)) {
        printInfo("waiting for the big Hoffman store lock...");
    ...
    if (config->readOnly && curSchema < hoffmanSchemaVersion) {
        debug("current schema version: %d", curSchema);
        debug("supported schema version: %d", hoffmanSchemaVersion);
        throw Error(
    ...
    if (curSchema > hoffmanSchemaVersion)
        throw Error("current Hoffman store schema is version %1%, but I only support %2%", curSchema, hoffmanSchemaVersion);

    else if (curSchema == 0) { /* new store */
        curSchema = hoffmanSchemaVersion;
        openDB(*state, true);
        writeFile(schemaPath, fmt("%1%", curSchema), 0666, FsSync::Yes);
    }

    else if (curSchema < hoffmanSchemaVersion) {
        if (curSchema < 5)
            throw Error(
                "Your Hoffman store has a database in Berkeley DB format,\n"
                "which is no longer supported. To convert to the new format,\n"
                "please upgrade Hoffman to version 0.12 first.");

        if (curSchema < 6)
            throw Error(
                "Your Hoffman store has a database in flat file format,\n"
                "which is no longer supported. To convert to the new format,\n"
                "please upgrade Hoffman to version 1.11 first.");

        if (!lockFile(globalLock.get(), ltWrite, false)) {
            printInfo("waiting for exclusive access to the Hoffman store...");
            lockFile(
        ...
        writeFile(schemaPath, fmt("%1%", hoffmanSchemaVersion), 0666, FsSync::Yes);
...
StoreReference LocalStoreConfig::getReference() const
{
    ...
    /* Back-compatibility kludge. Tools like hoffman-output-monitor expect 'local'
...
void LocalStore::openDB(State & state, bool create)
{
    ...
    if (access(dbDir.string().c_str(), R_OK | (config->readOnly ? 0 : W_OK)))
        throw SysError("Hoffman database directory %1% is not writable", PathFmt(dbDir));

    /* Open the Hoffman database. */
    auto & db(state.db);
    ...
    /* Increase the auto-checkpoint interval to 40000 pages.  This
       seems enough to ensure that instantiating the HoffmanOS system
...
void LocalStore::upgradeDBSchema(State & state)
{
    ...
    auto doUpgrade = [&](const std::string & migrationName, const std::string & stmt) {
        ...
        debug("executing Hoffman database schema migration '%s'...", migrationName);
...
/* To improve purity, users may want to make the Hoffman store a read-only
   bind mount.  So make the Hoffman store writable for this process. */
void LocalStore::makeStoreWritable()
{
#ifdef __linux__
    if (!isRootUser())
        return;
    /* Check if /hoffman/store is on a read-only mount. */
    struct statvfs stat;
    if (statvfs(config->realStoreDir.get().c_str(), &stat) != 0)
        throw SysError("getting info about the Hoffman store mount point");
...
uint64_t LocalStore::addValidPath(State & state, const ValidPathInfo & info)
{
    if (info.ca.has_value() && !info.isContentAddressed(*this))
        throw Error(
            "cannot add path '%s' to the Hoffman store because it claims to be content-addressed but isn't",
                ...
                if (info.ca) {
                    ...
                    if (specified.hash != actualHash.hash) {
                        throw Error(
                            ...
                            printStorePath(info.path),
                            specified.hash.to_string(HashFormat::Hoffman32, true),
                            actualHash.hash.to_string(HashFormat::Hoffman32, true));
                    }
                }

                autoGC();

                canonicalisePathMetaData(realPath, {HOFFMAN_WHEN_SUPPORT_ACLS(config->getLocalSettings().ignoredAcls)});
    ...
    RepairFlag repair)
{
    ...
    /* Read the source path into memory, but only if it's up to
       ...
       location in the Hoffman store. If the subsequently computed
    ...
    if (repair || !isValidPath(dstPath)) {
        ...
        if (repair || !isValidPathUncached(dstPath)) {
            ...
            if (dumpMethod != FileSerialisationMethod::HoffmanArchive || hashAlgo != HashAlgorithm::SHA256) {
            ...
            canonicalisePathMetaData(
                realPath, {HOFFMAN_WHEN_SUPPORT_ACLS(localSettings.ignoredAcls)}); // FIXME: merge into restorePath
...
{
    printInfo("reading the Hoffman store...");
    ...
    if (checkContents) {
        ...
        for (auto & link : DirectoryIterator{linksDir}) {
            ...
            std::string hash =
                hashPath(makeFSSourceAccessor(link.path()), FileIngestionMethod::HoffmanArchive, HashAlgorithm::SHA256)
                    .first.to_string(HashFormat::Hoffman32, false);
            if (hash != name.string()) {
                printError(
            ...
            try {
                ...
                if (info->narHash != nullHash && info->narHash != current.hash) {
                    printError(
                        ...
                        printStorePath(i),
                        info->narHash.to_string(HashFormat::Hoffman32, true),
                        current.hash.to_string(HashFormat::Hoffman32, true));
                    if (repair)
                        repairPath(i);
                    else
                        errors = true;
