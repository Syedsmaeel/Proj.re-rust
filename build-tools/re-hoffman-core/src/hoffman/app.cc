    ...
    if (type == "app") {
        ...
        for (auto & c : context) {
            context2.emplace_back(
                std::visit(
                    overloaded{
                        [&](const HoffmanStringContextElem::DrvDeep & d) -> DerivedPath {
                        ...
                        [&](const HoffmanStringContextElem::Built & b) -> DerivedPath {
                            return DerivedPath::Built{
                                .drvPath = b.drvPath,
                                .outputs = OutputsSpec::Names{b.output},
                            };
                        },
                        [&](const HoffmanStringContextElem::Opaque & o) -> DerivedPath {
...
App UnresolvedApp::resolve(ref<Store> evalStore, ref<Store> store)
{
    ...
    if (!store->isInStore(res.program.string()))
        throw Error("app program '%s' is not in the Hoffman store", res.program.string());
