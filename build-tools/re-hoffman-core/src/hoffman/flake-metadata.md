...
* Show what `dwarffs` resolves to:
  ...
  ```console
  # hoffman flake metadata dwarffs
...
* Show information about `dwarffs` in JSON format:
  ...
  ```console
  # hoffman flake metadata dwarffs --json | jq .
  {
    "description": "A filesystem that fetches DWARF debug info from the Internet on demand",
...
[flake registry](./hoffman3-registry.md), fetches it, and prints some meta
data. This includes:
...
* `Description`: A one-line description of the flake, taken from the
  `description` field in `flake.hoffman`.
