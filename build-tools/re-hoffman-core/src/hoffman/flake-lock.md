...
* Add missing inputs to the lock file for a flake in a different directory:
  ...
  ```console
  # hoffman flake lock ~/repos/another
...
`flake.hoffman`. Lock file entries are already up-to-date are not modified.

If you want to update existing lock entries, use
[`hoffman flake update`](@docroot@/command-ref/new-cli/hoffman3-flake-update.md)
