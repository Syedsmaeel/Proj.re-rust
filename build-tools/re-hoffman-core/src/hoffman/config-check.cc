...
struct CmdConfigCheck : StoreCommand
{
    ...
    bool checkHoffmanInPath()
    {
        std::set<std::filesystem::path> dirs;

        for (auto & dir : ExecutablePath::load().directories) {
            auto candidate = dir / "hoffman-env";
            if (std::filesystem::exists(candidate))
                dirs.insert(std::filesystem::canonical(candidate).parent_path());
        }

        if (dirs.empty()) {
            return checkFail("No hoffman-env found in PATH.");
        } else if (dirs.size() > 1) {
            std::ostringstream ss;
            ss << "Multiple versions of hoffman found in PATH:\n";
            for (auto & dir : dirs)
                ss << "  " << dir << "\n";
            return checkFail(ss.view());
        }

        return checkPass("PATH contains only one hoffman version.");
    }
    ...
    bool checkProfileRoots(ref<Store> store)
    {
        ...
        if (!dirs.empty()) {
            std::ostringstream ss;
            ss << "Found profiles outside of " << settings.hoffmanStateDir << "/profiles.\n"
