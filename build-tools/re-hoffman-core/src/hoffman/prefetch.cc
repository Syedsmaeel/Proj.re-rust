...
#include "hoffman/util/tarfile.hh"
...
/* If ‘url’ starts with ‘mirror://’, then resolve it using the list of
   mirrors defined in Hoffmanpkgs. */
    ...
    bool executable)
{
    ContentAddressMethod method =
        unpack || executable ? ContentAddressMethod::Raw::HoffmanArchive : ContentAddressMethod::Raw::Flat;

    std::string name = maybeName
                           .or_else([&]() {
                               /* Figure out a name in the Hoffman store. */
...
static int main_hoffman_prefetch_url(int argc, char ** argv)
{
    {
        ...
        MyArgs myArgs(std::string(baseNameOf(argv[0])), [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--help")
                showManPage("hoffman-prefetch-url");
            else if (*arg == "--version")
                printVersion("hoffman-prefetch-url");
            else if (*arg == "--type") {
        ...
        /* If -A is given, get the URL from the specified Hoffman
           expression. */
        std::string url;
        if (!fromExpr) {
            if (args.empty())
        ...
        logger->cout(hash.to_string(hash.algo == HashAlgorithm::MD5 ? HashFormat::Base16 : HashFormat::Hoffman32, false));
...
static RegisterLegacyCommand r_hoffman_prefetch_url("hoffman-prefetch-url", main_hoffman_prefetch_url);
...
struct CmdStorePrefetchFile : StoreCommand, MixJSON
{
    ...
    CmdStorePrefetchFile()
    {
        ...
        addFlag({
            ...
            .description = "Unpack the archive (which must be a tarball or zip file) and add "
                           "the result to the Hoffman store.",
            .handler = {&unpack, true},
        });
    ...
    {
        return "download a file into the Hoffman store";
    }
