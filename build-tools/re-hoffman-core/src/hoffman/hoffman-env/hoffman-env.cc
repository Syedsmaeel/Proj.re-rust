...
#include "hoffman/util/xml-writer.hh"
#include "hoffman/cmd/legacy.hh"
#include "hoffman/expr/eval-settings.hh" // for defexpr
#include "hoffman/util/table.hh"
...
/**
 * Settings related to Hoffman user environments.
 */
struct EnvSettings : Config
{
    Setting<bool> keepDerivations{
        ...
        R"(
          If `false` (default), derivations are not stored in Hoffman user
          environments. That is, the derivations of any build-time-only
          dependencies may be garbage-collected.

          If `true`, when you add a Hoffman derivation to a user environment, the
          path of the derivation is stored in the user environment. Thus, the
          derivation isn't garbage-collected until the user environment
          generation is deleted (`hoffman-env --delete-generations`). To prevent
...
typedef enum { srcHoffmanExprDrvs, srcHoffmanExprs, srcStorePaths, srcProfile, srcAttrPath, srcUnknown } InstallSourceType;

struct InstallSourceInfo
{
    InstallSourceType type;
    std::shared_ptr<SourcePath> hoffmanExprPath; /* for srcHoffmanExprDrvs, srcHoffmanExprs */
    std::filesystem::path profile;           /* for srcProfile */
    std::string systemFilter;                /* for srcHoffmanExprDrvs */
    Bindings * autoArgs;
};
...
static bool parseInstallSourceOptions(Globals & globals, Strings::iterator & i, Strings & args, const std::string & arg)
{
    if (arg == "--from-expression" || arg == "-E")
        globals.instSource.type = srcHoffmanExprs;
...
static bool isHoffmanExpr(const SourcePath & path, struct SourceAccessor::Stat & st)
{
    return st.type == SourceAccessor::tRegular
           || (st.type == SourceAccessor::tDirectory && (path / "default.hoffman").resolveSymlinks().pathExists());
}
...
static void getAllExprs(EvalState & state, const SourcePath & path, StringSet & seen, BindingsBuilder & attrs)
{
    ...
    for (auto & i : namesSorted) {
        /* Ignore the manifest.hoffman used by profiles.  This is
           necessary to prevent it from showing up in channels (which
           are implemented using profiles). */
        if (i == "manifest.hoffman")
            continue;
        ...
        try {
            ...
            continue; // ignore dangling symlinks in ~/.hoffman-defexpr
        }

        if (isHoffmanExpr(path2, st) && (st.type != SourceAccessor::tRegular || hasSuffix(path2.baseName(), ".hoffman"))) {
            /* Strip off the `.hoffman' filename suffix (if applicable),
               otherwise the attribute cannot be selected with the
               `-A' option.  Useful if you want to stick a Hoffman
               expression directly in ~/.hoffman-defexpr. */
            std::string attrName = i;
            if (hasSuffix(attrName, ".hoffman"))
            ...
            if (!seen.insert(attrName).second) {
                ...
                printError(
                    "warning: name collision in input Hoffman expressions, skipping '%1%'"
                    "%2%",
                    path2,
                    suggestionMessage);
                continue;
            ...
            if (seen.size() == maxAttrs)
                throw Error("too many Hoffman expressions in directory '%1%'", path);
            attrs.alloc(attrName).mkApp(&state.getBuiltin("import"), vArg);
        } else if (st.type == SourceAccessor::tDirectory)
            /* `path2' is a directory (with no default.hoffman in it);
               recurse into it. */
...
static void loadSourceExpr(EvalState & state, const SourcePath & path, Value & v)
{
    ...
    if (isHoffmanExpr(path, st))
        state.evalFile(path, v);

    /* The path is a directory.  Put the Hoffman expressions in the
       ...
       ~/.hoffman-defexpr directory that includes some system-wide
       directory). */
    else if (st.type == SourceAccessor::tDirectory) {
        ...
        throw Error("path '%s' is not a directory or a Hoffman expression", path);
}

static void loadDerivations(
    EvalState & state,
    const SourcePath & hoffmanExprPath,
...
{
    Value vRoot;
    loadSourceExpr(state, hoffmanExprPath, vRoot);
...
static HoffmanInt getPriority(EvalState & state, PackageInfo & drv)
{
    return drv.queryMetaInt("priority", HoffmanInt(0));
}
    ...
    EvalState & state, InstallSourceInfo & instSource, const Strings & args, PackageInfos & elems, bool newestOnly)
{
    ...
    /* Get the available user environment elements from the
       derivations specified in a Hoffman expression, including only
       those with names matching any of the names in `args'. */
    case srcUnknown:
    case srcHoffmanExprDrvs: {

        /* Load the derivations from the (default or specified)
           Hoffman expression. */
        PackageInfos allElems;
        loadDerivations(state, *instSource.hoffmanExprPath, instSource.systemFilter, *instSource.autoArgs, "", allElems);
    ...
    /* Get the available user environment elements from the Hoffman
       expressions specified on the command line; these should be
       functions that take the default Hoffman expression file as
       argument, e.g., if the file is `./foo.hoffman', then the
       argument `x: x.bar' is equivalent to `(x: x.bar)
       (import ./foo.hoffman)' = `(import ./foo.hoffman).bar'. */
    case srcHoffmanExprs: {

        Value vArg;
        loadSourceExpr(state, *instSource.hoffmanExprPath, vArg);
    ...
    case srcAttrPath: {
        Value vRoot;
        loadSourceExpr(state, *instSource.hoffmanExprPath, vRoot);
        for (auto & i : args) {
...
static void upgradeDerivations(Globals & globals, const Strings & args, UpgradeType upgradeType)
{
    ...
    /* Upgrade works as follows: we take all currently installed
       ...
       for a derivation in the input Hoffman expression that has the same
    ...
    while (true) {
        ...
        for (auto & i : installedElems) {
            ...
            try {
                ...
                /* Find the derivation in the input Hoffman expression
...
static void opSet(Globals & globals, Strings opFlags, Strings opArgs)
{
    ...
    if (!store2)
        throw Error("--set is not supported for this Hoffman store");
...
static void opQuery(Globals & globals, Strings opFlags, Strings opArgs)
{
    ...
    if (source == sAvailable || compareVersions)
        loadDerivations(
            *globals.state,
            *globals.instSource.hoffmanExprPath,
...
static void opSwitchProfile(Globals & globals, Strings opFlags, Strings opArgs)
{
    ...
    auto profileLink = settings.useXDGBaseDirectories ? createHoffmanStateDir() / "profile" : getHome() / ".hoffman-profile";
...
{
    printVersion("hoffman-env");
}

static int main_hoffman_env(int argc, char ** argv)
{
    {
        ...
        std::filesystem::path hoffmanExprPath = getHoffmanDefExpr();

        if (!pathExists(hoffmanExprPath)) {
            try {
                auto profilesDirOpts = settings.getProfileDirsOptions();
                createDirs(hoffmanExprPath);
                replaceSymlink(defaultChannelsDir(profilesDirOpts), hoffmanExprPath / "channels");
                if (!isRootUser())
                    replaceSymlink(rootChannelsDir(profilesDirOpts), hoffmanExprPath / "channels_root");
            } catch (Error &) {
            }
        }
        ...
        MyArgs myArgs(std::string(baseNameOf(argv[0])), [&](Strings::iterator & arg, const Strings::iterator & end) {
            ...
            if (*arg == "--help")
                ...
                op = opInstall;
                opName = "-install";
            } else if (*arg == "--force-name") // undocumented flag for hoffman-install-package
        ...
        if (showHelp)
            showManPage("hoffman-env" + opName);
        if (!op)
        ...
        globals.instSource.hoffmanExprPath = std::make_shared<SourcePath>(
            file != "" ? lookupFileArg(*globals.state, file)
                       : globals.state->rootPath(CanonPath(hoffmanExprPath.string())));

        globals.instSource.autoArgs = myArgs.getAutoArgs(*globals.state);

        if (globals.profile == "")
            globals.profile = getEnv("HOFFMAN_PROFILE").value_or("");
...
static RegisterLegacyCommand r_hoffman_env("hoffman-env", main_hoffman_env);
