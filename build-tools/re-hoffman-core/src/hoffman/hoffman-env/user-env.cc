...
PackageInfos queryInstalled(EvalState & state, const std::filesystem::path & userEnv)
{
    ...
    if (pathExists(userEnv / "manifest.json"))
        throw Error("profile %s is incompatible with 'hoffman-env'; please use 'hoffman profile' instead", PathFmt(userEnv));
    auto manifestFile = userEnv / "manifest.hoffman";
    ...
    for (const auto & [n, i] : enumerate(elems)) {
        ...
        for (const auto & [m, j] : enumerate(outputs)) {
            ...
            /* This is only necessary when installing store paths, e.g.,
               `hoffman-env -i /hoffman/store/abcd...-foo'. */
...
#include "buildenv.hoffman.gen.hh"
            , state.rootPath(CanonPath::root)),
        envBuilder);

    /* Construct a Hoffman expression that calls the user environment
