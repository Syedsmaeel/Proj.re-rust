...
static std::filesystem::path hoffmanDefExpr;

// Fetch Hoffman expressions and binary cache URLs from the subscribed channels.
static void update(const StringSet & channelNames)
{
    ...
    writeFull(
        fd.get(),
#include "unpack-channel.hoffman.gen.hh"
    );
    fd = -1;
    ...
    for (const auto & channel : channels) {
        ...
        // attribute (so that "hoffman-env -q" on the channels profile
        ...
        if (!(channelNames.empty() || channelNames.count(name))) {
            ...
            // got redirected in the process, so that we can grab the various parts of a hoffman channel
            ...
            if (!unpacked) {
                ...
                try {
                    result = fetchers::downloadFile(*store, fetchSettings, url + "/hoffmanexprs.tar.xz", "hoffmanexprs.tar.xz");
                } catch (FileTransferError & e) {
                    result =
                        fetchers::downloadFile(*store, fetchSettings, url + "/hoffmanexprs.tar.bz2", "hoffmanexprs.tar.bz2");
                }
            }
    ...
    // Unpack the channel tarballs into the Hoffman store and install them
    ...
    runProgram(getHoffmanBin("hoffman-env"), false, envArgs);

    // Make the channels appear in hoffman-env.
    PosixStat st;
    if (lstat(hoffmanDefExpr.c_str(), &st) == 0) {
        if (S_ISLNK(st.st_mode))
            // old-skool ~/.hoffman-defexpr
            unlink(hoffmanDefExpr);
    } else if (errno != ENOENT) {
        throw SysError("getting status of %1%", PathFmt(hoffmanDefExpr));
    }
    createDirs(hoffmanDefExpr);
    auto channelLink = hoffmanDefExpr / "channels";
    replaceSymlink(profile, channelLink);
}

static int main_hoffman_channel(int argc, char ** argv)
{
    {
        // Figure out the name of the `.hoffman-channels' file to use
        auto home = getHome();
        channelsList =
            settings.useXDGBaseDirectories ? (createHoffmanStateDir() / "channels").string() : home + "/.hoffman-channels";
        hoffmanDefExpr = getHoffmanDefExpr();
        ...
        parseCmdLine(argc, argv, [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--help") {
                showManPage("hoffman-channel");
            } else if (*arg == "--version") {
                printVersion("hoffman-channel");
            } else if (*arg == "--add") {
                cmd = cAdd;
            } else if (*arg == "--remove") {
                cmd = cRemove;
...
static RegisterLegacyCommand r_hoffman_channel("hoffman-channel", main_hoffman_channel);
