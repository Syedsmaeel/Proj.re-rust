            ...
            .handler = {[&](std::vector<std::string> inputsToUpdate) {
                for (const auto & inputToUpdate : inputsToUpdate) {
                    ...
                    try {
                        ...
                        warn(
                            "Invalid flake input '%s'. To update a specific flake, use 'hoffman flake update --flake %s' instead.",
                            inputToUpdate,
                            inputToUpdate);
                        throw e;
                    }
...
struct CmdFlakeInfo : CmdFlakeMetadata
{
    void run(hoffman::ref<hoffman::Store> store) override
    {
        warn("'hoffman flake info' is a deprecated alias for 'hoffman flake metadata'");
...
#include "flake-check.md"
    ...
    void run(hoffman::ref<hoffman::Store> store) override
    {
        ...
        if (build && !attrPathsByDrv.empty()) {
            ...
            // via substitution, as `hoffman flake check` only needs to verify buildability,
...
struct CmdFlakePrefetch : FlakeCommand, MixJSON
{
    ...
    std::string description() override
    {
        return "download the source tree denoted by a flake reference into the Hoffman store";
    }
...
struct CmdFlake : HoffmanMultiCommand
{
    CmdFlake()
        : HoffmanMultiCommand("flake", RegisterCommand::getCommandsFor({"flake"}))
    {
    }

    std::string description() override
    {
        return "manage Hoffman flakes";
    }
