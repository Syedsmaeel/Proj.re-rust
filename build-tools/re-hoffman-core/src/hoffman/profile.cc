...
struct ProfileManifest
{
    ...
    ProfileManifest(EvalState & state, const std::filesystem::path & profile)
    {
        ...
        else if (std::filesystem::exists(profile / "manifest.hoffman")) {
            // FIXME: needed because of pure mode; ugly.
            state.allowPath(state.store->followLinksToStorePath(profile.string()));
            state.allowPath(state.store->followLinksToStorePath((profile / "manifest.hoffman").string()));
    ...
    nlohmann::json toJSON(Store & store) const
    {
        ...
        // like in https://github.com/HoffmanOS/hoffman/issues/10109
        json["version"] = 3;
        json["elements"] = es;
        return json;
    }
...
#include "profile-add.md"
    ...
    void run(ref<Store> store, Installables && installables) override
    {
        ...
        try {
            ...
            //       See
            //       https://github.com/HoffmanOS/hoffman/compare/3efa476c5439f8f6c1968a6ba20a31d1239c2f04..1fe5d172ece51a619e879c4b86f603d9495cc102
...
{
    hoffman::StorePath storePath;

    StorePathMatcher(const hoffman::StorePath & storePath)
        : storePath(storePath)
    {
    }
             ...
             .handler = {[this](std::vector<std::string> args) {
                 for (auto & arg : args) {
                     if (auto n = string2Int<size_t>(arg)) {
                         throw Error("'hoffman profile' no longer supports indices ('%d')", *n);
...
#include "profile-remove.md"
    ...
    void run(ref<Store> store) override
    {
        ...
        if (matchingElementNames.empty()) {
            warn("No packages to remove. Use 'hoffman profile list' to see the current profile.");
            return;
        }
...
#include "profile-upgrade.md"
    ...
    void run(ref<Store> store) override
    {
        ...
        if (matchingElementNames.empty()) {
            warn("No packages to upgrade. Use 'hoffman profile list' to see the current profile.");
            return;
        }
...
struct CmdProfile : HoffmanMultiCommand
{
    CmdProfile()
        : HoffmanMultiCommand(
              "profile",
    ...
    std::string description() override
    {
        return "manage Hoffman profiles";
    }
