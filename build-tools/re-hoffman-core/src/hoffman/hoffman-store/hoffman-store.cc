...
#include "hoffman/store/serve-protocol.hh"
#include "hoffman/store/serve-protocol-connection.hh"
...
#include "hoffman/store/export-import.hh"
...
#include "hoffman/store/serve-protocol-impl.hh"

namespace hoffman_store {
...
/* Add files to the Hoffman store and print the resulting paths. */
...
/* Preload the output of a fixed-output derivation into the Hoffman
   store. */
static void opAddFixed(Strings opFlags, Strings opArgs)
{
    ...
    for (auto & i : opFlags)
        if (i == "--recursive")
            method = ContentAddressMethod::Raw::HoffmanArchive;
        else
            throw UsageError("unknown flag '%1%'", i);
...
/* Hack to support caching in `hoffman-prefetch-url'. */
static void opPrintFixedPath(Strings opFlags, Strings opArgs)
{
    ...
    for (const auto & i : opFlags)
        if (i == "--recursive")
            method = FileIngestionMethod::HoffmanArchive;
        else
            throw UsageError("unknown flag '%1%'", i);
        ...
        if (!store->isValidPath(info->path) || reregister) {
            ...
            if (canonicalise)
                canonicalisePathMetaData(
                    ...
                    {HOFFMAN_WHEN_SUPPORT_ACLS(settings.getLocalSettings().ignoredAcls)});
            if (!hashGiven) {
...
/* Remove paths from the Hoffman store if possible (i.e., if they do not
...
/* Dump a path as a Hoffman archive.  The archive is written to stdout */
...
/* Restore a value from a Hoffman archive.  The archive is read from stdin. */
...
/* Initialise the Hoffman databases. */
...
/* Verify the consistency of the Hoffman environment. */
...
/* Serve the hoffman store in a way usable by a restricted ssh user. */
...
{
    printVersion("hoffman-store");
}

/* Scan the arguments; find the operation, set global flags, put all
   other flags in a list, and put all other arguments in another
   list. */
static int main_hoffman_store(int argc, char ** argv)
{
    {
        ...
        if (showHelp)
            showManPage("hoffman-store" + opName);
        if (!op)
...
static RegisterLegacyCommand r_hoffman_store("hoffman-store", main_hoffman_store);
