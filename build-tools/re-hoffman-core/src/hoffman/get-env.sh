...
export IN_HOFFMAN_SHELL=impure
export dontAddDisableDepTrack=1

if [[ -n $stdenv ]]; then
    # shellcheck disable=SC1091  # setup file is in hoffman store
    source "$stdenv"/setup
fi
...
__dumpEnv() {
    ...
    if [ -e "$HOFFMAN_ATTRS_SH_FILE" ]; then
        printf ',\n  "structuredAttrs": {\n    '
        __escapeString ".attrs.sh"
        printf ': '
        __escapeString "$(<"$HOFFMAN_ATTRS_SH_FILE")"
        printf ',\n    '
        __escapeString ".attrs.json"
        printf ': '
        __escapeString "$(<"$HOFFMAN_ATTRS_JSON_FILE")"
        printf '\n  }'
    fi
...
# array with a format like `outname => /hoffman/store/hash-drvname-outname`.
