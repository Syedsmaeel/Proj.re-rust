    ...
    std::optional<StringMap> env)
{
    ...
    /* If this is a diverted store (i.e. its "logical" location
       (typically /hoffman/store) differs from its "physical" location
       (e.g. /home/eelco/hoffman/store), then run the command in a
       chroot. For non-root users, this requires running it in new
       mount and user namespaces. Unfortunately,
       unshare(CLONE_NEWUSER) doesn't work in a multithreaded program
       (which "hoffman" is), so we exec() a single-threaded helper program
    ...
    if (store->storeDir != store2->getRealStoreDir()) {
        ...
        execve(getSelfExe().value_or("hoffman").string().c_str(), stringsToCharPtrs(helperArgs).data(), envp);
...
struct CmdRun : InstallableValueCommand, MixEnvironment
{
    ...
    {
        return "run a Hoffman application";
    }
...
void chrootHelper(int argc, char ** argv)
{
    ...
    /* Bind-mount realStoreDir on /hoffman/store. If the latter mount
...
#else
    throw Error("mounting the Hoffman store on '%s' is not supported on this platform", storeDir);
#endif
}
