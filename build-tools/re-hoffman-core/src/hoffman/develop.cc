...
{
    Setting<std::string> bashPrompt{this, "", "bash-prompt", "The bash prompt (`PS1`) in `hoffman develop` shells."};

    Setting<std::string> bashPromptPrefix{
        this, "", "bash-prompt-prefix", "Prefix prepended to the `PS1` environment variable in `hoffman develop` shells."};

    Setting<std::string> bashPromptSuffix{
        this, "", "bash-prompt-suffix", "Suffix appended to the `PS1` environment variable in `hoffman develop` shells."};
};
...
static StorePath getDerivationEnvironment(ref<Store> store, ref<Store> evalStore, const StorePath & drvPath)
{
    ...
    if (builder != "bash")
        throw Error("'hoffman develop' only works on derivations that use 'bash' as their builder");
...
struct Common : InstallableCommand, MixProfile
{
        ...
        const std::filesystem::path & outputsDir = std::filesystem::path{std::filesystem::current_path()} / "outputs")
    {
        ...
        for (auto & var : savedVars) {
            out << fmt("%s=${%s:-}\n", var, var);
            out << fmt("hoffman_saved_%s=\"$%s\"\n", var, var);
        }

        buildEnvironment.toBash(out, ignoreVars);

        for (auto & var : savedVars)
            out << fmt("%s=\"$%s${hoffman_saved_%s:+:$hoffman_saved_%s}\"\n", var, var, var, var);

        out << "export HOFFMAN_BUILD_TOP=\"$(mktemp -d -t hoffman-shell.XXXXXX)\"\n";
        for (auto & i : {"TMP", "TMPDIR", "TEMP", "TEMPDIR"})
            out << fmt("export %s=\"$HOFFMAN_BUILD_TOP\"\n", i);
        ...
        if (buildEnvironment.providesStructuredAttrs()) {
            fixupStructuredAttrs(
                OS_STR("sh"), "HOFFMAN_ATTRS_SH_FILE", buildEnvironment.getAttrsSH(), rewrites, buildEnvironment, tmpDir);
            fixupStructuredAttrs(
                OS_STR("json"),
                "HOFFMAN_ATTRS_JSON_FILE",
    ...
    /**
     * Replace the value of HOFFMAN_ATTRS_*_FILE (`/build/.attrs.*`) with a tmp file
...
#include "develop.md"
    ...
    void run(ref<Store> store, ref<Installable> installable) override
    {
        ...
        auto [rcFileFd, rcFilePath] = createTempFile("hoffman-shell");

        AutoDelete tmpDir(createTempDir("", "hoffman-develop"), true);
        ...
        setEnv("HOFFMAN_GCROOT", store->printStorePath(gcroot).c_str());
        ...
        try {
            ...
            auto hoffmanpkgsLockFlags = lockFlags;
            ...
            if (!foundInteractive)
                throw Error("package 'hoffmanpkgs#bashInteractive' does not provide a 'bin/bash'");
        ...
        /* See: https://github.com/HoffmanOS/hoffman/issues/5873
...
#include "print-dev-env.md"
    ...
    void run(ref<Store> store, ref<Installable> installable) override
    {
        ...
        if (json) {
            ...
            AutoDelete tmpDir(createTempDir("", "hoffman-dev-env"), true);
