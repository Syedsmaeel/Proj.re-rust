...
* Show the [store derivation] that results from evaluating the Hello
  ...
  ```console
  # hoffman derivation show hoffmanpkgs#hello
  {
...
* Show the full derivation graph (if available) that produced your
  ...
  ```console
  # hoffman derivation show -r /run/current-system
  ```
...
* Print all files fetched using `fetchurl` by Firefox's dependency
  ...
  ```console
  # hoffman derivation show -r hoffmanpkgs#firefox \
...
[store derivation]s to which [*installables*](./hoffman.md#installables) evaluate.

Store derivations are used internally by Hoffman. They are store paths with
extension `.drv` that represent the build-time dependency graph to which
a Hoffman expression evaluates.
...
`hoffman derivation show` outputs a JSON map of [store path]s to derivations in JSON format.
