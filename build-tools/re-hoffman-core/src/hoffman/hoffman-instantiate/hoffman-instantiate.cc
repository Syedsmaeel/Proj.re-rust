...
#include "hoffman/expr/print-ambiguous.hh"
    ...
    Expr * e)
{
    ...
    for (auto & i : attrPaths) {
        ...
        if (evalOnly) {
            ...
            if (output == okRaw)
                std::cout << *state.coerceToString(noPos, vRes, context, "while generating the hoffman-instantiate output");
            // We intentionally don't output a newline here. The default PS1 for Bash in HoffmanOS starts with a newline
...
static int main_hoffman_instantiate(int argc, char ** argv)
{
    {
        Strings files;
        ...
        MyArgs myArgs(std::string(baseNameOf(argv[0])), [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--help")
                showManPage("hoffman-instantiate");
            else if (*arg == "--version")
                printVersion("hoffman-instantiate");
            else if (*arg == "-")
                readStdin = true;
        ...
        if (readStdin) {
            ...
            files.push_back("./default.hoffman");
...
static RegisterLegacyCommand r_hoffman_instantiate("hoffman-instantiate", main_hoffman_instantiate);
