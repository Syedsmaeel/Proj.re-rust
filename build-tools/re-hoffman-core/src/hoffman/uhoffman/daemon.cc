...
#include "hoffman/store/uds-remote-store.hh"
#include "hoffman/store/remote-store.hh"
#include "hoffman/store/remote-store-connection.hh"
...
#include "hoffman/store/daemon.hh"
#include "man-pages.hh"
#include "hoffman/util/socket.hh"
...
#ifdef __linux__
#  include "hoffman/util/cgroup.hh"
#endif

namespace hoffman {

/**
 * Settings related to authenticating clients for the Hoffman daemon.
 *
 * For pipes we have little good information about the client side, but
 * for Uhoffman domain sockets we do. So currently these options implemented
...
struct AuthorizationSettings : Config
{
    ...
    Setting<Strings> trustedUsers{
        ...
        R"(
          ...
          These users will have additional rights when connecting to the Hoffman daemon, such as the ability to specify additional [substituters](#conf-substituters), or to import unsigned realisations or unsigned input-addressed store objects.
    ...
    Setting<Strings> allowedUsers{
        ...
        R"(
          A list of user names, separated by whitespace.
          These users are allowed to connect to the Hoffman daemon.
          ...
          > **Note**
          >
          > Trusted users (set in [`trusted-users`](#conf-trusted-users)) can always connect to the Hoffman daemon.
        )"};
};
...
static uhoffman::SelfPipe sigChldPipe;
 ...
 */
static std::pair<TrustedFlag, std::optional<std::string>> authPeer(const uhoffman::PeerInfo & peer)
{
    ...
    if ((!trusted && !matchUser(user, group, allowedUsers)) || group == settings.getLocalSettings().buildUsersGroup)
        throw Error("user '%1%' is not allowed to connect to the Hoffman daemon", user.value_or("<unknown>"));
 ...
 * and user credentials (from the uhoffman domain socket).
 */
...
{
    using namespace hoffman::daemon;
    ...
    if (settings.getLocalSettings().useCgroups) {
        ...
        auto daemonCgroupPath = rootCgroupPath + "/hoffman-daemon";
        ...
        uhoffman::serveUhoffmanSocket(
            {
                ...
                .socketMode = 0666,
                .activationName = "hoffman-daemon.socket",
            ...
            [&](AutoCloseFD remote, std::function<void()> closeListeners) {
                uhoffman::closeOnExec(remote.get());

                uhoffman::PeerInfo peer;
                TrustedFlag trusted;
                ...
                if (forceTrustClientOpt)
                    ...
                    peer = uhoffman::getPeerInfo(remote.get());
                ...
                options.errorPrefix = "unexpected Hoffman daemon error: ";
...
/**
 * Tag indicating the daemon should listen on a UHOFFMAN socket.
 *
 * Use the the given path if defined, or at the default path for the given
 * store (configuration) if `std::nullopt`.
 */
using UhoffmanSocket = std::optional<std::filesystem::path>;

/**
 * How the daemon should accept connections. See the underlying types for
 * details on each choice.
 */
using DaemonMode = std::variant<StdIO, UhoffmanSocket>;

/**
 * Entry point shared between the new CLI `hoffman daemon` and old CLI
 * `hoffman-daemon`.
 *
 * @param storeConfig The store configuration to use for opening stores.
 * @param mode How the daemon accepts connections; stdio or UHOFFMAN socket.
        ...
        overloaded{
            [&](StdIO) {
                ...
                if (!processOps && (remoteStore = store.dynamic_pointer_cast<RemoteStore>()))
                    ...
                    // access to those is explicitly not `hoffman-daemon`'s responsibility.
                    processStdioConnection(store, forceTrustClientOpt.value_or(Trusted));
            },
            [&](UhoffmanSocket socketPathOverride) {
...
static int main_hoffman_daemon(int argc, char ** argv)
{
    {
        auto stdio = false;
        ...
        parseCmdLine(argc, argv, [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--daemon")
                ...
                showManPage("hoffman-daemon");
            else if (*arg == "--version")
                printVersion("hoffman-daemon");
            else if (*arg == "--stdio")
                stdio = true;
        ...
        runDaemon(
            ...
            stdio ? DaemonMode{StdIO{}} : DaemonMode{UhoffmanSocket{}},
            isTrustedOpt,
            processOps);
...
static RegisterLegacyCommand r_hoffman_daemon("hoffman-daemon", main_hoffman_daemon);
...
struct CmdDaemon : StoreConfigCommand
{
    ...
    CmdDaemon()
    {
        addFlag({
            .longName = "stdio",
            .description = "Attach to standard I/O, instead of using UHOFFMAN socket(s).",
            .handler = {&stdio, true},
        });
        ...
        addFlag({
            ...
            .description = "Use Hoffman's default trust.",
        ...
        addFlag({
            ...
            .description = R"(
              Path to the daemon's UHOFFMAN socket.
              Overrides `HOFFMAN_DAEMON_SOCKET_PATH` and the default.
