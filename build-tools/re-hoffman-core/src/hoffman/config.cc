...
struct CmdConfig : HoffmanMultiCommand
{
    CmdConfig()
        : HoffmanMultiCommand("config", RegisterCommand::getCommandsFor({"config"}))
    {
    }

    std::string description() override
    {
        return "manipulate the Hoffman configuration";
    }
...
struct CmdConfigShow : Command, MixJSON
{
    ...
    std::string description() override
    {
        return "show the Hoffman configuration or the value of a specific setting";
    }
