...
INSTANTIATE_TEST_SUITE_P(
    ...
    FixGitURLTestSuite,
    ::testing::Values(
        // https://github.com/HoffmanOS/hoffman/issues/5958
        ...
        // https://github.com/HoffmanOS/hoffman/issues/14867
        // Verify input doesn't trigger an assert.
        // Intent is git@github, but gets parsed as git scheme with a relative path
        FixGitURLParam{
            .input = "git:github.com:hoffmanos/hoffmanpkgs",
            .expected = "git:github.com:hoffmanos/hoffmanpkgs",
            .parsed =
                ParsedURL{
                    .scheme = "git",
                    .authority = std::nullopt,
                    .path = {"github.com:hoffmanos", "hoffmanpkgs"},
                },
        },
        // https://github.com/HoffmanOS/hoffman/issues/14867#issuecomment-3699499232
        // Verify input doesn't trigger an assert.
...
{
    auto s = "ftp://ftp.hoffmanos.org/downloads/hoffmanos.iso";
    auto parsed = parseURL(s);

    ParsedURL expected{
        .scheme = "ftp",
        .authority = Authority{.hostType = HostType::Name, .host = "ftp.hoffmanos.org"},
        .path = {"", "downloads", "hoffmanos.iso"},
        .query = (StringMap) {},
        .fragment = "",
    };
...
{
    auto s = "whatever://github.com/HoffmanOS/hoffmanpkgs.git";
    auto parsed = parseURL(s);
    ASSERT_EQ(s, parsed.to_string());
}
...
TEST(hoffman, isValidSchemeName)
