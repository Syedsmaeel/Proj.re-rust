...
protected:
    ...
    std::unique_ptr<hoffman::AutoDelete> delTmpDir;

    void SetUp() override
    {
        tmpDir = hoffman::createTempDir();
        delTmpDir = std::make_unique<hoffman::AutoDelete>(tmpDir, true);
    }
...
{
#ifdef _WIN32
    GTEST_SKIP() << "fd-based accessor is Uhoffman-only";
#endif
    auto accessor = makeFSSourceAccessor(tmpDir);
