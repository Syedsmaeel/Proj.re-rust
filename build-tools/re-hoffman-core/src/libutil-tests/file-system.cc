...
{
    ASSERT_EQ(isDirOrInDir(FS_ROOT "hoffman", FS_ROOT "hoffman"), true);
...
{
#ifdef _WIN32
    // Windows doesn't support Uhoffman-style permission bits - lstat always
    // returns the same mode regardless of what chmod sets.
    GTEST_SKIP() << "Broken on Windows";
#endif
    auto [autoClose_, tmpFile] = hoffman::createTempFile();
...
TEST(DirectoryIterator, works)
{
    ...
    hoffman::writeFile(tmpDir / "somefile", "");
...
TEST(createTempDir, works)
{
    ...
    hoffman::AutoDelete delTmpDir(tmpDir, /*recursive=*/true);
