...
#include "hoffman/util/tests/gtest-with-params.hh"
...
struct Arbitrary<hoffman::checked::Checked<T>>
{
    static Gen<hoffman::checked::Checked<T>> arbitrary()
    {
...
void checkDivision(TSmall a_, TSmall b)
{
    ...
    if (std::is_signed<TSmall>() && a_ == minV && b == -1) {
        ...
        RC_ASSERT_THROWS_AS(result.valueWrapping(), hoffman::checked::DivideByZero);
