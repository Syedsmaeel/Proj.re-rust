...
This script handles:
- Loading the custom hoffman-clang-tidy plugin
- Setting up header filters to only check project headers
- Managing parallelism based on available CPUs
- Working around run-clang-tidy's unpatched shebang (HoffmanOS/hoffmanpkgs#214945)
"""
...
def default_concurrency():
    return min(
        multiprocessing.cpu_count(), int(os.environ.get("HOFFMAN_BUILD_CORES", "16"))
    )
...
def go(
    ...
    args = [
        ...
        # shebang (see HoffmanOS/hoffmanpkgs#214945), which fails in sandboxed builds where
    ...
    args += [
        ...
        "-j",
        str(jobs),
        # Lint all project headers (public include/hoffman/*/ and private *.hh).
        # Project headers are reached via relative -I flags; system/installed
        # headers are absolute /hoffman/store/... paths. Match any non-absolute path
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Run clang-tidy on the Hoffman codebase")
    ap.add_argument(
        "--jobs",
        "-j",
        type=int,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to the hoffman-clang-tidy plugin",
    )
    ap.add_argument(
        "--config-file",
        type=Path,
