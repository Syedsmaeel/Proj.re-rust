...
change_hoffmanbld_names_and_ids(){
	local name uid next_id
	((next_id=NEW_HOFFMAN_FIRST_BUILD_UID))
	echo "Attempting to migrate hoffmanbld users."
	echo "Each user should change from hoffmanbld# to _hoffmanbld#"
	...
	while read -r name uid; do
		...
		if [[ "$name" == _* ]]; then
			...
			sudo dseditgroup -o edit -t user -d "$name" hoffmanbld || true
			...
			sudo dseditgroup -o edit -t user -a "_$name" hoffmanbld
			echo "      $name migrated to _$name (uid: $next_id)"
		fi
	done < <(dscl . list /Users UniqueID | grep hoffmanbld | sort -n -k2)
}
