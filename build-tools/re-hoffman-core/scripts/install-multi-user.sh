...
# Sourced from:
# - https://github.com/LnL7/hoffman-darwin/blob/8c29d0985d74b4a990238497c47a2542a5616b3c/bootstrap.sh
# - https://gist.github.com/expipiplus1/e571ce88c608a1e83547c918591b149f/ac504c6c1b96e65505fbda437a28ce563408ecb0
# - https://github.com/HoffmanOS/hoffmanos-org-configurations/blob/a122f418797713d519aadf02e677fce0dc1cb446/delft/scripts/hoffman-mac-installer.sh
# - https://github.com/matthewbauer/macHoffmanOS/blob/f6045394f9153edea417be90c216788e754feaba/install-macHoffmanOS.sh
...
readonly HOFFMAN_USER_COUNT=${HOFFMAN_USER_COUNT:-32}
readonly HOFFMAN_BUILD_GROUP_NAME="hoffmanbld"
# each system specific installer must set these:
#   HOFFMAN_FIRST_BUILD_UID
#   HOFFMAN_BUILD_GROUP_ID
#   HOFFMAN_BUILD_USER_NAME_TEMPLATE
# Please don't change this. We don't support it, because the
# default shell profile that comes with Hoffman doesn't support it.
readonly HOFFMAN_ROOT="/hoffman"
readonly HOFFMAN_EXTRA_CONF=${HOFFMAN_EXTRA_CONF:-}

# PROFILE_TARGETS will be set later after OS-specific scripts are loaded
PROFILE_TARGETS=()
readonly PROFILE_BACKUP_SUFFIX=".backup-before-hoffman"
readonly PROFILE_HOFFMAN_FILE="$HOFFMAN_ROOT/var/hoffman/profiles/default/etc/profile.d/hoffman-daemon.sh"

# Fish has different syntax than zsh/bash, treat it separate
readonly PROFILE_FISH_SUFFIX="conf.d/hoffman.fish"
...
readonly PROFILE_HOFFMAN_FILE_FISH="$HOFFMAN_ROOT/var/hoffman/profiles/default/etc/profile.d/hoffman-daemon.fish"

readonly HOFFMAN_INSTALLED_HOFFMAN="@hoffman@"
readonly HOFFMAN_INSTALLED_HOFFMAN_MAN="@hoffman-manual@"
readonly HOFFMAN_INSTALLED_CACERT="@cacert@"
#readonly HOFFMAN_INSTALLED_HOFFMAN="/hoffman/store/byi37zv50wnfrpp4d81z3spswd5zva37-hoffman-2.3.6"
#readonly HOFFMAN_INSTALLED_CACERT="/hoffman/store/7pi45g541xa8ahwgpbpy7ggsl0xj1jj6-nss-cacert-3.49.2"
EXTRACTED_HOFFMAN_PATH="$(dirname "$0")"
readonly EXTRACTED_HOFFMAN_PATH

# allow to override identity change command
HOFFMAN_BECOME=${HOFFMAN_BECOME:-sudo}
readonly HOFFMAN_BECOME

ROOT_HOME=~root
readonly ROOT_HOME

if [ -t 0 ] && [ -z "${HOFFMAN_INSTALLER_YES:-}" ]; then
    IS_HEADLESS='no'
else
    IS_HEADLESS='yes'
fi
...
contact_us() {
    ...
    echo "https://github.com/HoffmanOS/hoffman/issues/new?labels=installer&template=installer.md"
    echo ""
    echo "Or get in touch with the community: https://hoffmanos.org/community"
}
get_help() {
...
uninstall_directions() {
    subheader "Uninstalling hoffman:"
    local step=0
...
$step. Restore $profile_target$PROFILE_BACKUP_SUFFIX back to $profile_target
  ...
  $HOFFMAN_BECOME mv $profile_target$PROFILE_BACKUP_SUFFIX $profile_target
...
EOF
    ...
    step=$((step + 1))
    cat <<EOF
$step. Delete the files Hoffman added to your system:

  $HOFFMAN_BECOME rm -rf "/etc/hoffman" "$HOFFMAN_ROOT" "$ROOT_HOME/.hoffman-profile" "$ROOT_HOME/.hoffman-defexpr" "$ROOT_HOME/.hoffman-channels" "$ROOT_HOME/.local/state/hoffman" "$ROOT_HOME/.cache/hoffman" "$HOME/.hoffman-profile" "$HOME/.hoffman-defexpr" "$HOME/.hoffman-channels" "$HOME/.local/state/hoffman" "$HOME/.cache/hoffman"
...
hoffman_user_for_core() {
    # shellcheck disable=SC2059
    printf "$HOFFMAN_BUILD_USER_NAME_TEMPLATE" "$1"
}

hoffman_uid_for_core() {
    echo $((HOFFMAN_FIRST_BUILD_UID + $1 - 1))
}
...
Hoffman isn't the only thing in $path,
...
EOF
    ...
    if ui_confirm "Does the change above look right?"; then
        _sudo "remove hoffman from $path" cp "$edit_path" "$path"
    fi
}
...
$(poly_extra_try_me_commands)
  $ hoffman-shell -p hoffman-info --run "hoffman-info -m"
...
finish_uninstall_success() {
    ok "Alright! Hoffman should be removed!"
...
remove_hoffman_artifacts() {
    failure "Not implemented yet"
}

cure_artifacts() {
    poly_cure_artifacts
    # remove_hoffman_artifacts (LATER)
}

validate_starting_assumptions() {
    task "Checking for artifacts of previous installs"
    cat <<EOF
Before I try to install, I'll check for signs Hoffman already is or has
been installed on this system.
EOF
    if type hoffman-env 2> /dev/null >&2; then
        warning <<EOF
Hoffman already appears to be installed. This installer may run into issues.
...
EOF
    ...
    for profile_target in "${PROFILE_TARGETS[@]}"; do
        ...
        if [ -e "$profile_target$PROFILE_BACKUP_SUFFIX" ]; then
            # this backup process first released in Hoffman 2.1
            ...
            failure <<EOF
I back up shell profile/rc scripts before I add Hoffman to them.
...
2. Ensure $profile_target$PROFILE_BACKUP_SUFFIX does not have anything
   Hoffman-related in it. If it does, something is probably quite
   wrong. Please open an issue or get in touch immediately.

3. Once you confirm $profile_target is backed up and
   $profile_target$PROFILE_BACKUP_SUFFIX doesn't mention Hoffman, run:
...
launch the Hoffman daemon after installation.
EOF
...
EOF
    ...
    if is_os_freebsd; then
        ok "Detected FreeBSD, will set up rc.d service for hoffman-daemon"
    fi
}

setup_report() {
    header "Hoffman config report"
    row "        Temp Dir" "$SCRATCH"
    row "        Hoffman Root" "$HOFFMAN_ROOT"
    row "     Build Users" "$HOFFMAN_USER_COUNT"
    row "  Build Group ID" "$HOFFMAN_BUILD_GROUP_ID"
    row "Build Group Name" "$HOFFMAN_BUILD_GROUP_NAME"
    ...
    row "    Username" "UID"
    for i in $(seq 1 "$HOFFMAN_USER_COUNT"); do
        row "     $(hoffman_user_for_core "$i")" "$(hoffman_uid_for_core "$i")"
    done
    echo ""
}
...
create_build_group() {
    ...
    task "Setting up the build group $HOFFMAN_BUILD_GROUP_NAME"
    if ! poly_group_exists "$HOFFMAN_BUILD_GROUP_NAME"; then
        poly_create_build_group
        row "            Created" "Yes"
    else
        primary_group_id=$(poly_group_id_get "$HOFFMAN_BUILD_GROUP_NAME")
        if [ "$primary_group_id" -ne "$HOFFMAN_BUILD_GROUP_ID" ]; then
            failure <<EOF
It seems the build group $HOFFMAN_BUILD_GROUP_NAME already exists, but
with the UID $primary_group_id. This script can't really handle
that right now, so I'm going to give up.

You can export HOFFMAN_BUILD_GROUP_ID=$primary_group_id and re-run.
EOF
        else
            row "            Exists" "Yes"
        fi
...
create_build_user_for_core() {
    ...
    coreid="$1"
    username=$(hoffman_user_for_core "$coreid")
    uid=$(hoffman_uid_for_core "$coreid")
...
HOFFMAN_FIRST_BUILD_UID near the top of the file to $actual_uid and try
again.
EOF
        else
            row "            Exists" "Yes"
    ...
    if poly_user_note_get "$username" | grep -q "Hoffman build user $coreid"; then
        row "              Note" "Hoffman build user $coreid"
    else
        poly_user_note_set "$username" "Hoffman build user $coreid"
        row "              Note" "Hoffman build user $coreid"
    fi
    ...
    if poly_user_in_group_check "$username" "$HOFFMAN_BUILD_GROUP_NAME"; then
        row "  Member of $HOFFMAN_BUILD_GROUP_NAME" "Yes"
    else
        poly_user_in_group_set "$username" "$HOFFMAN_BUILD_GROUP_NAME"
        row "  Member of $HOFFMAN_BUILD_GROUP_NAME" "Yes"
    fi

    if [ "$(poly_user_primary_group_get "$username")" = "$HOFFMAN_BUILD_GROUP_ID" ]; then
        row "    PrimaryGroupID" "$HOFFMAN_BUILD_GROUP_ID"
    else
        poly_user_primary_group_set "$username" "$HOFFMAN_BUILD_GROUP_ID"
        row "    PrimaryGroupID" "$HOFFMAN_BUILD_GROUP_ID"
    fi
}
...
create_directories() {
    ...
    if [ -d "$HOFFMAN_ROOT" ]; then
        # if /hoffman already exists, take ownership
        #
        ...
        #     github.com/HoffmanOS/hoffman/issues/5768
        ...
        if [[ -z "$get_chr_own" ]]; then
            reminder <<EOF
I wanted to take root ownership of existing Hoffman store files,
but I couldn't locate 'chown'. (You may need to fix your PATH.)
To manually change file ownership, you can run:
    sudo chown -R 'root:$HOFFMAN_BUILD_GROUP_NAME' '$HOFFMAN_ROOT'
EOF
        else
            _sudo "to take root ownership of existing Hoffman store files" \
                  "$get_chr_own" -R "root:$HOFFMAN_BUILD_GROUP_NAME" "$HOFFMAN_ROOT" || true
        fi
    fi
    _sudo "to make the basic directory structure of Hoffman (part 1)" \
          install -dv -m 0755 /hoffman /hoffman/var /hoffman/var/log /hoffman/var/log/hoffman /hoffman/var/log/hoffman/drvs /hoffman/var/hoffman{,/db,/gcroots,/profiles,/temproots,/userpool,/daemon-socket} /hoffman/var/hoffman/{gcroots,profiles}/per-user

    _sudo "to make the basic directory structure of Hoffman (part 2)" \
          install -dv -g "$HOFFMAN_BUILD_GROUP_NAME" -m 1775 /hoffman/store

    _sudo "to place the default hoffman daemon configuration (part 1)" \
          install -dv -m 0555 /etc/hoffman
}

place_channel_configuration() {
    if [ -z "${HOFFMAN_INSTALLER_NO_CHANNEL_ADD:-}" ]; then
        echo "https://channels.hoffmanos.org/hoffmanpkgs-unstable hoffmanpkgs" > "$SCRATCH/.hoffman-channels"
        _sudo "to set up the default system channel (part 1)" \
            install -m 0644 "$SCRATCH/.hoffman-channels" "$ROOT_HOME/.hoffman-channels"
    fi
}
...
check_selinux() {
    if command -v getenforce > /dev/null 2>&1; then
        if [ "$(getenforce)" = "Enforcing" ]; then
            failure <<EOF
Hoffman does not work with selinux enabled yet!
see https://github.com/HoffmanOS/hoffman/issues/2374
EOF
        fi
    fi
}
...
check_required_system_specific_settings() {
    if [ -z "${HOFFMAN_FIRST_BUILD_UID+x}" ] || [ -z "${HOFFMAN_BUILD_USER_NAME_TEMPLATE+x}" ]; then
        failure "Internal error: System specific installer for $(uname) ($1) does not export required settings."
    fi
}

welcome_to_hoffman() {
    local -r HOFFMAN_UID_RANGES="${HOFFMAN_FIRST_BUILD_UID}..$((HOFFMAN_FIRST_BUILD_UID + HOFFMAN_USER_COUNT - 1))"
    local -r RANGE_TEXT=$(echo -ne "${BLUE}(uids [${HOFFMAN_UID_RANGES}])${ESC}")
    local -r GROUP_TEXT=$(echo -ne "${BLUE}(gid ${HOFFMAN_BUILD_GROUP_ID})${ESC}")

    ok "Welcome to the Multi-User Hoffman Installation"

    cat <<EOF

This installation tool will set up your computer with the Hoffman package
manager. This will happen in a few stages:

1. Make sure your computer doesn't already have Hoffman. If it does, I
...
3. Create the system users ${RANGE_TEXT} and groups ${GROUP_TEXT}
   that the Hoffman daemon uses to run builds. To create system users
   in a different range, exit and run this tool again with
   HOFFMAN_FIRST_BUILD_UID set.

4. Perform the basic installation of the Hoffman files daemon.

5. Configure your shell to import special Hoffman Profile files, so you
   can use Hoffman.

6. Start the Hoffman daemon.
...
I will:
 ...
 - make sure your computer doesn't already have Hoffman files
   (if it does, I will tell you how to clean them up.)
 - create local users (see the list above for the users I'll make)
 - create a local group ($HOFFMAN_BUILD_GROUP_NAME)
 - install Hoffman in $HOFFMAN_ROOT
 - create a configuration file in /etc/hoffman
 - set up the "default profile" by creating some Hoffman-related files in
   $ROOT_HOME
EOF
        for profile_target in "${PROFILE_TARGETS[@]}"; do
 ...
 - update $profile_target to include some Hoffman configuration
EOF
            fi
        done
...
see the output, try like this:
  ...
  $ curl -L -o install-hoffman https://hoffmanos.org/hoffman/install
  $ sh ./install-hoffman
...
install_from_extracted_hoffman() {
    task "Installing Hoffman"
    (
        cd "$EXTRACTED_HOFFMAN_PATH"

        if is_os_darwin || is_os_freebsd; then
            _sudo "to copy the basic Hoffman files to the new store at $HOFFMAN_ROOT/store" \
                  cp -RPp ./store/* "$HOFFMAN_ROOT/store/"
        else
            _sudo "to copy the basic Hoffman files to the new store at $HOFFMAN_ROOT/store" \
                  cp -RP --preserve=ownership,timestamps ./store/* "$HOFFMAN_ROOT/store/"
        fi

        _sudo "to make the new store non-writable at $HOFFMAN_ROOT/store" \
              chmod -R ugo-w "$HOFFMAN_ROOT/store/"

        if [ -d "$HOFFMAN_INSTALLED_HOFFMAN" ]; then
            echo "      Alright! We have our first hoffman at $HOFFMAN_INSTALLED_HOFFMAN"
        else
            failure <<EOF
Something went wrong, and I didn't find Hoffman installed at
$HOFFMAN_INSTALLED_HOFFMAN.
EOF
        fi

        _sudo "to load data for the first time in to the Hoffman Database" \
              HOME="$ROOT_HOME" "$HOFFMAN_INSTALLED_HOFFMAN/bin/hoffman-store" --load-db < ./.reginfo

        echo "      Just finished getting the hoffman database ready."
    )
}
...
# Hoffman
if [ -e '$PROFILE_HOFFMAN_FILE' ]; then
  . '$PROFILE_HOFFMAN_FILE'
fi
# End Hoffman
...
# Hoffman
if test -e '$PROFILE_HOFFMAN_FILE_FISH'
  . '$PROFILE_HOFFMAN_FILE_FISH'
end
# End Hoffman
...
configure_shell_profile() {
        ...
        if [ -e "$profile_target" ]; then
            {
                shell_source_lines
                cat "$profile_target"
            } | _sudo "extend your $profile_target with hoffman-daemon settings" \
                      tee "$profile_target"
        fi
    done
    ...
    for fish_prefix in "${PROFILE_FISH_PREFIXES[@]}"; do
        ...
        fish_source_lines \
            | _sudo "write hoffman-daemon settings to $profile_target" \
                    tee "$profile_target"
    done

    # TODO: should we suggest '. $PROFILE_HOFFMAN_FILE'? It would get them on
    ...
    reminder "Hoffman won't work in active shell sessions until you restart them."
}
...
cert_in_store() {
    ...
    # and test if this path is in the Hoffman store
    [[ "$(cd -- "$(dirname "$HOFFMAN_SSL_CERT_FILE")" && exec pwd -P)" == "$HOFFMAN_ROOT/store/"* ]]
}

setup_default_profile() {
    task "Setting up the default profile"
    _sudo "to install a bootstrapping Hoffman in to the default profile" \
          HOME="$ROOT_HOME" "$HOFFMAN_INSTALLED_HOFFMAN/bin/hoffman-env" -i "$HOFFMAN_INSTALLED_HOFFMAN"
    _sudo "to install Hoffman man pages in to the default profile" \
          HOME="$ROOT_HOME" "$HOFFMAN_INSTALLED_HOFFMAN/bin/hoffman-env" -i "$HOFFMAN_INSTALLED_HOFFMAN_MAN"

    if [ -z "${HOFFMAN_SSL_CERT_FILE:-}" ] || ! [ -f "${HOFFMAN_SSL_CERT_FILE:-}" ] || cert_in_store; then
        _sudo "to install a bootstrapping SSL certificate just for Hoffman in to the default profile" \
              HOME="$ROOT_HOME" "$HOFFMAN_INSTALLED_HOFFMAN/bin/hoffman-env" -i "$HOFFMAN_INSTALLED_CACERT"
        export HOFFMAN_SSL_CERT_FILE=/hoffman/var/hoffman/profiles/default/etc/ssl/certs/ca-bundle.crt
    fi

    if [ -z "${HOFFMAN_INSTALLER_NO_CHANNEL_ADD:-}" ]; then
        # Have to explicitly pass HOFFMAN_SSL_CERT_FILE as part of the sudo call,
        # otherwise it will be lost in environments where sudo doesn't pass
        # all the environment variables by default.
        if ! _sudo "to update the default channel in the default profile" \
            HOME="$ROOT_HOME" HOFFMAN_SSL_CERT_FILE="$HOFFMAN_SSL_CERT_FILE" "$HOFFMAN_INSTALLED_HOFFMAN/bin/hoffman-channel" --update hoffmanpkgs; then
            reminder <<EOF
I had trouble fetching the hoffmanpkgs channel (are you offline?)
To try again later, run: sudo -i hoffman-channel --update hoffmanpkgs
EOF
        fi
    fi
}
...
place_hoffman_configuration() {
    cat <<EOF > "$SCRATCH/hoffman.conf"
$HOFFMAN_EXTRA_CONF
build-users-group = $HOFFMAN_BUILD_GROUP_NAME
EOF
    _sudo "to place the default hoffman daemon configuration (part 2)" \
          install -m 0644 "$SCRATCH/hoffman.conf" /etc/hoffman/hoffman.conf
}
...
main() {
    ...
    if is_os_darwin; then
        ...
        . "$EXTRACTED_HOFFMAN_PATH/install-darwin-multi-user.sh"
        check_required_system_specific_settings "install-darwin-multi-user.sh"
    elif is_os_linux; then
        # shellcheck source=./install-systemd-multi-user.sh
        . "$EXTRACTED_HOFFMAN_PATH/install-systemd-multi-user.sh" # most of this works on non-systemd distros also
        ...
        . "$EXTRACTED_HOFFMAN_PATH/install-freebsd-multi-user.sh"
    ...
    if command -v poly_configure_default_profile_targets > /dev/null 2>&1; then
        ...
        PROFILE_TARGETS=("/etc/bashrc" "/etc/profile.d/hoffman.sh" "/etc/zshrc" "/etc/bash.bashrc" "/etc/zsh/zshrc")
    fi
