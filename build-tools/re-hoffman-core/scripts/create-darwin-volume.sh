...
if [ "${1-}" = "no-main" ]; then
    ...
    #
    readonly HOFFMAN_ROOT="${HOFFMAN_ROOT:-/hoffman}"
...
readonly HOFFMAN_VOLUME_FS="${HOFFMAN_VOLUME_FS:-APFS}"
readonly HOFFMAN_VOLUME_LABEL="${HOFFMAN_VOLUME_LABEL:-Hoffman Store}"
# Strongly assuming we'll make a volume on the device / is on
# But you can override HOFFMAN_VOLUME_USE_DISK to create it on some other device
readonly HOFFMAN_VOLUME_USE_DISK="${HOFFMAN_VOLUME_USE_DISK:-$(root_disk_identifier)}"
HOFFMAN_VOLUME_USE_SPECIAL="${HOFFMAN_VOLUME_USE_SPECIAL:-}"
HOFFMAN_VOLUME_USE_UUID="${HOFFMAN_VOLUME_USE_UUID:-}"
readonly HOFFMAN_VOLUME_MOUNTD_DEST="${HOFFMAN_VOLUME_MOUNTD_DEST:-/Library/LaunchDaemons/org.hoffmanos.darwin-store.plist}"
...
if /usr/bin/fdesetup isactive >/dev/null; then
    ...
    HOFFMAN_VOLUME_DO_ENCRYPT="${HOFFMAN_VOLUME_DO_ENCRYPT:-1}"
else
    test_filevault_in_use() { return 1; }
    HOFFMAN_VOLUME_DO_ENCRYPT="${HOFFMAN_VOLUME_DO_ENCRYPT:-0}"
fi

should_encrypt_volume() {
    test_filevault_in_use && (( HOFFMAN_VOLUME_DO_ENCRYPT == 1 ))
}
...
right_disk() {
    ...
    [[ "$volume_special" == "$HOFFMAN_VOLUME_USE_DISK"s* ]]
}

right_volume() {
    local volume_special="$1" # (i.e., disk1s7)
    # if set, it must match; otherwise ensure it's on the right disk
    if [ -z "$HOFFMAN_VOLUME_USE_SPECIAL" ]; then
        if right_disk "$volume_special"; then
            HOFFMAN_VOLUME_USE_SPECIAL="$volume_special" # latch on
            return 0
        else
            return 1
        fi
    else
        [ "$volume_special" = "$HOFFMAN_VOLUME_USE_SPECIAL" ]
    fi
}
...
right_uuid() {
    ...
    if [ -z "$HOFFMAN_VOLUME_USE_UUID" ]; then
        HOFFMAN_VOLUME_USE_UUID="$volume_uuid" # latch on
        return 0
    else
        [ "$volume_uuid" = "$HOFFMAN_VOLUME_USE_UUID" ]
    fi
}
...
cure_volumes() {
    ...
    for volume in $(volumes_labeled "$HOFFMAN_VOLUME_LABEL"); do
        # CAUTION: this could (maybe) be a more normal read
        # loop like:
        #   while IFS== read -r special uuid; do
        #       # ...
        #   done <<<"$(volumes_labeled "$HOFFMAN_VOLUME_LABEL")"
        #
        ...
        # Hoffman Store volume, and have deleted its credential
...
disk=${HOFFMAN_VOLUME_USE_DISK} special=${HOFFMAN_VOLUME_USE_SPECIAL:-${HOFFMAN_VOLUME_USE_DISK}sX} uuid=${HOFFMAN_VOLUME_USE_UUID:-any}
EOF
...
EOF
    ...
    if [ -z "${found:-}" ]; then
        readonly HOFFMAN_VOLUME_USE_SPECIAL HOFFMAN_VOLUME_USE_UUID
    fi
}
...
test_fstab() {
    /usr/bin/grep -q "$HOFFMAN_ROOT apfs rw" /etc/fstab 2>/dev/null
}

test_hoffman_root_is_symlink() {
    [ -L "$HOFFMAN_ROOT" ]
}

test_synthetic_conf_either(){
    /usr/bin/grep -qE "^${HOFFMAN_ROOT:1}($|\t.{3,}$)" /etc/synthetic.conf 2>/dev/null
}

test_synthetic_conf_mountable() {
    /usr/bin/grep -q "^${HOFFMAN_ROOT:1}$" /etc/synthetic.conf 2>/dev/null
}
...
test_hoffman_volume_mountd_installed() {
    test -e "$HOFFMAN_VOLUME_MOUNTD_DEST"
}
...
test_voldaemon() {
    test -f "$HOFFMAN_VOLUME_MOUNTD_DEST"
}
...
generate_mount_command() {
    ...
    printf -v mountpoint "%q" "$HOFFMAN_ROOT"
...
<dict>
  ...
  <true/>
  <key>Label</key>
  <string>org.hoffmanos.darwin-store</string>
...
uninstall_launch_daemon_directions() {
    local daemon_label="$1" # i.e., org.hoffmanos.blah-blah
...
hoffman_volume_mountd_uninstall_directions() {
    uninstall_launch_daemon_directions "org.hoffmanos.darwin-store" \
        "$HOFFMAN_VOLUME_MOUNTD_DEST"
}

hoffman_volume_mountd_uninstall_prompt() {
    uninstall_launch_daemon_prompt "org.hoffmanos.darwin-store" \
        "$HOFFMAN_VOLUME_MOUNTD_DEST" \
        "mount your Hoffman volume"
}

# TODO: move hoffman_daemon to install-darwin-multi-user if/when uninstall_launch_daemon_prompt moves up to install-multi-user
hoffman_daemon_uninstall_prompt() {
    uninstall_launch_daemon_prompt "org.hoffmanos.hoffman-daemon" \
        "$HOFFMAN_DAEMON_DEST" \
        "run the hoffman-daemon"
}

# TODO: remove with --uninstall?
hoffman_daemon_uninstall_directions() {
    uninstall_launch_daemon_directions "org.hoffmanos.hoffman-daemon" \
        "$HOFFMAN_DAEMON_DEST"
}
...
synthetic_conf_uninstall_directions() {
    ...
    substep "Remove ${HOFFMAN_ROOT:1} from /etc/synthetic.conf" \
        "  If hoffman is the only entry: sudo rm /etc/synthetic.conf" \
        "  Otherwise: sudo /usr/bin/sed -i '' -e '/^${HOFFMAN_ROOT:1}$/d' /etc/synthetic.conf"
}

synthetic_conf_uninstall_prompt() {
    cat <<EOF

During install, I add '${HOFFMAN_ROOT:1}' to /etc/synthetic.conf, which instructs
macOS to create an empty root directory for mounting the Hoffman volume.
EOF
    # make the edit to a copy
    /usr/bin/grep -vE "^${HOFFMAN_ROOT:1}($|\t.{3,}$)" /etc/synthetic.conf > "$SCRATCH/synthetic.conf.edit"

    if test_synthetic_conf_symlinked; then
        warning <<EOF

/etc/synthetic.conf already contains a line instructing your system
to make '${HOFFMAN_ROOT}' as a symlink:
    $(/usr/bin/grep -nE "^${HOFFMAN_ROOT:1}\t.{3,}$" /etc/synthetic.conf)

This may mean your system has/had a non-standard Hoffman install.
...
EOF
    ...
    if /usr/bin/diff -q <(:) <(/usr/bin/grep -v "^#" "$SCRATCH/synthetic.conf.edit") &>/dev/null; then
        if confirm_rm "/etc/synthetic.conf"; then
            if test_hoffman_root_is_symlink; then
                failure >&2 <<EOF
I removed /etc/synthetic.conf, but $HOFFMAN_ROOT is already a symlink
(-> $(readlink "$HOFFMAN_ROOT")). The system should remove it when you reboot.
...
EOF
        ...
        if confirm_edit "$SCRATCH/synthetic.conf.edit" "/etc/synthetic.conf"; then
            if test_hoffman_root_is_symlink; then
                failure >&2 <<EOF
I edited Hoffman out of /etc/synthetic.conf, but $HOFFMAN_ROOT is already a symlink
(-> $(readlink "$HOFFMAN_ROOT")). The system should remove it when you reboot.
Once you've rebooted, run the installer again.
EOF
            fi
    ...
    echo "Manually remove hoffman from /etc/synthetic.conf"
    return 1
}

add_hoffman_vol_fstab_line() {
    local uuid="$1"
    # shellcheck disable=SC1003,SC2026
    local escaped_mountpoint="${HOFFMAN_ROOT/ /'\\\'040}"
    shift

    # wrap `ex` to work around problems w/ vim features breaking exit codes
    # - plugins (see github.com/HoffmanOS/hoffman/issues/5468): -u NONE
    # - swap file: -n
    #
    # the first draft used `--noplugin`, but github.com/HoffmanOS/hoffman/issues/6462
...
EOF
    ...
    EDITOR="$SCRATCH/ex_cleanroom_wrapper" _sudo "to add hoffman to fstab" "$@" <<EOF
:a
...
EOF
    ...
    # There *may* be some issue regarding volume ownership, see hoffman#3156
    #
    ...
    # diskutil info -plist "$HOFFMAN_VOLUME_LABEL" | xmllint --xpath "(/plist/dict/key[text()='GlobalPermissionsEnabled'])/following-sibling::*[1][name()='true']" -; echo $?
    #
...
delete_hoffman_vol_fstab_line() {
    # TODO: I'm scaffolding this to handle the new hoffman volumes
    # but it might be nice to generalize a smidge further to
    # go ahead and set up a pattern for curing "old" things
    # we no longer do?
    EDITOR="/usr/bin/patch" _sudo "to cut hoffman from fstab" "$@" < <(/usr/bin/diff /etc/fstab <(/usr/bin/grep -v "$HOFFMAN_ROOT apfs rw" /etc/fstab))
...
fstab_uninstall_directions() {
    substep "Remove ${HOFFMAN_ROOT} from /etc/fstab" \
      "  If hoffman is the only entry: sudo rm /etc/fstab" \
      "  Otherwise, run 'sudo /usr/sbin/vifs' to remove the hoffman line"
}

fstab_uninstall_prompt() {
    cat <<EOF
During install, I add '${HOFFMAN_ROOT}' to /etc/fstab so that macOS knows what
mount options to use for the Hoffman volume.
EOF
    ...
    delete_hoffman_vol_fstab_line patch "$SCRATCH/fstab.edit" &>/dev/null
    ...
    if /usr/bin/diff -q <(:) <(/usr/bin/grep -v "^#" "$SCRATCH/fstab.edit") &>/dev/null; then
        ...
        if confirm_rm "/etc/fstab"; then
            ...
            echo "Remove hoffman from /etc/fstab (or remove the file)"
        fi
    else
        ...
        if ! _diff "/etc/fstab" "$SCRATCH/fstab.edit" && ui_confirm "Does the change above look right?"; then
            delete_hoffman_vol_fstab_line /usr/sbin/vifs
        else
            echo "Remove hoffman from /etc/fstab (or remove the file)"
        fi
    fi
...
cure_volume() {
    local volume_special="$1" # (i.e., disk1s7)
    local volume_uuid="$2"
    header "Found existing Hoffman volume"
...
EOF
            if password_confirm "delete this volume"; then
                ...
                # remind idiom...
                failure <<EOF
Your Hoffman volume is encrypted, but I couldn't find its password. Either:
...
EOF
        ...
        row "encrypted" "no"
        warning <<EOF
FileVault is on, but your $HOFFMAN_VOLUME_LABEL volume isn't encrypted.
EOF
        # if we're interactive, give them a chance to
        # encrypt the volume. If not, /shrug
        if ! headless && (( HOFFMAN_VOLUME_DO_ENCRYPT == 1 )); then
            if ui_confirm "Should I encrypt it and add the decryption key to your keychain?"; then
                encrypt_volume "$volume_uuid" "$HOFFMAN_VOLUME_LABEL"
                HOFFMAN_VOLUME_DO_ENCRYPT=0
            else
                HOFFMAN_VOLUME_DO_ENCRYPT=0
                reminder "FileVault is on, but your $HOFFMAN_VOLUME_LABEL volume isn't encrypted."
            fi
        fi
    else
        row "encrypted" "no"
...
remove_volume_artifacts() {
    if test_synthetic_conf_either; then
        # HOFFMAN_ROOT is in synthetic.conf
        if synthetic_conf_uninstall_prompt; then
            ...
            # reminder "macOS will clean up the empty mount-point directory at $HOFFMAN_ROOT on reboot."
            :
        fi
    fi
    if test_fstab; then
    ...
    if test_hoffman_volume_mountd_installed; then
        hoffman_volume_mountd_uninstall_prompt
    fi
}
...
setup_synthetic_conf() {
    if test_hoffman_root_is_symlink; then
        if ! test_synthetic_conf_symlinked; then
            failure >&2 <<EOF
error: $HOFFMAN_ROOT is a symlink (-> $(readlink "$HOFFMAN_ROOT")).
Please remove it. If hoffman is in /etc/synthetic.conf, remove it and reboot.
EOF
        fi
    fi
    if ! test_synthetic_conf_mountable; then
        task "Configuring /etc/synthetic.conf to make a mount-point at $HOFFMAN_ROOT" >&2
        # technically /etc/synthetic.d/hoffman is supported in Big Sur+
        # but handling both takes even more code...
        # See earlier note; `-u NONE` disables vim plugins/rc, `-n` skips swapfile
        _sudo "to add Hoffman to /etc/synthetic.conf" \
...
EOF
        ...
        create_synthetic_objects
        if ! test_hoffman; then
            failure >&2 <<EOF
error: failed to bootstrap $HOFFMAN_ROOT
...
setup_fstab() {
    ...
    if ! test_fstab; then
        ...
        add_hoffman_vol_fstab_line "$volume_uuid" /usr/sbin/vifs
    fi
}
...
encrypt_volume() {
    ...
    task "Encrypt the Hoffman volume" >&2
    ...
    _sudo "to mount your Hoffman volume for encrypting" \
        /usr/sbin/diskutil mount "$volume_label"

    password="$(/usr/bin/xxd -l 32 -p -c 256 /dev/random)"
    _sudo "to add your Hoffman volume's password to Keychain" \
        /usr/bin/security -i <<EOF
add-generic-password -a "$volume_label" -s "$volume_uuid" -l "$volume_label encryption password" -D "Encrypted volume password" -j "Added automatically by the Hoffman installer for use by $HOFFMAN_VOLUME_MOUNTD_DEST" -w "$password" -T /System/Library/CoreServices/APFSUserAgent -T /System/Library/CoreServices/CSUserAgent -T /usr/bin/security "/Library/Keychains/System.keychain"
EOF
    builtin printf "%s" "$password" | _sudo "to actually encrypt your Hoffman volume" \
...
create_volume() {
    # Notes:
    # 1) using `-nomount` instead of `-mountpoint "$HOFFMAN_ROOT"` to get
    ...
    # anything useful with Hoffman, it is possibly to specify
    ...
    _sudo "to create a new APFS volume '$HOFFMAN_VOLUME_LABEL' on $HOFFMAN_VOLUME_USE_DISK" \
        /usr/sbin/diskutil apfs addVolume "$HOFFMAN_VOLUME_USE_DISK" "$HOFFMAN_VOLUME_FS" "$HOFFMAN_VOLUME_LABEL" -nomount | /usr/bin/awk '/Created new APFS Volume/ {print $5}'
}
...
await_volume() {
    ...
    until /usr/sbin/diskutil info "$HOFFMAN_ROOT" &>/dev/null; do
        :
    done
}
...
setup_volume() {
    ...
    task "Creating a Hoffman volume" >&2

    use_special="${HOFFMAN_VOLUME_USE_SPECIAL:-$(create_volume)}"

    _sudo "to ensure the Hoffman volume is not mounted" \
        /usr/sbin/diskutil unmount force "$use_special" || true # might not be mounted

    use_uuid=${HOFFMAN_VOLUME_USE_UUID:-$(volume_uuid_from_special "$use_special")}

    setup_fstab "$use_uuid"

    if should_encrypt_volume; then
        encrypt_volume "$use_uuid" "$HOFFMAN_VOLUME_LABEL"
    ...
    if [ "$(/usr/sbin/diskutil info -plist "$HOFFMAN_ROOT" | xmllint --xpath "(/plist/dict/key[text()='GlobalPermissionsEnabled'])/following-sibling::*[1]" -)" = "<false/>" ]; then
        _sudo "to set enableOwnership (enabling users to own files)" \
            /usr/sbin/diskutil enableOwnership "$HOFFMAN_ROOT"
    fi
    ...
    if type -p hoffman-env; then
        profile_packages="$(hoffman-env --query --installed)"
        ...
        if ! [ $(printf "$profile_packages" | /usr/bin/wc -l) = "0" ]; then
            reminder <<EOF
Hoffman now supports only multi-user installs on Darwin/macOS, and your user's
Hoffman profile has some packages in it. These packages may obscure those in the
default profile, including the Hoffman this installer will add. You should
...
setup_volume_daemon() {
    ...
    if ! test_voldaemon; then
        task "Configuring LaunchDaemon to mount '$HOFFMAN_VOLUME_LABEL'" >&2
        # See earlier note; `-u NONE` disables vim plugins/rc, `-n` skips swapfile
        _sudo "to install the Hoffman volume mounter" /usr/bin/ex -u NONE -n "$HOFFMAN_VOLUME_MOUNTD_DEST" <<EOF
:a
...
EOF
        ...
        _sudo "to launch the Hoffman volume mounter" \
            launchctl bootstrap system "$HOFFMAN_VOLUME_MOUNTD_DEST" || true
        ...
        _sudo "to launch the Hoffman volume mounter" \
            launchctl kickstart -k system/org.hoffmanos.darwin-store
    fi
}
    ...
    main() {
        {
            echo ""
            echo "     ------------------------------------------------------------------ "
            echo "    | This installer will create a volume for the hoffman store and        |"
            echo "    | configure it to mount at $HOFFMAN_ROOT.  Follow these steps to uninstall. |"
            echo "     ------------------------------------------------------------------ "
            echo ""
            echo "  1. Remove the entry from fstab using 'sudo /usr/sbin/vifs'"
            echo "  2. Run 'sudo launchctl bootout system/org.hoffmanos.darwin-store'"
            echo "  3. Remove $HOFFMAN_VOLUME_MOUNTD_DEST"
            echo "  4. Destroy the data volume using '/usr/sbin/diskutil apfs deleteVolume'"
            echo "  5. Remove the 'hoffman' line from /etc/synthetic.conf (or the file)"
            echo ""
        } >&2
